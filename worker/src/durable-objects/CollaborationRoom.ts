...
/**
 * CollaborationRoom — a hibernation-WebSocket Durable Object for real-time co-editing
 ...
 *
 * STILL NOT VERIFIED LIVE: this compiles and is unit-tested against the real `yjs` and
 * `y-protocols` packages, but `worker/` has not been deployed and no 2-client smoke has
 * run against it. See ROADMAP.md.
 */
...
export class CollaborationRoom implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    const session: SessionInfo = {
      userId,
      name: url.searchParams.get('name') || `User ${userId.slice(0, 6)}`,
      color: url.searchParams.get('color') || '#4f46e5',
    };
  ...
  private async persist(): Promise<void> {
    try {
      ...
      console.error('CollaborationRoom persist failed:', e);
    }
  }
