## Integration Audit Contract (Health Endpoint — /api/v1/audit/health/:projectId)
...
Expose the integration health deduction (DTO and endpoint contract) used by `AuditDashboard` for GET `/api/v1/audit/health/:projectId`, aligned to the schema in `docs/information-security/14-integration-audit-schema.md` (IntegrationHealth) and the DTOs in `Builderforce.ai/frontend/src/lib/dto/auditQueryOptions.ts`.

### 2. Endpoint

```
GET /api/v1/audit/health/:projectId?integrationType?=&status?=&minScore?=&maxScore?=&includeGaps&includeRecommendations&sortBy=&sortOrder=
```

- `projectId` is a required path parameter (numeric project ID). Returns data for that project only.
- The endpoint returns a JSON summary (200 OK) and errors (4xx/5xx) per JSON-API conventions.

### 3. Request Headers

The endpoint requires standard auth (token) via existing middleware; project-level access is enforced through the existing role-based access primitives.
...
|---------------|-------------------------------|-----------------------------------------------------------|-------------------------------------------------------------------|
| projectId     | Yes (path param)              | Project identifier                                        | Numeric                                                            |
| integrationType | Optional                     | Filter integrations by IntegrationType                    | "source-control", "issue-tracker", "communication", "cicd", "monitoring", "calendar" |
...
Score calculation behavior is outlined in `specs/builderforce/10-prd-integration-audit.md` (section 4.3) and `docs/information-security/14-integration-audit-schema.md` (model IntegrationCompletenessScore). The backend must align `completenessScore` returned by this contract with the schema and PRD. Assumptions for this turn: use defined weights for criticality and apply recency thresholds as defined in schema and PRD.
...
- Project-scoped: `projectId` is a path parameter, and the returned integration health data is scoped to that project.
...
| docs/information-security/14-integration-audit-schema.md | Schema model                           | Schema defines models; endpoint contract's IntegrationHealth↔IntegrationConnection relations and score calculations align with schema. |
| Builderforce.ai/frontend/src/components/integration/AuditDashboard.tsx | Dashboard component                    | Consumes /api/v1/audit/health/:projectId; ensures the JSON contract matches component expectations. |
