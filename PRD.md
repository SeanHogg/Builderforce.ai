...
| 14 | `b34e4a70` | Marketplace Upwork-competitive | First $ of GMV transacted through the marketplace | **Partial** | `api/src/presentation/routes/marketplaceRoutes.ts` (698 lines) — public skills marketplace, auth, profiles; `api/src/presentation/routes/marketplaceStatsRoutes.ts` (181 lines) — marketplace statistics. The marketplace listing and discovery layer is built. However, `ROADMAP.md` gap register explicitly states: "Payments/payouts still ride the existing stub (`isPayoutsConfigured`) — a fixed-bid gig has no escrow/milestone flow." GMV transaction capability is not yet live. |
| 15 | `097b01f2` | Marketplace Upwork-competitive | Close P0/P1 marketplace gaps from the Upwork analysis | **Partial** | `ROADMAP.md` Upwork-parity gap audit lists: (P0) Fixed-price contracts + milestones + escrow — "only hourly exists … no milestone/escrow model"; (P1) Richer job posting + proposal fields — "Jobs lack hourly-vs-fixed, budget/experience level, project length, screening questions, and attachments." Several P0/P1 gaps are explicitly logged as not yet closed. |
| 16 | `8a97a09c` | Marketplace Upwork-competitive | Payments move end-to-end: charge → escrow → payout live in production | **Partial** | Charge: `api/src/infrastructure/payment/StripeProvider.ts` — subscription billing is live. Escrow: `ROADMAP.md` confirms "no escrow/milestone model" — the word "escrow" appears only in the gap register. Payout: `api/src/env.ts` defines `PAYOUT_WEBHOOK_URL` as optional; `api/src/application/integrations/payments.ts` — `isPayoutsConfigured()` gates on it, `createPayout()` POSTs to it. The charge leg exists; escrow is absent; payout is env-gated stub. |
...
| **Partial** | 12 | Evermind bench score (4), retrain/transfer loop (5), delta extraction report (7), baseline snapshots (8), transfer uplift (10), marketplace GMV (14), Upwork gaps (15), end-to-end payments (16), new LLM providers (24), advanced reasoning (26) |
...
3. **Marketplace monetization (13–16)**: Subscription billing is `Implemented` (Stripe + Helcim providers). The marketplace has listings (`marketplaceRoutes.ts`, 698 lines) and stats (`marketplaceStatsRoutes.ts`, 181 lines), but escrow and payout are explicitly deferred per `ROADMAP.md`. Stripe handles the charge leg; escrow has zero code; payout is an env-gated stub (`isPayoutsConfigured` in `payments.ts`).
