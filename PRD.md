...
The `taskPrStatusTool` is implemented in `agent-runtime/src/builderforce/tools/task-pr-status-tracker-tool.ts` as an `AgentTool` that:

1. Accepts `taskIds` (string[]), `owner` (GitHub org/user), and optional `repo`.
2. Queries the GitHub REST API using three complementary search strategies:
   - **Strategy 1** — Issue/PR search: `"TASK_ID" type:pr repo:owner/repo in:title,body`
   - **Strategy 2** — Open PRs list, filtered by title/body/branch-name substring match
   - **Strategy 3** — Recently-closed PRs list, same filter
3. Classifies each PR as `Open`, `Merged`, or `Closed (Unmerged)` based on `state` + `merged_at`.
4. Produces a `TrackerReport` with per-task status + aggregate summary counts.

The tool is registered in the shared tool registry via `builderforce-tools.ts` and exported from `tools/index.ts`. A unit test suite (`task-pr-status-tracker-tool.test.ts`) covers the pure classification, dedup, and report-aggregation logic.
