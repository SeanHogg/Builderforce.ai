...
**Task #687 outcome:** The PRD describes a generic reactive-system primitive (signal graph / scheduler / createEffect / vi.fn() spy). A targeted search in seanhogg/builderforce.ai found NO such primitive: `createEffect` returned 0 matches; the repo is the BuilderForce platform (agent orchestration, not a reactive framework). The only real side effect triggered by an assignment in this codebase is the board's **lane auto-run dispatch** (maybeAutoRunOnLaneEntry → decideLaneAutoRun). That mechanism is already isolated with guards (same-lane guard, in-flight execution guard) and is guarded against duplicate agents. **Outcome:** AC-style tests were written and added to api/src/application/swimlane/laneAutoRun.test.ts to verify the lane-auto-run invariant (single dispatch per entry, independent lanes don't cross-fire, no owner double-add). To make these tests behave like PRD-style i/o tests, they still rely on the underlying action logic (no vi.fn()-based spy needed). If the PRD intends to test a NEW reactive primitive beyond the existing lane auto-run, that primitive must first be designed/targeted in a separate task/PRD. This task addressed the lane-auto-run portion as it's the closest real analog present in the codebase. (Original Test Evidence section was a placeholder.)