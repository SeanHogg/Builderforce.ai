...
- `frontend/src/lib/inputMode.ts` — shared FormSchema types, validation (`validateField` / `validateGroup` / `validateAll`), paste/upload parsers, mode persistence (`getStoredInputMode` / `setStoredInputMode`), URL prefill (`parseQueryPrefill`), template storage, analytics wrappers (`trackInputModeEvent` via `trackActivity`).
- `frontend/src/components/GuidedInput.tsx` — step-by-step flow: progress indicator, per-step validation, back navigation, help tooltips, review step with edit links, focus management, retryable submit error.
- `frontend/src/components/ExpressInput.tsx` — single-screen form: same FieldGroups, paste-to-fill popover, CSV/JSON upload with unmapped-column feedback, consolidated error summary with field focus links, drag-drop.
- `frontend/src/components/InputModeForm.tsx` — top-level orchestrator: reads persisted mode preference, preserves field values across mode switches, exposes single `onSubmit` endpoint, live region for screen reader announcements.
