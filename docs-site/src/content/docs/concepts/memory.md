---
title: "Memory"
summary: "How BuilderForce Agents memory works (workspace files + automatic memory flush)"
read_when:
...
BuilderForce Agents memory is **plain Markdown in the agent workspace**. The files are the
...
`~/.builderforce/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.
...
When a session is **close to auto-compaction**, BuilderForce Agents triggers a **silent,
...
BuilderForce Agents can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, BuilderForce Agents auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. BuilderForce Agents
...
BM25 + vectors + reranking. Markdown stays the source of truth; BuilderForce Agents shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.builderforce/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.builderforce/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, BuilderForce Agents retries with `qmd query`. If QMD fails or the binary is
  missing, BuilderForce Agents automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- BuilderForce Agents does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - BuilderForce Agents sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index BuilderForce Agents
    uses), run a one-off query with the agent’s XDG dirs.

    BuilderForce Agents’s QMD state lives under your **state dir** (defaults to `~/.builderforce`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    BuilderForce Agents uses:

    ```bash
    # Pick the same state dir BuilderForce Agents uses
    STATE_DIR="${BUILDERFORCE_AGENTS_STATE_DIR:-$HOME/.builderforce}"
...
- When `scope` denies a search, BuilderForce Agents logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, BuilderForce Agents exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.builderforce/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- Index storage: per-agent SQLite at `~/.builderforce/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, BuilderForce Agents automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, BuilderForce Agents combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, BuilderForce Agents falls back to vector-only search.
...
BuilderForce Agents can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.builderforce/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, BuilderForce Agents stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, BuilderForce Agents logs the
