...
read_when:
  - You want to understand what “context” means in BuilderForce Agents
...
“Context” is **everything BuilderForce Agents sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (BuilderForce Agents-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How BuilderForce Agents builds the system prompt

The system prompt is **BuilderForce Agents-owned** and rebuilt each run. It includes:
...
By default, BuilderForce Agents injects a fixed set of workspace files (if present):
