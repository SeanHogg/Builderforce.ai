...
Deploy BuilderForce Agents on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/builderforce-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/builderforce` — Control UI

## What you get

- Hosted BuilderForce Agents Gateway + Control UI
...
- `BUILDERFORCE_AGENTS_STATE_DIR=/data/.builderforce` (recommended)
- `BUILDERFORCE_AGENTS_WORKSPACE_DIR=/data/workspace` (recommended)
- `BUILDERFORCE_AGENTS_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your BuilderForce Agents state + workspace so you can migrate to another host without losing config or memory.
