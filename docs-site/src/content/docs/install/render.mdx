...
Deploy BuilderForce Agents on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/SeanHogg/Builderforce.ai)
...
repository configures everything needed to run BuilderForce Agents:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: BUILDERFORCE_AGENTS_STATE_DIR
        value: /data/.builderforce
      - key: BUILDERFORCE_AGENTS_WORKSPACE_DIR
        value: /data/workspace
      - key: BUILDERFORCE_AGENTS_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: builderforce-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/builderforce`.
...
If you use the original BuilderForce Agents repository, Render will not auto-deploy your BuilderForce Agents. To update it, run a manual Blueprint sync from the dashboard.
...
For BuilderForce Agents, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any BuilderForce Agents host.
