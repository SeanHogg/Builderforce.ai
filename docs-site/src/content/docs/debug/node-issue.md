...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in BuilderForce Agents
...
Running BuilderForce Agents via Node with `tsx` fails at startup with:

```
[builderforce] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `builderforce status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch builderforce.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node builderforce.mjs status` works on Node 25.
