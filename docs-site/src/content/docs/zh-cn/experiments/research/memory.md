---
read_when:
  - 设计超越每日 Markdown 日志的工作区记忆（~/.builderforce/workspace）
  - Deciding: standalone CLI vs deep BuilderForce Agents integration
  - 添加离线回忆 + 反思（retain/recall/reflect）
summary: 研究笔记：BuilderForce Agents 工作区的离线记忆系统（Markdown 作为数据源 + 派生索引）
title: 工作区记忆研究
x-i18n:
...
目标：BuilderForce Agents 风格的工作区（`agents.defaults.workspace`，默认 `~/.builderforce/workspace`），其中"记忆"以每天一个 Markdown 文件（`memory/YYYY-MM-DD.md`）加上一小组稳定文件（例如 `memory.md`、`SOUL.md`）的形式存储。
...
- 以实体为中心的答案（"告诉我关于 Alice / The Castle / builderforce 的信息"）而无需重读多个文件
- 观点/偏好稳定性（以及变化时的证据）
...
保持 `~/.builderforce/workspace` 作为规范的人类可读记忆。
...
- `memory.md` 保持"小型 + 类似核心"：你希望 BuilderForce Agents 每次会话都能看到的内容。
...
```
~/.builderforce/workspace/.memory/index.sqlite
```
...
- B @builderforce: I fixed the Baileys WS crash by wrapping connection.update handlers in try/catch (see memory/2025-11-27.md).
...
- 实体：`@Peter`、`@builderforce` 等（slug 映射到 `bank/entities/*.md`）
...
建议：**深度集成到 BuilderForce Agents**，但保持可分离的核心库。

### 为什么要集成到 BuilderForce Agents？

- BuilderForce Agents 已经知道：
  - 工作区路径（`agents.defaults.workspace`）
  - 会话模型 + 心跳
  - 日志记录 + 故障排除模式
- 你希望智能体自己调用工具：
  - `builderforce memory recall "…" --k 25 --since 30d`
  - `builderforce memory reflect --since 7d`
...
对于 `~/.builderforce/workspace` 的务实观点：
