...
read_when:
  ...
  - 调试语音唤醒/会话生命周期问题
summary: BuilderForce Agents 日志：滚动诊断文件日志 + 统一日志隐私标志
title: macOS 日志
x-i18n:
...
BuilderForce Agents 通过 swift-log（默认使用统一日志）路由 macOS 应用日志，并且在需要持久化捕获时可以将本地轮转文件日志写入磁盘。
...
## 为 BuilderForce Agents 启用（`bot.molt`）
...
- 使用现有的辅助脚本查看更丰富的输出，例如 `./scripts/agentlog.sh --category WebChat --last 5m`。
