...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 CoderClaw 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `coderclaw webhooks` 获取 Gmail 助手命令。
...
Hook 包可以附带依赖；它们将安装在 `~/.coderclaw/hooks/<id>` 下。
