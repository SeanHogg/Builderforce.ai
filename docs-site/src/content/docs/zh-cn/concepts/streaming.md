...
BuilderForce Agents 有两个独立的"流式传输"层：
...
启用分块流式传输时，BuilderForce Agents 可以在发送前**合并连续的块分块**。这减少了"单行刷屏"，同时仍提供渐进式输出。
...
当草稿流式传输活跃时，BuilderForce Agents 会为该回复禁用分块流式传输以避免双重流式传输。
