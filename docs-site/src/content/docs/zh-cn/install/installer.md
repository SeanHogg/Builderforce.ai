...
BuilderForce Agents 提供两个安装器脚本（托管在 `builderforce.ai`）：

- `https://builderforce.ai/install.sh` — "推荐"安装器（默认全局 npm 安装；也可从 GitHub 检出安装）
- `https://builderforce.ai/install-cli.sh` — 无需 root 权限的 CLI 安装器（安装到带有独立 Node 的前缀目录）
- `https://builderforce.ai/install.ps1` — Windows PowerShell 安装器（默认 npm；可选 git 安装）
...
```powershell
& ([scriptblock]::Create((iwr -useb https://builderforce.ai/install.ps1))) -?
```
...
- 如果是升级现有安装：运行 `builderforce doctor --non-interactive`（尽力执行）。
- 对于 git 安装：安装/更新后运行 `builderforce doctor --non-interactive`（尽力执行）。
- 通过默认设置 `SHARP_IGNORE_GLOBAL_LIBVIPS=1` 来缓解 `sharp` 原生安装问题（避免使用系统 libvips 编译）。

如果你*希望* `sharp` 链接到全局安装的 libvips（或你正在调试），请设置：

```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://builderforce.ai/install.sh | bash
```

### 可发现性 / "git 安装"提示

如果你在**已有的 BuilderForce Agents 源码检出目录中**运行安装器（通过 `package.json` + `pnpm-workspace.yaml` 检测），它会提示：

- 更新并使用此检出（`git`）
- 或迁移到全局 npm 安装（`npm`）

在非交互式上下文中（无 TTY / `--no-prompt`），你必须传入 `--install-method git|npm`（或设置 `BUILDERFORCE_AGENTS_INSTALL_METHOD`），否则脚本将以退出码 `2` 退出。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g builderforce@latest`
  - `git`：克隆/构建源码检出并安装包装脚本
- 在升级和 git 安装时运行 `builderforce doctor --non-interactive`（尽力执行）。
...
```powershell
iwr -useb https://builderforce.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://builderforce.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\builderforce"
```
