...
read_when:
  ...
  - 你想要所有内容都固定并以声明式管理
summary: 使用 Nix 声明式安装 BuilderForce Agents
title: Nix
x-i18n:
...
使用 Nix 运行 BuilderForce Agents 的推荐方式是通过 **[nix-builderforce](https://github.com/builderforce/nix-builderforce)** — 一个开箱即用的 Home Manager 模块。
...
> **📦 完整指南：[github.com/builderforce/nix-builderforce](https://github.com/builderforce/nix-builderforce)**
>
> nix-builderforce 仓库是 Nix 安装的权威来源。本页只是一个快速概述。
...
当设置 `BUILDERFORCE_AGENTS_NIX_MODE=1` 时（nix-builderforce 会自动设置）：

BuilderForce Agents 支持 **Nix 模式**，使配置确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
BuilderForce Agents 从 `BUILDERFORCE_AGENTS_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `BUILDERFORCE_AGENTS_STATE_DIR` 中。

- `BUILDERFORCE_AGENTS_STATE_DIR`（默认：`~/.builderforce`）
- `BUILDERFORCE_AGENTS_CONFIG_PATH`（默认：`$BUILDERFORCE_AGENTS_STATE_DIR/builderforce.json`）
...
- [nix-builderforce](https://github.com/builderforce/nix-builderforce) — 完整设置指南
