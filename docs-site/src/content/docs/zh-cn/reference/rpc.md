...
BuilderForce Agents 通过 JSON-RPC 集成外部 CLI。目前使用两种模式。
...
- 健康探测：`/api/v1/check`。
- 当 `channels.signal.autoStart=true` 时，BuilderForce Agents 负责生命周期管理。

设置和端点参见 [Signal](/channels/signal)。

## 模式 B：stdio 子进程（imsg）

- BuilderForce Agents 将 `imsg rpc` 作为子进程生成。
