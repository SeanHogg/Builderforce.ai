...
- [ ] 验证注册表：`npm view builderforce version`、`npm view builderforce dist-tags` 和 `npx -y builderforce@X.Y.Z --version`（或 `--help`）。
...
- [ ] 从干净的临时目录（无 `package.json`），运行 `npx -y builderforce@X.Y.Z send --help` 确认安装/CLI 入口点正常工作。
- [ ] 宣布/分享发布说明。
