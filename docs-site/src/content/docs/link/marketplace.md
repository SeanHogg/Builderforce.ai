...
description: Publish, discover, and install reusable AI agent skills at tenant or agent level
---

# Skills Marketplace

The BuilderForce Agents.ai Skills Marketplace is the **public registry for reusable AI agent capabilities**. Publish skills your agents use, discover skills built by the community, and install them in one click — at the tenant or agent level.

---

## 1. What is a Skill?

A **skill** is a self-contained capability that a BuilderForce Agents agent can use. Skills extend what agents can do without modifying the core agent runtime.
...
Skills are described by a name, slug, description, category, tags, version, README, and an optional icon and repository URL. The actual skill implementation lives in the BuilderForce Agents project's `.builderforce/skills/` directory or in a linked GitHub repository.
...
Skills are installed via the **Skill Assignments** API, not the Marketplace API. The Marketplace API is for browsing and publishing; Skill Assignments link a marketplace skill to a tenant or agent.

```http
# Install at tenant level (all agents inherit)
...
POST /api/skill-assignments/agents/42
Authorization: Bearer <user-jwt>

{ "slug": "ts-linter" }
```

The BuilderForce Agents runtime discovers its assigned skills on the next sync and loads the skill implementation from `.builderforce/skills/` or the linked repository.

---

## 7. Skill Scoping: Tenant vs Agent

| Scope | API path | When to use |
|-------|----------|-------------|
| **Tenant** | `/api/skill-assignments/tenant` | Skill is useful for all agents (e.g. a linter, a PR reviewer) |
| **Agent** | `/api/skill-assignments/agents/:agentNodeId` | Skill is specific to one agent's role (e.g. a deployment skill on the CI agent only) |

Agent-level assignments override tenant-level for the same slug — a agent can have a different version of a skill than the tenant default.
...
| `POST` | `/api/skill-assignments/agents/:id` | User JWT (MANAGER+) | Install on agent |
| `DELETE` | `/api/skill-assignments/tenant/:slug` | User JWT (MANAGER+) | Remove tenant assignment |
| `DELETE` | `/api/skill-assignments/agents/:id/:slug` | User JWT (MANAGER+) | Remove agent assignment |
