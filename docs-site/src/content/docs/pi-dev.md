...
This guide summarizes a sane workflow for working on the pi integration in BuilderForce Agents.
...
- Trigger the agent directly:
  - `pnpm builderforce agent --message "Hello" --thinking low`
...
State lives under the BuilderForce Agents state directory. Default is `~/.builderforce`. If `BUILDERFORCE_AGENTS_STATE_DIR` is set, use that directory instead.

To reset everything:

- `builderforce.json` for config
