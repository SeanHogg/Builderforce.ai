---
summary: "Mattermost bot setup and BuilderForce Agents config"
read_when:
...
```bash
builderforce plugins install @builderforce/mattermost
```

Local checkout (when running from a git repo):

```bash
builderforce plugins install ./extensions/mattermost
```
...
4. Configure BuilderForce Agents and start the gateway.
...
- Approve via:
  - `builderforce pairing list mattermost`
  - `builderforce pairing approve mattermost <CODE>`
...
Use these target formats with `builderforce message send` or cron/webhooks:
