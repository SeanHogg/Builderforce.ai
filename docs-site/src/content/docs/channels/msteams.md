...
```bash
builderforce plugins install @builderforce/msteams
```

Local checkout (when running from a git repo):

```bash
builderforce plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
BuilderForce Agents will offer the local install path automatically.
...
3. Configure BuilderForce Agents with those credentials.
...
- Talk to BuilderForce Agents via Teams DMs, group chats, or channels.
...
- On startup, BuilderForce Agents resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.builderforce/builderforce.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring BuilderForce Agents, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                     |
   | ------------------ | --------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `builderforce-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `builderforce plugins install @builderforce/msteams`
   - From a local checkout: `builderforce plugins install ./extensions/msteams`
...
4. **Configure BuilderForce Agents**
...
```json
{
  ...
  "name": { "short": "BuilderForce Agents" },
  "developer": {
    "name": "Your Org",
  ...
  "description": { "short": "BuilderForce Agents in Teams", "full": "BuilderForce Agents in Teams" },
...
BuilderForce Agents handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, BuilderForce Agents only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/BuilderForce AgentsShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

BuilderForce Agents sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `builderforce message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.builderforce/msteams-polls.json`.
...
builderforce message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
builderforce message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
builderforce message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
builderforce message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
