...
Matrix is an open, decentralized messaging protocol. BuilderForce Agents connects as a Matrix **user**
...
```bash
builderforce plugins install @builderforce/matrix
```

Local checkout (when running from a git repo):

```bash
builderforce plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `builderforce plugins install @builderforce/matrix`
   - From a local checkout: `builderforce plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: BuilderForce Agents calls the same
     login endpoint, stores the access token in `~/.builderforce/credentials/matrix/credentials.json`,
...
- On first connection, BuilderForce Agents requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  BuilderForce Agents logs a warning.
...
`~/.builderforce/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `builderforce pairing list matrix`
  - `builderforce pairing approve matrix <CODE>`
...
- On startup, BuilderForce Agents resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
