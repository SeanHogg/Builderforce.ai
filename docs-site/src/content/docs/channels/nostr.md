...
read_when:
  - You want BuilderForce Agents to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables BuilderForce Agents to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`builderforce onboard`) and `builderforce channels add` list optional channel plugins.
...
```bash
builderforce plugins install @builderforce/nostr
```

Use a local checkout (dev workflows):

```bash
builderforce plugins install --link <path-to-builderforce>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "builderforce",
        "displayName": "BuilderForce Agents",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "builderforce@example.com",
        "lud16": "builderforce@example.com"
      }
    }
  }
}
