---
summary: "Chrome extension: let BuilderForce Agents drive your existing Chrome tab"
read_when:
...
The BuilderForce Agents Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate builderforce-managed Chrome profile.
...
BuilderForce Agents then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the BuilderForce Agents release (npm package) as static files. There is no separate “build” step.

After upgrading BuilderForce Agents:

- Re-run `builderforce browser extension install` to refresh the installed files under your BuilderForce Agents state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

BuilderForce Agents ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want BuilderForce Agents to control.
...
- `ON`: attached; BuilderForce Agents can drive that tab.
...
Debugging: `builderforce sandbox explain`
...
`builderforce browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `builderforce browser extension install` first to copy the extension to a stable location under your BuilderForce Agents state directory.
...
- **This is not isolated** like the dedicated builderforce-managed profile.
