---
summary: "Use Synthetic's Anthropic-compatible API in BuilderForce Agents"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. BuilderForce Agents registers it as the
...
Note: BuilderForce Agents's Anthropic client appends `/v1` to the base URL, so use
