---
title: "BuilderForce Agents Workflow Patterns"
---

# BuilderForce Agents Workflow Patterns

This document describes the built-in and custom workflow patterns available in BuilderForce Agents's multi-agent orchestration system.
...
Beyond the built-in code patterns below, Builderforce.ai ships a drag-and-drop
**Agentic Workflow Builder** (`/workflows/builder`) — an IPAAS-style canvas for
composing your own workflows from a node palette:
...
Graphs are saved as reusable **workflow definitions** and compiled into the same
orchestrator steps the built-in patterns produce, so a run shows up in the
existing monitoring + dependency-graph views with no extra setup. The LLM-logic
and ETL nodes execute in-process in the agent's orchestrator; agent nodes dispatch
to your configured runtimes (including `remote:` agents).
...
```bash
builderforce agent --message "Create user authentication feature" --thinking high
```
...
```bash
builderforce agent --message "Refactor the authentication module" --thinking high
```
...
```bash
builderforce agent --message "Review the latest changes for security issues" --thinking high
```
...
```bash
builderforce agent --message "Document the API endpoints" --thinking medium
```
...
```bash
# Get workflow status
builderforce agent --message "What's the status of workflow abc-123?" --thinking low
...
```bash
builderforce agent --message "Adversarially review the API authentication design" --thinking high
```
...
BuilderForce Agents stores session context in `.builderforce/sessions/` so any agent session can pick up exactly where the last one stopped — no need to replay history or re-explain the project.

**At the end of a session**, save a handoff:

```bash
builderforce agent --message "Save a session handoff for what we accomplished today" --thinking low
```

The agent writes a YAML file to `.builderforce/sessions/<session-id>.yaml` containing:
...
```bash
builderforce agent --message "Resume from the last session" --thinking low
```

BuilderForce Agents reads `.builderforce/sessions/` and surfaces the most recent handoff as starting context.
...
```yaml
# .builderforce/workflows/api-development.yaml
...
Define project-specific workflow patterns in `.builderforce/workflows/`:

```yaml
# .builderforce/workflows/pr-review.yaml
name: pr-review
...
```bash
builderforce agent --message "Run PR review workflow for the latest changes"
```

## Integration with CI/CD

BuilderForce Agents workflows can be integrated into CI/CD pipelines:

### GitHub Actions Example

```yaml
name: BuilderForce Agents Review
on: [pull_request]
  ...
  review:
    ...
    steps:
      ...
      - name: Run Review Workflow
        run: |
          builderforce init
          builderforce agent --message "Review PR changes" > review.txt
      - name: Comment on PR
...
BuilderForce Agents's workflow patterns enable:
