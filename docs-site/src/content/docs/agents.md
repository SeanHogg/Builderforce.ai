---
title: "BuilderForce Agents: Multi-Agent Developer System"
---

# BuilderForce Agents: Multi-Agent Developer System

BuilderForce Agents is a developer-first, multi-agent AI system designed for code creation, review, testing, debugging, refactoring, and deep codebase understanding. It operates as an **orchestration engine inside real developer workflows**.
...
## Why BuilderForce Agents?

Unlike closed‑source cloud assistants, BuilderForce Agents is self‑hosted and open‑source (MIT).
...
workflows. For a side‑by‑side comparison see [BuilderForce Agents vs Alternatives](agents-vs-alternatives.md).

## Core Capabilities

### 1. Deep Knowledge & Context Engine

BuilderForce Agents builds and maintains a persistent, structured project knowledge model:
...
All project knowledge is stored in `.builderforce/`:
...
Define project-specific agents in `.builderforce/agents/`:

```yaml
# .builderforce/agents/api-specialist.yaml
name: api-specialist
...
Add custom skills in `.builderforce/skills/`:
...
When you initialize a BuilderForce Agents project:
BuilderForce Agents stores project-specific context in a `.builderforce/` directory:
...
# Or specify a path
builderforce init /path/to/project
```

This creates the `.builderforce/` directory with default configuration.
...
## Using BuilderForce Agents

### From Command Line

```bash
# Analyze codebase structure
builderforce agent --message "Analyze the dependency graph" --thinking high

# Create feature with multi-agent workflow
builderforce agent --message "Create user authentication with tests and review" --thinking high

# Fix bug with systematic approach
builderforce agent --message "Debug the memory leak in the parser module" --thinking high

# Refactor code
builderforce agent --message "Refactor the API module for better modularity" --thinking high
```
...
```
@builderforce analyze the codebase structure

@builderforce create a feature for user authentication with tests

@builderforce review the latest changes for security issues

@builderforce refactor the authentication module
```

### Using BuilderForce Agents Tools

Agents automatically have access to BuilderForce Agents tools in initialized projects:
...
BuilderForce Agents's existing capabilities:

- **Skills**: Use coding-agent skill for interactive development
- **Subagents**: Orchestrate workflows spawn subagents automatically
- **Memory**: Project knowledge managed by BuilderForce Agents's memory system
- **Tools**: BuilderForce Agents tools available across all workflows
...
BuilderForce Agents's infrastructure includes:

### Components

- **Tool System**: Uses AgentTool interface for consistency
- **Subagent Spawning**: Leverages existing subagent lifecycle management
- **Session Management**: Integrates with BuilderForce Agents's session tracking
- **Configuration**: Extends BuilderForce Agents config structure
...
BuilderForce Agents's security model includes:
...
1. **Initialize projects early**: Run `builderforce init` when starting new projects
...
7. **Version control .BuilderForce Agents**: Check in context for team collaboration
8. **Document architectural decisions**: Keep `architecture.md` current

## Examples

See [examples/builderforce](https://github.com/SeanHogg/Builderforce.ai/tree/main/examples/builderforce) for comprehensive examples:

1. **Project Initialization**: Set up .BuilderForce Agents directory
...
We welcome contributions! See [CONTRIBUTING.md](https://github.com/SeanHogg/Builderforce.ai/blob/main/CONTRIBUTING.md) for guidelines.
...
MIT - See [LICENSE](https://github.com/SeanHogg/Builderforce.ai/blob/main/LICENSE) for details.
