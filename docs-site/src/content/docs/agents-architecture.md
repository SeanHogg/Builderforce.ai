---
title: "BuilderForce Agents Architecture"
---

# BuilderForce Agents Architecture

This document describes the architectural design and implementation of BuilderForce Agents, the multi-agent developer system.

## System Overview

BuilderForce Agents is designed as an **orchestration engine** that sits inside developer workflows, providing:
...
3. **Persistent project context** stored in `.builderforce/` directories
...
**AST Parser (`src/builderforce/ast-parser.ts`)**
...
**Code Map Builder (`src/builderforce/code-map.ts`)**
...
**Git History Tool (`src/builderforce/tools/git-history-tool.ts`)**
...
**Project Context (`src/builderforce/project-context.ts`)**

- Manages `.builderforce/` directory structure
...
Source Files → AST Parser → File Metadata
         ...
         Git History → Change Patterns
              ↓
      Project Context → Persistent Storage (.builderforce/)
```
...
**Agent Roles (`src/builderforce/agent-roles.ts`)**
...
- Loads YAML definitions from `.builderforce/personas/`
...
**Orchestrator (`src/builderforce/orchestrator.ts`)**
...
**Enhanced Orchestrator (`src/builderforce/orchestrator-enhanced.ts`)**
...
The outer layer connecting to BuilderForce Agents infrastructure.

#### Integration Layer Components

**Tool System Integration**

- Uses BuilderForce Agents's `AgentTool` interface
- Registers BuilderForce Agents tools: `code_analysis`, `project_knowledge`, `git_history`, `orchestrate`, `workflow_status`
- Follows BuilderForce Agents tool conventions

**Subagent Integration**

- Leverages BuilderForce Agents's subagent spawning
- Integrates with session management
- Respects tool policy and security boundaries

**Transport Layer**

- `AgentLinkTransportAdapter` — HTTP task lifecycle (`/api/runtime/executions`)
- `AgentLinkRelayService` (`src/infra/agentlink-relay.ts`) — persistent upstream WebSocket relay:
  - Connects to `AgentHostRelayDO` at `wss://api.builderforce.ai/api/agents/:id/upstream`
  - Bridges browser chat ↔ local gateway in real time
  - Sends HTTP heartbeat every 5 minutes to keep `lastSeenAt` current
  - Auto-reconnects with exponential backoff on disconnect
- `AgentLinkDirectorySync` (`src/infra/agentlink-directory-sync.ts`) — uploads `.builderforce/` files to the API on gateway boot
...
```
BuilderForce Agents Tools ←→ BuilderForce Agents Tool System
Orchestrator ←→ BuilderForce Agents Subagent Spawning
Task Engine ←→ Transport Layer
Security ←→ BuilderForce Agents Security Service
```

## Data Structures

### Project Context (.builderforce/context.yaml)
...
### Project Rules (.builderforce/rules.yaml)
...
### Custom Agent Role (.builderforce/personas/custom.yaml)
...
```
1. User runs `builderforce init`
2. CLI prompts for project information
3. Create .builderforce/ directory structure
...
2. Load project context from .builderforce/
...
- Workflow execution respects BuilderForce Agents's tool policy
...
Add new AST parsers in `src/builderforce/`:
...
Define YAML in `.builderforce/personas/`:
...
BuilderForce Agents's architecture is designed to be:
