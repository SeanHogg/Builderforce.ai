...
This document explains how BuilderForce Agents manages sessions end-to-end:
...
BuilderForce Agents is designed around a single **Gateway process** that owns session state.
...
BuilderForce Agents persists sessions in two layers:
...
- Transcripts: `~/.builderforce/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

BuilderForce Agents resolves these via `src/config/sessions.ts`.
...
BuilderForce Agents intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (BuilderForce Agents consumes the events, but Pi decides when to compact).
...
BuilderForce Agents also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, BuilderForce Agents bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, BuilderForce Agents leaves it alone.
...
- `builderforce status` (CLI)
- `builderforce sessions` / `sessions --json`
...
BuilderForce Agents supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- BuilderForce Agents strips/suppresses this in the delivery layer.

As of `2026.1.10`, BuilderForce Agents also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
BuilderForce Agents uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but BuilderForce Agents’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `builderforce status`.
