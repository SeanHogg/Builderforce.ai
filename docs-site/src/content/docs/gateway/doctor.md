...
`builderforce doctor` is the repair + migration tool for BuilderForce Agents. It fixes stale
...
```bash
builderforce doctor --repair --force
```
...
```bash
cat ~/.builderforce/builderforce.json
```
...
- Extra workspace dir detection (`~/builderforce`).
...
you to run `builderforce doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.builderforce/builderforce.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.builderforce/sessions/` to `~/.builderforce/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.builderforce/agent/` to `~/.builderforce/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.builderforce/credentials/*.json` (except `oauth.json`)
  - to `~/.builderforce/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `builderforce doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.builderforce` folders exist across
  home directories or when `BUILDERFORCE_AGENTS_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.builderforce/builderforce.json` is
...
offers to remove them and install the BuilderForce Agents service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named BuilderForce Agents gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `builderforce doctor --generate-gateway-token` to force token
...
- `builderforce doctor` prompts before rewriting supervisor config.
- `builderforce doctor --yes` accepts the default repair prompts.
- `builderforce doctor --repair` applies recommended fixes without prompts.
- `builderforce doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `builderforce gateway install --force`.
