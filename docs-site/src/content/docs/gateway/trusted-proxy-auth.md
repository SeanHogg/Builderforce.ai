...
read_when:
  - Running BuilderForce Agents behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of BuilderForce Agents
...
- You run BuilderForce Agents behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. BuilderForce Agents checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. BuilderForce Agents extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://builderforce.example.com
    to: http://builderforce-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
builderforce.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy builderforce:18789 {
...
location / {
    ...
    proxy_pass http://builderforce:18789;
...
`builderforce security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update BuilderForce Agents config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `builderforce security audit` and review findings
