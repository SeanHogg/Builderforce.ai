...
read_when:
  ...
  - Wiring external systems into BuilderForce Agents
title: "Webhooks"
---
...
```json5
{
  hooks: {
    enabled: true,
    token: "${BUILDERFORCE_AGENTS_HOOKS_TOKEN}",
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your BuilderForce Agents config directory (typically `~/.builderforce/hooks/transforms`).
...
- `builderforce webhooks gmail setup` writes `hooks.gmail` config for `builderforce webhooks gmail run`.
