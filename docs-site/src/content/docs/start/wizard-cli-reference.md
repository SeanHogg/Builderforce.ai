...
read_when:
  - You need detailed behavior for builderforce onboard
...
This page is the full reference for `builderforce onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.builderforce/builderforce.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `builderforce doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `builderforce pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `builderforce health`.
    - `builderforce status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.builderforce/.env` so launchd can read it.
...
- OAuth credentials: `~/.builderforce/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.builderforce/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.builderforce/credentials/oauth.json` (or `$BUILDERFORCE_AGENTS_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`builderforce onboard`](/cli/onboard)
