...
This branch's `PRD.md` previously contained a concatenation of this BurnRateOS PRD and main's **Evermind Knowledge & Learning Pipeline** PRD (task #487), because the repo's root `PRD.md` is overwritten per-ticket. During this merge-conflict recovery the shared `PRD.md` was restored to main's version (task #487) verbatim to preserve both intents, and this file (`PRD-248-BurnRateOS.md`) was introduced to keep task #248's spec intact without colliding. The correct long-term fix is to move per-ticket PRDs out of the repo root into `docs/prds/<ticket>/` (tracked by follow-up gap ticket #1371 and friends).
