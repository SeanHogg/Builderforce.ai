...
export async function handleWhatsAppStart(host: BuilderForceAgentsApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: BuilderForceAgentsApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: BuilderForceAgentsApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: BuilderForceAgentsApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: BuilderForceAgentsApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: BuilderForceAgentsApp): string {
...
function resolveGatewayHttpAuthHeader(host: BuilderForceAgentsApp): string | null {
...
function buildGatewayHttpHeaders(host: BuilderForceAgentsApp): Record<string, string> {
  const authorization = resolveGatewayHttpAuthHeader(host);
  return authorization ? { Authorization: authorization } : {};
}

export function handleNostrProfileEdit(
  host: BuilderForceAgentsApp,
  accountId: string,
...
export function handleNostrProfileCancel(host: BuilderForceAgentsApp) {
...
export function handleNostrProfileToggleAdvanced(host: BuilderForceAgentsApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: BuilderForceAgentsApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: BuilderForceAgentsApp) {
  const state = host.nostrProfileFormState;
