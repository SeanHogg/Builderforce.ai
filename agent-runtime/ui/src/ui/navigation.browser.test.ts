...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/builderforce/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/builderforce");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/builderforce/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__BUILDERFORCE_AGENTS_CONTROL_UI_BASE_PATH__ = "/builderforce";
    const app = mountApp("/builderforce/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/builderforce");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/builderforce/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "builderforce.control.settings.v1",
