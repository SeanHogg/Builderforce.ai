...
<title>BuilderForceAgents Canvas</title>
<style>
...
<div class="wrap">
  <div class="card">
    <div class="title">
      <h1>BuilderForceAgents Canvas</h1>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.builderforceCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.builderforceCanvasA2UIAction &&
        typeof window.builderforceCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.builderforceSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("builderforce:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android BuilderForceAgents node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.builderforceSendUserAction === "function"
        ? window.builderforceSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>BuilderForceAgents Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
