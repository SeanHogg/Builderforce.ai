...
describe("canvas host state dir defaults", () => {
  ...
  it("uses BUILDERFORCE_AGENTS_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("BuilderForceAgents Canvas");
    } finally {
      await handler.close();
