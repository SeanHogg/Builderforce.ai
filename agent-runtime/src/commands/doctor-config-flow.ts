...
function stripUnknownConfigKeys(config: BuilderForceAgentsConfig): {
  config: BuilderForceAgentsConfig;
  removed: string[];
} {
  const parsed = BuilderForceAgentsSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: BuilderForceAgentsConfig) {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: BuilderForceAgentsConfig): Promise<{
  config: BuilderForceAgentsConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: BuilderForceAgentsConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: BuilderForceAgentsConfig): {
  config: BuilderForceAgentsConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: BuilderForceAgentsConfig): {
  config: BuilderForceAgentsConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".builderforce");
  const targetPath = path.join(targetDir, "builderforce.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".builderforce", "builderforce.json"),
    path.join(home, ".builderforce", "builderforce.json"),
    path.join(home, ".builderforce", "builderforce.json"),
  ];
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: BuilderForceAgentsConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("builderforce doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("builderforce doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("builderforce doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("builderforce doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("builderforce doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "builderforce doctor --fix" to remove these keys.');
    }
  }
