...
export async function statusCommand(
  ...
  runtime.log(theme.heading("BuilderForceAgents status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("builderforce security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("builderforce security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.builderforce.ai/faq");
  runtime.log("Troubleshooting: https://docs.builderforce.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("builderforce status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("builderforce logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("builderforce status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("builderforce gateway probe")}`);
  }
}
