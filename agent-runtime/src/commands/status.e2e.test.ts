...
beforeAll(() => {
  envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_PROFILE"]);
  process.env.BUILDERFORCE_AGENTS_PROFILE = "isolated";
});
...
vi.mock("../infra/builderforce-root.js", () => ({
  resolveBuilderForceAgentsPackageRoot: vi.fn().mockResolvedValue("/tmp/builderforce"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/builderforce/pnpm-lock.yaml",
      markerPath: "/tmp/builderforce/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("BuilderForceAgents status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("builderforce status --all") ||
          l.includes("builderforce --profile isolated status --all") ||
          l.includes("builderforce status --all") ||
          l.includes("builderforce --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN = "abcd1234";
    try {
