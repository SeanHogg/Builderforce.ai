...
describe("normalizeLegacyConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.BUILDERFORCE_AGENTS_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-oauth-"));
    process.env.BUILDERFORCE_AGENTS_OAUTH_DIR = tempOauthDir;
  });

  afterEach(() => {
    if (previousOauthDir === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_OAUTH_DIR;
    } else {
      process.env.BUILDERFORCE_AGENTS_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-wa-auth-"));
    try {
