...
const SEARCH_TOOL = "https://docs.builderforce.ai/mcp.SearchBuilderForceAgents";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.builderforce.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('builderforce docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.builderforce.ai/");
      runtime.log(`Search: ${formatCliCommand('builderforce docs "your query"')}`);
    }
    return;
  }
