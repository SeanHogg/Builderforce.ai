...
import {
  ...
  requireBuilderForceAgentsAgentDir,
  setupAuthTestEnv,
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    const env = await setupAuthTestEnv("builderforce-auth-");
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireBuilderForceAgentsAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
