...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from builderforce.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "BuilderForceAgents update wizard" : "BuilderForceAgents configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: BuilderForceAgentsConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("builderforce doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const newPassword =
      nextConfig.gateway?.auth?.password ?? process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD;
    const oldPassword =
      baseConfig.gateway?.auth?.password ?? process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
