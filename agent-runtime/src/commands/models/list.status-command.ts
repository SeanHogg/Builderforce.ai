...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveBuilderForceAgentsAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("builderforce models auth setup-token")}\` or \`${formatCliCommand("builderforce configure")}\`.`
          : `Run \`${formatCliCommand("builderforce configure")}\` or set an API key env var.`;
