...
export async function loadValidConfigOrThrow(): Promise<BuilderForceAgentsConfig> {
...
export async function updateConfig(
  mutator: (cfg: BuilderForceAgentsConfig) => BuilderForceAgentsConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: BuilderForceAgentsConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: BuilderForceAgentsConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("builderforce agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
