...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveBuilderForceAgentsAgentDir: vi.fn().mockReturnValue("/tmp/builderforce-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/builderforce-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/builderforce-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveBuilderForceAgentsAgentDir: mocks.resolveBuilderForceAgentsAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/builderforce-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveBuilderForceAgentsAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/builderforce-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/builderforce-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/builderforce-agent-custom");
