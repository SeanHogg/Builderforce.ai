...
import type { BuilderForceAgentsConfig, GatewayBindMode } from "../config/config.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { isLoopbackHost, resolveGatewayBindHost } from "../gateway/net.js";
import { resolveDmAllowState } from "../security/dm-policy-shared.js";
import { note } from "../terminal/note.js";

export async function noteSecurityWarnings(cfg: BuilderForceAgentsConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("builderforce security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("builderforce configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("builderforce config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("builderforce doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("builderforce config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('builderforce config set session.dmScope "per-channel-peer"') +
