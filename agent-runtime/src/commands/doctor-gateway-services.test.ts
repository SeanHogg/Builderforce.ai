...
function setupGatewayTokenRepairScenario(expectedToken: string) {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      BUILDERFORCE_AGENTS_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service BUILDERFORCE_AGENTS_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      BUILDERFORCE_AGENTS_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses BUILDERFORCE_AGENTS_GATEWAY_TOKEN when config token is missing", async () => {
    const previousToken = process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousToken === undefined) {
        ...
        process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN = previousToken;
      }
    }
  });
});
