...
import type { BuilderForceAgentsConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: BuilderForceAgentsConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: BuilderForceAgentsConfig) {
