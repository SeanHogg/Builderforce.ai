...
vi.mock("../../plugins/loader.js", () => ({
  loadBuilderForceAgentsPlugins: vi.fn(),
}));
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const cfg: BuilderForceAgentsConfig = { update: { channel } };
...
describe("ensureOnboardingPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: BuilderForceAgentsConfig = { plugins: { allow: ["other"] } };
    ...
    expect(result.cfg.plugins?.installs?.zalo?.spec).toBe("@builderforce/zalo");
    expect(result.cfg.plugins?.installs?.zalo?.installPath).toBe("/tmp/zalo");
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({ spec: "@builderforce/zalo" }),
    );
  });
