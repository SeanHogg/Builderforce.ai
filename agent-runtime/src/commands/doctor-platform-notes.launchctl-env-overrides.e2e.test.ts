...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "BUILDERFORCE_AGENTS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("BUILDERFORCE_AGENTS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv BUILDERFORCE_AGENTS_GATEWAY_TOKEN");
    expect(message).not.toContain("BUILDERFORCE_AGENTS_GATEWAY_PASSWORD");
  });
