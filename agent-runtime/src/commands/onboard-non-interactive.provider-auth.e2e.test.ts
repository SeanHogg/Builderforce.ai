...
async function withOnboardEnv(
  ...
  const prev = captureEnv([
    "HOME",
    "BUILDERFORCE_AGENTS_STATE_DIR",
    "BUILDERFORCE_AGENTS_CONFIG_PATH",
    "BUILDERFORCE_AGENTS_SKIP_CHANNELS",
    "BUILDERFORCE_AGENTS_SKIP_GMAIL_WATCHER",
    "BUILDERFORCE_AGENTS_SKIP_CRON",
    "BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST",
    "BUILDERFORCE_AGENTS_GATEWAY_TOKEN",
    "BUILDERFORCE_AGENTS_GATEWAY_PASSWORD",
    "CUSTOM_API_KEY",
    "BUILDERFORCE_AGENTS_DISABLE_CONFIG_CACHE",
  ]);
  ...
  process.env.BUILDERFORCE_AGENTS_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "builderforce.json");
  process.env.HOME = tempHome;
  process.env.BUILDERFORCE_AGENTS_STATE_DIR = tempHome;
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("builderforce-onboard-minimax-", async (env) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("builderforce-onboard-minimax-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("builderforce-onboard-zai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("builderforce-onboard-zai-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("builderforce-onboard-xai-", async (env) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("builderforce-onboard-ai-gateway-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("builderforce-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("builderforce-onboard-openai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("rejects vLLM auth choice in non-interactive mode", async () => {
    await withOnboardEnv("builderforce-onboard-vllm-non-interactive-", async ({ runtime }) => {
      await expect(
  ...
  it("rejects local auth choice in non-interactive mode", async () => {
    await withOnboardEnv("builderforce-onboard-local-non-interactive-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          authChoice: "local",
          skipSkills: true,
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("builderforce-onboard-litellm-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      ...
      prefix: "builderforce-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "builderforce-onboard-cf-gateway-infer-",
      options: {},
    },
  ])(
    "$name",
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("builderforce-onboard-together-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("builderforce-onboard-qianfan-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("builderforce-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractiveOnboardingWithDefaults(runtime, {
        authChoice: "custom-api-key",
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "builderforce-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "builderforce-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "builderforce-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveOnboardingWithDefaults(runtime, {
            authChoice: "custom-api-key",
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("builderforce-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "builderforce-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveOnboardingWithDefaults(runtime, {
            customApiKey: "custom-test-key",
