...
function resolveMode(cfg: BuilderForceAgentsConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("BuilderForceAgents doctor");

  const root = await resolveBuilderForceAgentsPackageRoot({
  ...
  let cfg: BuilderForceAgentsConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("builderforce configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("builderforce config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("builderforce setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("builderforce doctor --fix")}" to apply changes.`);
  }
