...
export function requireBuilderForceAgentsAgentDir(): string {
  const agentDir = process.env.BUILDERFORCE_AGENTS_AGENT_DIR;
  if (!agentDir) {
    throw new Error("BUILDERFORCE_AGENTS_AGENT_DIR not set");
  }
  return agentDir;
}
