...
export const resolveBuilderForceAgentsPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadBuilderForceAgentsPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/builderforce.json",
    createConfigIO,
...
vi.mock("../infra/builderforce-root.js", () => ({
  resolveBuilderForceAgentsPackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveBuilderForceAgentsPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.BUILDERFORCE_AGENTS_STATE_DIR;
  originalUpdateInProgress = process.env.BUILDERFORCE_AGENTS_UPDATE_IN_PROGRESS;
  process.env.BUILDERFORCE_AGENTS_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.BUILDERFORCE_AGENTS_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.BUILDERFORCE_AGENTS_UPDATE_IN_PROGRESS;
  } else {
    process.env.BUILDERFORCE_AGENTS_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
