...
function resolveAgentName(cfg: BuilderForceAgentsConfig, agentId: string) {
...
function resolveAgentModel(cfg: BuilderForceAgentsConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: BuilderForceAgentsConfig): AgentSummary[] {
