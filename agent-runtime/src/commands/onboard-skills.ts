...
export async function setupSkills(
  ...
  let next: BuilderForceAgentsConfig = cfg;
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("builderforce doctor")}\` to review skills + requirements.`,
      );
      runtime.log("Docs: https://docs.builderforce.ai/skills");
    }
  }
