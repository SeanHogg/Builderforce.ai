...
export async function applyNonInteractiveAuthChoice(params: {
...
}): Promise<BuilderForceAgentsConfig | null> {
  ...
  if (authChoice === "claude-cli" || authChoice === "codex-cli") {
    runtime.error(
      [
        ...
        'Use "--auth-choice builderforcellm" (recommended default), "--auth-choice token" (Anthropic setup-token), or "--auth-choice openai-codex".',
      ].join("\n"),
    );
    runtime.exit(1);
    return null;
  ...
  if (authChoice === "builderforcellm") {
    runtime.log("Using default BuilderForceAgentsLLM provider (builderforcellm/auto).");
    return applyAgentDefaultModelPrimary(nextConfig, "builderforcellm/auto");
  }
  ...
  const applyMoonshotApiKeyChoice = async (
    applyConfig: (cfg: BuilderForceAgentsConfig) => BuilderForceAgentsConfig,
  ): Promise<BuilderForceAgentsConfig | null> => {
    const resolved = await resolveNonInteractiveApiKey({
      provider: "moonshot",
      cfg: baseConfig,
