...
import type { BuilderForceAgentsConfig, ConfigValidationIssue } from "./types.js";
import { BuilderForceAgentsSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: BuilderForceAgentsConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: BuilderForceAgentsConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = BuilderForceAgentsSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as BuilderForceAgentsConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as BuilderForceAgentsConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as BuilderForceAgentsConfig,
  };
}
