...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    const shared = path.join(tmpdir(), "builderforce-shared-agentdir");
        ...
        JSON.stringify(
          {
            agents: {
              list: [
                { id: "a", agentDir: "~/.builderforce/agents/shared/agent" },
                { id: "b", agentDir: "~/.builderforce/agents/shared/agent" },
              ],
            },
