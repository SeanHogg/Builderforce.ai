...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = BuilderForceAgentsSchema.safeParse({
      $schema: "https://builderforce.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://builderforce.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = BuilderForceAgentsSchema.safeParse({ $schema: 123 });
...
describe("talk.voiceAliases", () => {
  it("accepts a string map of voice aliases", () => {
    const res = validateConfigObject({
      talk: {
        voiceAliases: {
          BuilderForceAgents: "EXAVITQu4vr4xnSDxMaL",
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = BuilderForceAgentsSchema.safeParse({
      cron: {
        enabled: true,
