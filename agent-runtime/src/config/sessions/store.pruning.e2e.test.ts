...
// Keep integration tests deterministic: never read a real builderforce.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.BUILDERFORCE_AGENTS_SESSION_CACHE_TTL_MS;
    process.env.BUILDERFORCE_AGENTS_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_SESSION_CACHE_TTL_MS;
    } else {
      process.env.BUILDERFORCE_AGENTS_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
