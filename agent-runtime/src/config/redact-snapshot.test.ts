...
import type { ConfigFileSnapshot } from "./types.builderforce.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.builderforce/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/builderforce/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/builderforce/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/builderforce/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/builderforce/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = BuilderForceAgentsSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "BuilderForceAgentsConfig";
    const hints = mapSensitivePaths(BuilderForceAgentsSchema, "", {});
