...
import type { BuilderForceAgentsConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareBuilderForceAgentsVersions } from "./version.js";
...
function coerceConfig(value: unknown): BuilderForceAgentsConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as BuilderForceAgentsConfig;
}
...
function stampConfigVersion(cfg: BuilderForceAgentsConfig): BuilderForceAgentsConfig {
...
function warnIfConfigFromFuture(cfg: BuilderForceAgentsConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareBuilderForceAgentsVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer BuilderForceAgents (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as BuilderForceAgentsConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): BuilderForceAgentsConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as BuilderForceAgentsConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: BuilderForceAgentsConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as BuilderForceAgentsConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.BUILDERFORCE_AGENTS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.BUILDERFORCE_AGENTS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.BUILDERFORCE_AGENTS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.BUILDERFORCE_AGENTS_WATCH_SESSION === "string" &&
        deps.env.BUILDERFORCE_AGENTS_WATCH_SESSION.trim().length > 0
          ? deps.env.BUILDERFORCE_AGENTS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.BUILDERFORCE_AGENTS_WATCH_COMMAND === "string" &&
        deps.env.BUILDERFORCE_AGENTS_WATCH_COMMAND.trim().length > 0
          ? deps.env.BUILDERFORCE_AGENTS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `BUILDERFORCE_AGENTS_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.BUILDERFORCE_AGENTS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.BUILDERFORCE_AGENTS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): BuilderForceAgentsConfig {
