...
describe("config env vars", () => {
  ...
  it("applies env vars from env.vars when missing", async () => {
    await withEnvOverride({ GROQ_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { GROQ_API_KEY: "gsk-config" } } } as BuilderForceAgentsConfig);
      expect(process.env.GROQ_API_KEY).toBe("gsk-config");
    });
  });

  it("loads ${VAR} substitutions from ~/.builderforce/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.BUILDERFORCE_AGENTS_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected BUILDERFORCE_AGENTS_STATE_DIR to be set by withTempHome");
        }
        await fs.mkdir(stateDir, { recursive: true });
        await fs.writeFile(path.join(stateDir, ".env"), "BRAVE_API_KEY=from-dotenv\n", "utf-8");

        const config: BuilderForceAgentsConfig = {
          tools: {
            web: {
              search: {
        ...
        const first = resolveConfigEnvVars(config, process.env) as BuilderForceAgentsConfig;
        expect(first.tools?.web?.search?.apiKey).toBe("from-dotenv");

        delete process.env.BRAVE_API_KEY;
        loadDotEnv({ quiet: true });
        const second = resolveConfigEnvVars(config, process.env) as BuilderForceAgentsConfig;
