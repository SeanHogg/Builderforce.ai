...
declare const __BUILDERFORCE_AGENTS_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __BUILDERFORCE_AGENTS_VERSION__ === "string" && __BUILDERFORCE_AGENTS_VERSION__) ||
  process.env.BUILDERFORCE_AGENTS_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `builderforce --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT ??
    process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.BUILDERFORCE_AGENTS_GATEWAY_BIND ??
    process.env.BUILDERFORCE_AGENTS_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (BUILDERFORCE_AGENTS_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
...
void main().catch((err) => {
  console.error(
    "[builderforce] Gateway daemon failed:",
