...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { BUILDERFORCE_AGENTS_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { BUILDERFORCE_AGENTS_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports BUILDERFORCE_AGENTS_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { BUILDERFORCE_AGENTS_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { BUILDERFORCE_AGENTS_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { BUILDERFORCE_AGENTS_SMOKE: "nope" })).toBe("qr");
  });
});
