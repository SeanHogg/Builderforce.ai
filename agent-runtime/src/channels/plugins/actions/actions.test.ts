...
function telegramCfg(): BuilderForceAgentsConfig {
  return { channels: { telegram: { botToken: "tok" } } } as BuilderForceAgentsConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as BuilderForceAgentsConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as BuilderForceAgentsConfig;
