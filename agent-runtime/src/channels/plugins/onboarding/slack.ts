...
function setSlackDmPolicy(cfg: BuilderForceAgentsConfig, dmPolicy: DmPolicy) {
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "BuilderForceAgents";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for BuilderForceAgents`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          ...
          description: "Send a message to BuilderForceAgents",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: BuilderForceAgentsConfig, allowFrom: string[]): BuilderForceAgentsConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
    ...
    const slackBotName = String(
      await prompter.text({
        ...
        initialValue: "BuilderForceAgents",
      }),
    ).trim();
