...
function setWhatsAppDmPolicy(cfg: BuilderForceAgentsConfig, dmPolicy: DmPolicy): BuilderForceAgentsConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: BuilderForceAgentsConfig, allowFrom?: string[]): BuilderForceAgentsConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: BuilderForceAgentsConfig, selfChatMode: boolean): BuilderForceAgentsConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: BuilderForceAgentsConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so BuilderForceAgents can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppAllowFrom(
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for BuilderForceAgents" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("builderforce channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
