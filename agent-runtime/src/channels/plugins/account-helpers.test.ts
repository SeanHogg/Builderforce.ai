...
function cfg(accounts?: Record<string, unknown> | null): BuilderForceAgentsConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as BuilderForceAgentsConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as BuilderForceAgentsConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as BuilderForceAgentsConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as BuilderForceAgentsConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as BuilderForceAgentsConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as BuilderForceAgentsConfig)).toBe("default");
    });
  });
});
