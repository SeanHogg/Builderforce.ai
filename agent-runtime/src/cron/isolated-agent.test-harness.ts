...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "builderforce-cron-" });
}
...
export async function writeSessionStore(
  ...
  const dir = path.join(home, ".builderforce", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<BuilderForceAgentsConfig> = {},
): BuilderForceAgentsConfig {
  const base: BuilderForceAgentsConfig = {
    agents: {
      defaults: {
