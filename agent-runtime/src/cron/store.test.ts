...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses BUILDERFORCE_AGENTS_HOME for tilde expansion", () => {
    vi.stubEnv("BUILDERFORCE_AGENTS_HOME", "/srv/builderforce-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/builderforce-home", "cron", "jobs.json"));
  });
});
