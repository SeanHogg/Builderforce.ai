import { isToolAllowedByPolicies } from "../agents/coding-tools.policy.js";
import {
...
function summarizeGroupPolicy(cfg: BuilderForceAgentsConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: BuilderForceAgentsConfig): boolean {
...
function collectModels(cfg: BuilderForceAgentsConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: BuilderForceAgentsConfig): Set<string> {
  const baseCfg: BuilderForceAgentsConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: BuilderForceAgentsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: BuilderForceAgentsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: BuilderForceAgentsConfig): boolean {
...
function isBrowserEnabled(cfg: BuilderForceAgentsConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: BuilderForceAgentsConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: BuilderForceAgentsConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep BUILDERFORCE_AGENTS_STATE_DIR on a local-only volume and re-run "${formatCliCommand("builderforce security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: BuilderForceAgentsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer BUILDERFORCE_AGENTS_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const builderforceGatewayToken =
    typeof env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN === "string" && env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN.trim()
      ? env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-builderforce-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: BuilderForceAgentsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: BuilderForceAgentsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: BuilderForceAgentsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
