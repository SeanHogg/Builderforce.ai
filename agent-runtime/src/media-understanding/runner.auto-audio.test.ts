...
async function withAudioFixture(
  ...
  const tmpPath = path.join(os.tmpdir(), `builderforce-auto-audio-${Date.now()}.wav`);
...
function createOpenAiAudioCfg(extra?: Partial<BuilderForceAgentsConfig>): BuilderForceAgentsConfig {
  return {
    models: {
      providers: {
        openai: {
