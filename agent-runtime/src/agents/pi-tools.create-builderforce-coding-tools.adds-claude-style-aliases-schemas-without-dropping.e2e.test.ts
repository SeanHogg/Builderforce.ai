...
describe("createBuilderForceAgentsCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createBuilderForceAgentsTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createBuilderForceAgentsCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "builderforce-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createBuilderForceAgentsReadTool(
      baseRead as unknown as Parameters<typeof createBuilderForceAgentsReadTool>[0],
    );
