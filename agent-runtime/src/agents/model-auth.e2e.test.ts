...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const envSnapshot = captureEnv([
      ...
      "BUILDERFORCE_AGENTS_AGENT_DIR",
      "PI_CODING_AGENT_DIR",
    ]);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-oauth-"));

    try {
      process.env.BUILDERFORCE_AGENTS_STATE_DIR = tempDir;
      process.env.BUILDERFORCE_AGENTS_AGENT_DIR = path.join(tempDir, "agent");
      process.env.PI_CODING_AGENT_DIR = process.env.BUILDERFORCE_AGENTS_AGENT_DIR;
  ...
  it("resolves builderforcellm API key from shared env registration", async () => {
    const envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_STATE_DIR", "BUILDERFORCE_API_KEY"]);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-auth-"));
  ...
  it("provides onboarding guidance when builderforcellm is not registered", async () => {
    const envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_STATE_DIR", "BUILDERFORCE_API_KEY"]);
    ...
    try {
      ...
      expect(String(error)).toContain(
        'No Builderforce registration found for provider "builderforcellm".',
      );
      expect(String(error)).toContain("builderforce onboard");
    } finally {
      envSnapshot.restore();
      await fs.rm(tempDir, { recursive: true, force: true });
    }
