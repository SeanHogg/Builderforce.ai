...
function buildMemorySection(params: {
  ...
  const lines = [
    "## Memory Recall",
    "Before answering anything about prior work, decisions, dates, people, preferences, or todos: run memory_search on .builderforce/MEMORY.md + .builderforce/memory/*.md; then use memory_get to pull only the needed lines. If low confidence after search, say you checked.",
  ];
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; BuilderForceAgents handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `BuilderForceAgents docs: ${docsPath}`,
    "Mirror: https://docs.builderforce.ai",
    "Source: https://github.com/SeanHogg/Builderforce.ai",
    "Community: https://discord.gg/builderforce",
    "Find new skills: https://builderforce.ai/marketplace",
    "For BuilderForceAgents behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `builderforce status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running BuilderForceAgents process",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside BuilderForceAgents.";
  }

  const lines = [
    "You are a personal assistant running inside BuilderForceAgents.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control BuilderForceAgents's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "BuilderForceAgents is controlled via subcommands. Do not invent commands.",
    ...
    hasGateway && !isMinimal ? "## BuilderForceAgents Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          "Get Updates (self-update) is ONLY allowed when the user explicitly asks for it.",
          "Do not run config.apply or update.run unless the user explicitly requests an update or config change; if it's not explicit, ask first.",
          "Actions: config.get, config.schema, config.apply (validate + write full config, then restart), update.run (update deps or git, then restart).",
          "After restart, BuilderForceAgents pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by BuilderForceAgents and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'BuilderForceAgents treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
