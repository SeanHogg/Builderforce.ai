...
const GOOGLE_LIVE_SINGLE_KEY = "BUILDERFORCE_AGENTS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "BUILDERFORCE_AGENTS_LIVE_ANTHROPIC_KEY",
    listVar: "BUILDERFORCE_AGENTS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "BUILDERFORCE_AGENTS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `BUILDERFORCE_AGENTS_LIVE_${base}_KEY`;
