...
function resolvePayloadLogConfig(env: NodeJS.ProcessEnv): PayloadLogConfig {
  const enabled = parseBooleanValue(env.BUILDERFORCE_AGENTS_ANTHROPIC_PAYLOAD_LOG) ?? false;
  const fileOverride = env.BUILDERFORCE_AGENTS_ANTHROPIC_PAYLOAD_LOG_FILE?.trim();
