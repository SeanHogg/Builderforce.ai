...
function resolveSkillsLimits(config?: BuilderForceAgentsConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "builderforce-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "builderforce-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveBuilderForceAgentsMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: BuilderForceAgentsConfig }): {
...
export function buildWorkspaceSkillSnapshot(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`builderforce skills check\` to audit.`
    : "";
