...
describe("resolveBundledSkillsDir", () => {
  const originalOverride = process.env.BUILDERFORCE_AGENTS_BUNDLED_SKILLS_DIR;

  afterEach(() => {
    if (originalOverride === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_BUNDLED_SKILLS_DIR;
    } else {
      process.env.BUILDERFORCE_AGENTS_BUNDLED_SKILLS_DIR = originalOverride;
    }
  });
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "builderforce" }));
