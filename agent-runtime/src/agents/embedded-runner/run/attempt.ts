...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveBuilderForceAgentsAgentDir();
    ...
    const toolsRaw = params.disableTools
      ...
      : createBuilderForceAgentsCodingTools({
          ...
          modelAuthMode: await resolveModelAuthMode(params.model.provider, params.config),
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveBuilderForceAgentsDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      const localBrainEnv = process.env.BUILDERFORCE_AGENTS_LOCAL_BRAIN?.trim();
      ...
      if (localBrainEnabled) {
        ...
        const providerConfig = params.config?.models?.providers?.["builderforcellm-local"];
        const cacheDir =
        ...
        activeSession.agent.streamFn = createBuilderForceAgentsLlmLocalStreamFn({
          config: params.config,
      ...
      try {
        ...
        // double-compaction. See: https://github.com/SeanHogg/Builderforce.ai/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt") {
          try {
            sessionManager.appendCustomEntry("builderforce:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/SeanHogg/Builderforce.ai/issues/8643
