...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("builderforce-ws-", async (workspaceDir) => {
      await withTempDir("builderforce-cwd-", async (otherDir) => {
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("builderforce-ws-", async (workspaceDir) => {
      await withTempDir("builderforce-override-", async (overrideDir) => {
        const tools = createBuilderForceAgentsCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("builderforce-sandbox-", async (sandboxDir) => {
      await withTempDir("builderforce-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          containerName: "builderforce-sbx-test",
        ...
        const tools = createBuilderForceAgentsCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
