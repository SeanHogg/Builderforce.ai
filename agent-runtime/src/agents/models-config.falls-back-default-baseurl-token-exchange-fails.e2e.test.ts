...
describe("models-config", () => {
  it("falls back to default baseUrl when token exchange fails", async () => {
    await withTempHome(async () => {
      ...
      try {
        await ensureBuilderForceAgentsModelsJson({ models: { providers: {} } });

        const agentDir = path.join(process.env.HOME ?? "", ".builderforce", "agents", "main", "agent");
  ...
  it("uses agentDir override auth profiles for copilot injection", async () => {
    await withTempHome(async (home) => {
      await withUnsetCopilotTokenEnv(async () => {
        ...
        await ensureBuilderForceAgentsModelsJson({ models: { providers: {} } }, agentDir);
