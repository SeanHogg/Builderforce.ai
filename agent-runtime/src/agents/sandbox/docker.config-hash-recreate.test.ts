  ...
  return {
    ...
    spawn: (command: string, args: string[]) => {
      ...
      if (command !== "docker") {
        ...
        args[0] === "inspect" &&
        args[1] === "-f" &&
        args[2]?.includes('index .Config.Labels "builderforce.configHash"')
      ) {
...
function createSandboxConfig(dns: string[]): SandboxConfig {
  return {
    ...
    docker: {
      image: "builderforce-sandbox:test",
    ...
    browser: {
      enabled: false,
      image: "builderforce-browser:test",
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall?.args).toContain(`builderforce.configHash=${newHash}`);
