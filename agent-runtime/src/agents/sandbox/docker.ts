...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "builderforce.sandbox=1");
  args.push("--label", `builderforce.sessionKey=${params.scopeKey}`);
  args.push("--label", `builderforce.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `builderforce.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "builderforce.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`builderforce sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`builderforce sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("builderforce sandbox recreate --all");
}
