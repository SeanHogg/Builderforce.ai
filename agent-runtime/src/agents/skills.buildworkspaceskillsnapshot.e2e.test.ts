...
describe("buildWorkspaceSkillSnapshot", () => {
  ...
  it("limits discovery for nested repo-style skills roots (dir/skills/*)", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-"));
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-skills-repo-"));
  ...
  it("enforces maxSkillFileBytes for root-level SKILL.md", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-"));
    const rootSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-root-skill-"));
