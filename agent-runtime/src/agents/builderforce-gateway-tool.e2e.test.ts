...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_STATE_DIR", "BUILDERFORCE_AGENTS_PROFILE"]);
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-test-"));
    process.env.BUILDERFORCE_AGENTS_STATE_DIR = stateDir;
    process.env.BUILDERFORCE_AGENTS_PROFILE = "isolated";

    try {
      const tool = createBuilderForceAgentsTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: builderforce --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/builderforce" } }\n}\n';
