    ...
    if (hasModels && !normalizedProvider.apiKey?.trim()) {
      ...
      if (authMode === "aws-sdk") {
        ...
        if (apiKey?.trim()) {
          ...
          // Local inference providers (e.g. builderforcellm-local) don't need
...
// ---------------------------------------------------------------------------
// builderForceAgentsLLM — first-party free-model proxy (api.builderforce.ai/llm)
// ---------------------------------------------------------------------------

const BUILDERFORCE_AGENTSLLM_BASE_URL = "https://api.builderforce.ai/llm/v1";
const BUILDERFORCE_AGENTSLLM_FREE_COST = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 };

export function buildBuilderForceAgentsllmProvider(): ProviderConfig {
  return {
    baseUrl: BUILDERFORCE_AGENTSLLM_BASE_URL,
    api: "openai-completions",
    models: [
      {
        id: "auto",
...
export async function resolveImplicitProviders(params: {
  ...
  // builderForceAgentsLLM — first-party free-model proxy.
  // This provider is always available; the server-side OpenRouter key lives in Builderforce.
  providers.builderforcellm = {
    ...buildBuilderForceAgentsllmProvider(),
    apiKey: "builderforcellm-proxy",
  };

  // llama — local GGUF model via node-llama-cpp.
  // Opt-in: configure models.providers.llama.modelPath in .builderforce/config.yaml,
  // or set BUILDERFORCE_AGENTS_LLAMA_MODEL_PATH env var.
  if (!params.explicitProviders?.llama) {
    const llamaModelPath = (process.env.BUILDERFORCE_AGENTS_LLAMA_MODEL_PATH ?? "").trim();
    if (llamaModelPath) {
...
export async function resolveImplicitCopilotProvider(params: {
  ...
  // Our Copilot auth lives in BuilderForceAgents's auth-profiles store instead, so we also
  // write a runtime-only auth.json entry for pi-coding-agent to pick up.
  //
  // This is safe because it's (1) within BuilderForceAgents's agent dir, (2) contains the
