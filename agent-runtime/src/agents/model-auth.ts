...
export async function resolveApiKeyForProvider(params: {
  ...
  if (normalized === "builderforcellm") {
    ...
    throw new Error(
      `No Builderforce registration found for provider "builderforcellm". Run ${formatCliCommand("builderforce onboard")} (or /setup in TUI) to register your agentNode, then retry.`,
    );
  }
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("builderforce agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
...
export function resolveEnvApiKey(provider: string): EnvApiKeyResult | null {
  ...
  const envMap: Record<string, string> = {
    ...
    builderforcellm: "BUILDERFORCE_API_KEY",
...
export async function resolveModelAuthMode(
  ...
  if (
    normalizeProviderId(resolved) === "builderforcellm" &&
