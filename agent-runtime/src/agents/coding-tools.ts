...
import {
  ...
  CLAUDE_PARAM_GROUPS,
  createBuilderForceAgentsReadTool,
...
function resolveExecConfig(params: { cfg?: BuilderForceAgentsConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: BuilderForceAgentsConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createBuilderForceAgentsCodingTools(options?: {
  ...
  const base = (["read", "bash", "edit", "write"] as const).flatMap((toolName): AnyAgentTool[] => {
    if (toolName === "read") {
      ...
      const wrapped = createBuilderForceAgentsReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
        root: workspaceRoot,
      });
