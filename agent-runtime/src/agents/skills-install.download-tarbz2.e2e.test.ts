...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-skills-install-"));
  try {
...
function restoreBuilderForceAgentsStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.BUILDERFORCE_AGENTS_STATE_DIR;
    return;
  }
  process.env.BUILDERFORCE_AGENTS_STATE_DIR = originalValue;
}

const originalStateDir = process.env.BUILDERFORCE_AGENTS_STATE_DIR;

afterEach(() => {
  restoreBuilderForceAgentsStateDir(originalStateDir);
});
