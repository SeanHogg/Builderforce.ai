...
const originalBuilderForceAgentsStateDir = process.env.BUILDERFORCE_AGENTS_STATE_DIR;

afterEach(() => {
  if (originalBuilderForceAgentsStateDir === undefined) {
    delete process.env.BUILDERFORCE_AGENTS_STATE_DIR;
  } else {
    process.env.BUILDERFORCE_AGENTS_STATE_DIR = originalBuilderForceAgentsStateDir;
  }
});
