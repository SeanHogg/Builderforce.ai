...
function resolveDefaultModelRef(cfg?: BuilderForceAgentsConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: BuilderForceAgentsConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: BuilderForceAgentsConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureBuilderForceAgentsModelsJson(effectiveCfg, params.agentDir);
