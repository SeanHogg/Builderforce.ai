...
type CreateBuilderForceAgentsTools = (typeof import("./builderforce-tools.js"))["createBuilderForceAgentsTools"];
type CreateBuilderForceAgentsToolsOpts = Parameters<CreateBuilderForceAgentsTools>[0];

async function getSessionsSpawnTool(opts: CreateBuilderForceAgentsToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createBuilderForceAgentsTools } = await import("./builderforce-tools.js");
  const tool = createBuilderForceAgentsTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("builderforce-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
