...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses BUILDERFORCE_AGENTS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      BUILDERFORCE_AGENTS_HOME: "/srv/builderforce-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/builderforce-home"), ".builderforce", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".builderforce", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("builderforce-workspace-");
