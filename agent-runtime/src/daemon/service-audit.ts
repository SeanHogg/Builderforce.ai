...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.BUILDERFORCE_AGENTS_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service BUILDERFORCE_AGENTS_GATEWAY_TOKEN does not match gateway.auth.token in builderforce.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `builderforce gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
