...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.BUILDERFORCE_AGENTS_GATEWAY_PORT).toBe("18789");
    expect(env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN).toBe("secret");
    expect(env.BUILDERFORCE_AGENTS_SERVICE_MARKER).toBe("builderforce");
    expect(env.BUILDERFORCE_AGENTS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.BUILDERFORCE_AGENTS_SERVICE_VERSION).toBe("string");
    expect(env.BUILDERFORCE_AGENTS_SYSTEMD_UNIT).toBe("builderforce-gateway.service");
    if (process.platform === "darwin") {
      expect(env.BUILDERFORCE_AGENTS_LAUNCHD_LABEL).toBe("ai.builderforce.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", BUILDERFORCE_AGENTS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.BUILDERFORCE_AGENTS_SYSTEMD_UNIT).toBe("builderforce-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.BUILDERFORCE_AGENTS_LAUNCHD_LABEL).toBe("ai.builderforce.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".builderforce"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", BUILDERFORCE_AGENTS_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".builderforce-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", BUILDERFORCE_AGENTS_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".builderforce"));
  });

  it("uses BUILDERFORCE_AGENTS_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", BUILDERFORCE_AGENTS_STATE_DIR: "/var/lib/builderforce" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/builderforce"));
  });

  it("expands ~ in BUILDERFORCE_AGENTS_STATE_DIR", () => {
    const env = { HOME: "/Users/test", BUILDERFORCE_AGENTS_STATE_DIR: "~/builderforce-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/builderforce-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { BUILDERFORCE_AGENTS_STATE_DIR: "C:\\State\\builderforce" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\builderforce");
  });
});
