...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.builderforce.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", BUILDERFORCE_AGENTS_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    ...
    const label = "ai.builderforce.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when BUILDERFORCE_AGENTS_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.builderforce.gateway.plist",
    );
  });

  it("uses profile-specific label when BUILDERFORCE_AGENTS_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", BUILDERFORCE_AGENTS_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.builderforce.jbphoenix.plist",
    );
  });

  it("prefers BUILDERFORCE_AGENTS_LAUNCHD_LABEL over BUILDERFORCE_AGENTS_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      BUILDERFORCE_AGENTS_PROFILE: "jbphoenix",
      BUILDERFORCE_AGENTS_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from BUILDERFORCE_AGENTS_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      BUILDERFORCE_AGENTS_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty BUILDERFORCE_AGENTS_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      BUILDERFORCE_AGENTS_PROFILE: "myprofile",
      BUILDERFORCE_AGENTS_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.builderforce.myprofile.plist",
    );
  });
});
