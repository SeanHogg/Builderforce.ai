...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.BUILDERFORCE_AGENTS_PROFILE;
  ...
  const stateDir =
    env.BUILDERFORCE_AGENTS_STATE_DIR?.trim() || resolveConfigStateDir(env as NodeJS.ProcessEnv);
  const configPath =
    env.BUILDERFORCE_AGENTS_CONFIG_PATH?.trim() ||
    (stateDir ? path.join(stateDir, "builderforce.json") : undefined);
  ...
  return {
    ...
    BUILDERFORCE_AGENTS_PROFILE: profile,
    BUILDERFORCE_AGENTS_STATE_DIR: stateDir,
    BUILDERFORCE_AGENTS_CONFIG_PATH: configPath,
    BUILDERFORCE_AGENTS_GATEWAY_PORT: String(port),
    BUILDERFORCE_AGENTS_GATEWAY_TOKEN: token,
    BUILDERFORCE_AGENTS_LAUNCHD_LABEL: resolvedLaunchdLabel,
    BUILDERFORCE_AGENTS_SYSTEMD_UNIT: systemdUnit,
    BUILDERFORCE_AGENTS_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    BUILDERFORCE_AGENTS_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const { env } = params;
  const stateDir = env.BUILDERFORCE_AGENTS_STATE_DIR;
  const configPath = env.BUILDERFORCE_AGENTS_CONFIG_PATH;
  const tmpDir = env.TMPDIR?.trim() || os.tmpdir();
  return {
    HOME: env.HOME,
    TMPDIR: tmpDir,
    ...
    BUILDERFORCE_AGENTS_STATE_DIR: stateDir,
    BUILDERFORCE_AGENTS_CONFIG_PATH: configPath,
    BUILDERFORCE_AGENTS_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    BUILDERFORCE_AGENTS_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    BUILDERFORCE_AGENTS_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    BUILDERFORCE_AGENTS_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    BUILDERFORCE_AGENTS_LOG_PREFIX: "node",
    BUILDERFORCE_AGENTS_SERVICE_MARKER: NODE_SERVICE_MARKER,
    BUILDERFORCE_AGENTS_SERVICE_KIND: NODE_SERVICE_KIND,
    BUILDERFORCE_AGENTS_SERVICE_VERSION: VERSION,
  };
}
