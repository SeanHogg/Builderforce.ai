...
export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.BUILDERFORCE_AGENTS_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.BUILDERFORCE_AGENTS_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export function buildTaskLauncher(cmdScriptPath: string): string {
  ...
  return (
    [
      "# BuilderForceAgents Gateway Launcher — hidden window + crash-loop protection",
      `$cmdScript = '${escaped}'`,
