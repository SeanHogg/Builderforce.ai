...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("builderforce-triggers-", async (home) => {
      const inboundDir = join(home, ".builderforce", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "builderforce"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("builderforce-triggers-bypass-", async (home) => {
      // Sensitive host file outside .builderforce
