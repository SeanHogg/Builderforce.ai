...
function resolveForegroundMs(cfg: BuilderForceAgentsConfig): number {
...
export async function handleBashChatCommand(params: {
  ...
  if (params.cfg.commands?.bash !== true) {
    return {
      text: "⚠️ bash is disabled. Set commands.bash=true to enable. Docs: https://docs.builderforce.ai/tools/slash-commands#config",
    };
  }
