...
describe("matchesMentionWithExplicit", () => {
  const mentionRegexes = [/\bbuilderforce\b/i];
...
describe("resolveResponsePrefixTemplate", () => {
  ...
  it("resolves {identity.name} variable", () => {
    const result = resolveResponsePrefixTemplate("[{identity.name}]", {
      identityName: "BuilderForceAgents",
    });
    expect(result).toBe("[BuilderForceAgents]");
  });
  ...
  it("handles complex template with all variables", () => {
    ...
    expect(result).toBe("[BuilderForceAgents] anthropic/claude-opus-4-5 (think:high)");
  });
});
