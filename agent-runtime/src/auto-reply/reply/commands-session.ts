...
import { scheduleGatewaySigusr1Restart, triggerBuilderForceAgentsRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting BuilderForceAgents in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerBuilderForceAgentsRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting BuilderForceAgents via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
