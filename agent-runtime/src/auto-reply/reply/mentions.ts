...
function resolveMentionPatterns(cfg: BuilderForceAgentsConfig | undefined, agentId?: string): string[] {
  if (!cfg) {
    return [];
  }
...
export function buildMentionRegexes(cfg: BuilderForceAgentsConfig | undefined, agentId?: string): RegExp[] {
...
export function normalizeMentionText(text: string): string {
  const stripped = (text ?? "")
    .replace(/[\u200b-\u200f\u202a-\u202e\u2060-\u206f]/g, "")
    .toLowerCase();
  return stripped.replace(/\bopenagentNode\b/g, "builderforce");
}
