...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { BUILDERFORCE_AGENTS_STATE_DIR: process.env.BUILDERFORCE_AGENTS_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.BUILDERFORCE_AGENTS_STATE_DIR === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_STATE_DIR;
    } else {
      process.env.BUILDERFORCE_AGENTS_STATE_DIR = snapshot.BUILDERFORCE_AGENTS_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "builderforce-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "builderforce-memory-flush-"));
  });
  ...
  it("runs a memory flush turn and updates session metadata", async () => {
    await withTempStore(async (storePath) => {
      ...
      state.runEmbeddedAgentMock.mockImplementation(async (params: EmbeddedRunParams) => {
