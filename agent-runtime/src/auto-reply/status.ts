...
type AgentDefaults = NonNullable<NonNullable<BuilderForceAgentsConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.builderforce/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildHelpMessage(cfg?: BuilderForceAgentsConfig): string {
  const lines = ["ℹ️ Help", ""];
