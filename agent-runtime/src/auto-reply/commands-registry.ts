...
export function isCommandEnabled(cfg: BuilderForceAgentsConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: BuilderForceAgentsConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as BuilderForceAgentsConfig),
...
export function getCommandDetection(_cfg?: BuilderForceAgentsConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: BuilderForceAgentsConfig) {
