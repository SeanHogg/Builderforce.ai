...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    {
      env: {
        BUILDERFORCE_AGENTS_AGENT_DIR: (home) => path.join(home, ".builderforce", "agent"),
        PI_CODING_AGENT_DIR: (home) => path.join(home, ".builderforce", "agent"),
      },
      prefix: "builderforce-reply-",
    },
  );
}
