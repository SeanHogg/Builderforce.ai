...
import { getBuiltInAgentRoles } from "../builderforce/agent-roles.js";
import { globalPersonaRegistry, USER_PERSONAS_DIR } from "../builderforce/personas.js";
import {
...
ensureBuilderForceAgentsCliOnPath();
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.BUILDERFORCE_AGENTS_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "BUILDERFORCE_AGENTS_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "BUILDERFORCE_AGENTS_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("builderforce doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("builderforce doctor")}" to repair, then retry.`,
    );
  }
  ...
  // Ensure .builderforce/ project directory exists.
  ...
  if (!(await isBuilderForceAgentsProject(projectRoot))) {
    log.info("gateway: initialising .builderforce/ project directory");
  ...
  try {
    ...
    if (userCount > 0) {
      log.info(`Loaded ${userCount} user-global persona plugin(s) from ~/.builderforce/personas`);
    }

    const builderForceAgentsDir = resolveBuilderForceAgentsDir(projectRoot);
    const projectCount = await globalPersonaRegistry.loadFromDir(
      builderForceAgentsDir.personasDir,
      "project-local",
    );
    if (projectCount > 0) {
      log.info(`Loaded ${projectCount} project-local persona plugin(s) from .builderforce/personas`);
    }
