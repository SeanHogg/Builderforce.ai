...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.BUILDERFORCE_AGENTS_STATE_DIR;
  previousConfigPath = process.env.BUILDERFORCE_AGENTS_CONFIG_PATH;
  previousSkipBrowserControl = process.env.BUILDERFORCE_AGENTS_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.BUILDERFORCE_AGENTS_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.BUILDERFORCE_AGENTS_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.BUILDERFORCE_AGENTS_SKIP_CHANNELS;
  previousSkipProviders = process.env.BUILDERFORCE_AGENTS_SKIP_PROVIDERS;
  previousSkipCron = process.env.BUILDERFORCE_AGENTS_SKIP_CRON;
  previousMinimalGateway = process.env.BUILDERFORCE_AGENTS_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.BUILDERFORCE_AGENTS_STATE_DIR = path.join(tempHome, ".builderforce");
  delete process.env.BUILDERFORCE_AGENTS_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.BUILDERFORCE_AGENTS_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.BUILDERFORCE_AGENTS_SKIP_GMAIL_WATCHER = "1";
  process.env.BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST = "1";
  process.env.BUILDERFORCE_AGENTS_SKIP_CHANNELS = "1";
  process.env.BUILDERFORCE_AGENTS_SKIP_PROVIDERS = "1";
  process.env.BUILDERFORCE_AGENTS_SKIP_CRON = "1";
  process.env.BUILDERFORCE_AGENTS_TEST_MINIMAL_GATEWAY = "1";
  process.env.BUILDERFORCE_AGENTS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "builderforce-test-no-bundled-extensions")
    : "builderforce-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "builderforce-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".builderforce-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.BUILDERFORCE_AGENTS_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_CONFIG_PATH;
    } else {
      process.env.BUILDERFORCE_AGENTS_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.BUILDERFORCE_AGENTS_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_SKIP_GMAIL_WATCHER;
    } else {
      process.env.BUILDERFORCE_AGENTS_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST;
    } else {
      process.env.BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.BUILDERFORCE_AGENTS_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_SKIP_CHANNELS;
    } else {
      process.env.BUILDERFORCE_AGENTS_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_SKIP_PROVIDERS;
    } else {
      process.env.BUILDERFORCE_AGENTS_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_SKIP_CRON;
    } else {
      process.env.BUILDERFORCE_AGENTS_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.BUILDERFORCE_AGENTS_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_GATEWAY_TOKEN"]);
  const prev = process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultToken =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { token?: unknown } | undefined)?.token === "string"
        ...
        : process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ? undefined
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ? ((testState.gatewayAuth as { password?: string }).password ?? undefined)
        : process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD;
