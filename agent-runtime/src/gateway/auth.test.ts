...
describe("gateway auth", () => {
  it("ignores legacy BUILDERFORCE_AGENTS gateway env vars in favor of config", () => {
    expect(
      resolveGatewayAuth({
        authConfig: { token: "config-token", password: "config-password" },
        env: {
          BUILDERFORCE_AGENTS_GATEWAY_TOKEN: "env-token",
          BUILDERFORCE_AGENTS_GATEWAY_PASSWORD: "env-password",
  ...
  it("does not resolve legacy BUILDERFORCE_AGENTS gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          BUILDERFORCE_AGENTS_GATEWAY_TOKEN: "legacy-token",
          BUILDERFORCE_AGENTS_GATEWAY_PASSWORD: "legacy-password",
        } as NodeJS.ProcessEnv,
      }),
    ).toMatchObject({
      mode: "none",
