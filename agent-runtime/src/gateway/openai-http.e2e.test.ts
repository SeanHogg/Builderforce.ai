    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "builderforce", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-builderforce-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-builderforce-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "builderforce", messages: [{ role: "user", content: "hi" }] },
          {
            "x-builderforce-agent-id": "beta",
            "x-builderforce-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
