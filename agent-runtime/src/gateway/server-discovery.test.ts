...
describe("resolveTailnetDnsHint", () => {
  ...
  beforeEach(() => {
    prevTailnetDns.value = process.env.BUILDERFORCE_AGENTS_TAILNET_DNS;
    delete process.env.BUILDERFORCE_AGENTS_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.BUILDERFORCE_AGENTS_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.BUILDERFORCE_AGENTS_TAILNET_DNS = "studio.tailnet.ts.net.";
