...
type GetReplyFromConfigFn = (
  ...
  configOverride?: BuilderForceAgentsConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `builderforce-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.BUILDERFORCE_AGENTS_CONFIG_PATH = path.join(root, "builderforce.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "builderforce.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "builderforce-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadBuilderForceAgentsPlugins: () => pluginRegistryState.registry,
  };
});
