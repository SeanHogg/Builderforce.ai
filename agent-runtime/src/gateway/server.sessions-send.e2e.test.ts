...
const { createBuilderForceAgentsTools } = await import("../agents/builderforce-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_GATEWAY_PORT", "BUILDERFORCE_AGENTS_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT = String(gatewayPort);
  process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createBuilderForceAgentsTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("BUILDERFORCE_AGENTS_CONFIG_PATH missing in gateway test environment");
    }
