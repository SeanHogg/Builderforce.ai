...
export function buildGatewayConnectionDetails(
  options: { config?: BuilderForceAgentsConfig; url?: string; configPath?: string } = {},
...
export async function callGateway<T = Record<string, unknown>>(
    ...
    (!urlOverride
      ? isRemoteMode
        ? typeof remote?.token === "string" && remote.token.trim().length > 0
          ? remote.token.trim()
          : undefined
        : process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN?.trim() ||
          process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN?.trim() ||
  ...
  const password =
    ...
    (!urlOverride
      ? process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD?.trim() ||
        process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD?.trim() ||
        (isRemoteMode
