...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.BUILDERFORCE_AGENTS_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.BUILDERFORCE_AGENTS_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.BUILDERFORCE_AGENTS_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.BUILDERFORCE_AGENTS_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("connect (req) handshake prefers service version fallback in hello-ok payload", async () => {
      await withRuntimeVersionEnv(
        {
          BUILDERFORCE_AGENTS_VERSION: " ",
          BUILDERFORCE_AGENTS_SERVICE_VERSION: "2.4.6-service",
    ...
    test("connect (req) handshake prefers BUILDERFORCE_AGENTS_VERSION over service version", async () => {
      await withRuntimeVersionEnv(
        {
          BUILDERFORCE_AGENTS_VERSION: "9.9.9-cli",
          BUILDERFORCE_AGENTS_SERVICE_VERSION: "2.4.6-service",
          npm_package_version: "1.0.0-package",
        },
    ...
    test("connect (req) handshake falls back to npm_package_version when higher-precedence env values are blank", async () => {
      await withRuntimeVersionEnv(
        {
          BUILDERFORCE_AGENTS_VERSION: " ",
          BUILDERFORCE_AGENTS_SERVICE_VERSION: "\t",
          npm_package_version: "1.0.0-package",
        },
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `builderforce-test-device-${randomUUID()}.json`),
      });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
      process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { getPairedDevice } = await import("../infra/device-pairing.js");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "builderforce-device-scope-"));
