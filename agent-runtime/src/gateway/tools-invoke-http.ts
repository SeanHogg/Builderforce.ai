...
import { createBuilderForceAgentsTools } from "../agents/builderforce-tools.js";
import {
  resolveEffectiveToolPolicy,
  resolveGroupToolPolicy,
  resolveSubagentToolPolicy,
} from "../agents/coding-tools.policy.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-builderforce-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-builderforce-account-id")?.trim() || undefined;
  ...
  const allTools = createBuilderForceAgentsTools({
