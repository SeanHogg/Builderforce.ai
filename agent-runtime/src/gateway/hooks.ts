...
export function resolveHooksConfig(cfg: BuilderForceAgentsConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: BuilderForceAgentsConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-builderforce-token"] === "string"
      ? req.headers["x-builderforce-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
