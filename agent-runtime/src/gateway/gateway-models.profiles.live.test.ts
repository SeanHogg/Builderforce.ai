...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.BUILDERFORCE_AGENTS_LIVE_GATEWAY_PROVIDERS);
...
async function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<BuilderForceAgentsConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const toolProbePath = path.join(workspaceDir, `.builderforce-live-tool-probe.${nonceA}.txt`);
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-live-"));
  ...
  await ensureBuilderForceAgentsModelsJson(nextCfg);
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "BuilderForceAgents live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.BUILDERFORCE_AGENTS_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.builderforce-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
