...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) {
    return "BuilderForceAgents";
  }
  if (/builderforce/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (BuilderForceAgents)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.BUILDERFORCE_AGENTS_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "builderforce");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.BUILDERFORCE_AGENTS_TAILNET_DNS?.trim();
