...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "builderforce_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "builderforce_cmdarg", "arg-menu options");
  });
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("builderforce_cmdarg");
  ...
  it("serves filtered options for external_select menus", async () => {
    ...
    expect(blockId).toContain("builderforce_cmdarg_ext:");
