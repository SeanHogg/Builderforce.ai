...
const HOME = path.join(os.tmpdir(), "builderforce-home-redirect");
...
describe("media store redirects", () => {
  ...
  beforeAll(async () => {
    envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_STATE_DIR"]);
    await fs.rm(HOME, { recursive: true, force: true });
    process.env.BUILDERFORCE_AGENTS_STATE_DIR = HOME;
  });
