...
describe("media store", () => {
  ...
  beforeAll(async () => {
    ...
    home = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-test-home-"));
    ...
    await fs.mkdir(path.join(home, ".builderforce"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.builderforce${path.sep}media`);
