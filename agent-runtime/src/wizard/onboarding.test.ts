...
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.builderforce/builderforce.json",
    exists: false,
...
vi.mock("../commands/onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "/tmp/builderforce-workspace",
...
describe("runOnboardingWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-onboard-suite-"));
  });
  ...
  it("runs Builderforce registration wizard when builderforcellm is selected without key", async () => {
