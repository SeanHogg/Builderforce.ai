...
describe("canvas a2ui copy", () => {
  ...
  beforeEach(() => {
    savedSkipMissing = process.env.BUILDERFORCE_AGENTS_A2UI_SKIP_MISSING;
    delete process.env.BUILDERFORCE_AGENTS_A2UI_SKIP_MISSING;
  });

  afterEach(() => {
    if (savedSkipMissing !== undefined) {
      process.env.BUILDERFORCE_AGENTS_A2UI_SKIP_MISSING = savedSkipMissing;
    } else {
      delete process.env.BUILDERFORCE_AGENTS_A2UI_SKIP_MISSING;
    }
  });
  ...
  it("throws a helpful error when assets are missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-a2ui-"));
