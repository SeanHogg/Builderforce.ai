...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "builderforce-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.builderforce when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "builderforce-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers BUILDERFORCE_AGENTS_HOME over HOME", () => {
    vi.stubEnv("BUILDERFORCE_AGENTS_HOME", "/srv/builderforce-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/builderforce-home"));
...
describe("shortenHomePath", () => {
  it("uses $BUILDERFORCE_AGENTS_HOME prefix when BUILDERFORCE_AGENTS_HOME is set", () => {
    vi.stubEnv("BUILDERFORCE_AGENTS_HOME", "/srv/builderforce-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomePath(`${path.resolve("/srv/builderforce-home")}/.builderforce/builderforce.json`),
    ).toBe("$BUILDERFORCE_AGENTS_HOME/.builderforce/builderforce.json");
...
describe("shortenHomeInString", () => {
  it("uses $BUILDERFORCE_AGENTS_HOME replacement when BUILDERFORCE_AGENTS_HOME is set", () => {
    vi.stubEnv("BUILDERFORCE_AGENTS_HOME", "/srv/builderforce-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(
        `config: ${path.resolve("/srv/builderforce-home")}/.builderforce/builderforce.json`,
      ),
    ).toBe("config: $BUILDERFORCE_AGENTS_HOME/.builderforce/builderforce.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/builderforce")).toBe(path.resolve(os.homedir(), "builderforce"));
  });
  ...
  it("prefers BUILDERFORCE_AGENTS_HOME for tilde expansion", () => {
    vi.stubEnv("BUILDERFORCE_AGENTS_HOME", "/srv/builderforce-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/builderforce")).toBe(path.resolve("/srv/builderforce-home", "builderforce"));
