...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub BUILDERFORCE_AGENTS_STATE_DIR.
    // Also keep it outside os.tmpdir() so tmpdir localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["BUILDERFORCE_AGENTS_STATE_DIR"]);
    process.env.BUILDERFORCE_AGENTS_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "builderforce-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows default BuilderForceAgents state workspace and sandbox roots", async () => {
  ...
  it("rejects default BuilderForceAgents state per-agent workspace-* roots without explicit local roots", async () => {
    const { resolveStateDir } = await import("../config/paths.js");
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

    await expect(
      loadWebMedia(path.join(stateDir, "workspace-agentNodedy", "tmp", "render.bin"), {
        maxBytes: 1024 * 1024,
        readFile,
      }),
    ).rejects.toThrow(/not under an allowed directory/i);
  ...
  it("allows per-agent workspace-* paths with explicit local roots", async () => {
    ...
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
    const agentWorkspaceDir = path.join(stateDir, "workspace-agentNodedy");
