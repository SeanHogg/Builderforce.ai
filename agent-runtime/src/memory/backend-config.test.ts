...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } } as BuilderForceAgentsConfig;
  ...
  it("resolves qmd backend with default collections", () => {
    ...
    expect(names.has("builderforce-memory-dir-main")).toBe(true);
  });
  ...
  it("scopes qmd collection names per agent", () => {
    ...
    expect(mainNames.has("builderforce-memory-dir-main")).toBe(true);
    expect(devNames.has("memory-dir-dev")).toBe(true);
    expect(devNames.has("builderforce-memory-dir-dev")).toBe(true);
