...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "builderforce-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    BUILDERFORCE_AGENTS_GATEWAY_TOKEN: "test-token",
    BUILDERFORCE_AGENTS_CONFIG_DIR: join(sandbox.rootDir, "config"),
    BUILDERFORCE_AGENTS_WORKSPACE_DIR: join(sandbox.rootDir, "builderforce"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync(systemBash, [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        BUILDERFORCE_AGENTS_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        BUILDERFORCE_AGENTS_EXTRA_MOUNTS: undefined,
        BUILDERFORCE_AGENTS_HOME_VOLUME: "builderforce-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("BUILDERFORCE_AGENTS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("BUILDERFORCE_AGENTS_EXTRA_MOUNTS=");
    expect(envFile).toContain("BUILDERFORCE_AGENTS_HOME_VOLUME=builderforce-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("builderforce-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("builderforce-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg BUILDERFORCE_AGENTS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
