...
function resolveGatewayPort(cfg: BuilderForceAgentsConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: BuilderForceAgentsConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD?.trim() ||
    env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD?.trim() ||
