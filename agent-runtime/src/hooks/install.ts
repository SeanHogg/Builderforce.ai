...
async function ensureBuilderForceAgentsHooks(manifest: HookPackageManifest) {
  ...
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing builderforce.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json builderforce.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureBuilderForceAgentsHooks(manifest);
  } catch (err) {
...
export async function installHooksFromArchive(params: {
  ...
  return await withTempDir("builderforce-hook-", async (tmpDir) => {
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await withTempDir("builderforce-hook-pack-", async (tmpDir) => {
