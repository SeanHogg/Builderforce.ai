...
homepage: https://docs.builderforce.ai/automation/hooks#command-logger
metadata:
  {
    "builderforce":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.builderforce/logs/commands.log`
...
`~/.builderforce/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.builderforce/logs/commands.log ~/.builderforce/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/builderforce`:
   ```
   /home/username/.builderforce/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
