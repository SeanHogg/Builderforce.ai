...
This directory contains hooks that ship with BuilderForceAgents. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
homepage: https://docs.builderforce.ai/automation/hooks#my-hook
metadata:
...
Hooks can be configured in `~/.builderforce/builderforce.json`:
...
2. Restart gateway: `pkill -9 -f 'builderforce.*gateway' && pnpm builderforce gateway`
3. Enable the hook: `builderforce hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.builderforce.ai/automation/hooks
