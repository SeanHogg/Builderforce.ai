...
async function runNewWithPreviousSessionEntry(params: {
  ...
  const event = createHookEvent("command", "new", "agent:main:main", {
    cfg:
      ...
      } satisfies BuilderForceAgentsConfig),
    previousSessionEntry: params.previousSessionEntry,
  });

  await handler(event);

  const memoryDir = path.join(params.tempDir, ".builderforce", "memory");
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => BuilderForceAgentsConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("builderforce-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): BuilderForceAgentsConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
...
describe("session-memory hook", () => {
  it("skips non-command events", async () => {
    ...
    const memoryDir = path.join(tempDir, ".builderforce", "memory");
