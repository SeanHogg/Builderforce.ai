...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.BUILDERFORCE_AGENTS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*builderforce-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "builderforce";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "builderforce");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "builderforce-gw");
  ...
  records.push(`_builderforce-gw._tcp IN PTR ${instanceLabel}._builderforce-gw._tcp`);
  records.push(`${instanceLabel}._builderforce-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._builderforce-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; builderforce-content-hash: ${contentHash}\n${contentBody}`;
}
