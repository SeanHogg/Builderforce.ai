...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: BuilderForceAgentsConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: BuilderForceAgentsConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: BuilderForceAgentsConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: BuilderForceAgentsConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: BuilderForceAgentsConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: BuilderForceAgentsConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: BuilderForceAgentsConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
