  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set BUILDERFORCE_AGENTS_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.BUILDERFORCE_AGENTS_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
