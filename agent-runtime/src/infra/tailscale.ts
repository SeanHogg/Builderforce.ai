...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.BUILDERFORCE_AGENTS_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for BuilderForceAgents. You can keep running the web gateway without it: \`${formatCliCommand("builderforce gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
