...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.BUILDERFORCE_AGENTS_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.BUILDERFORCE_AGENTS_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_TEST_TAILSCALE_BINARY;
    } else {
      process.env.BUILDERFORCE_AGENTS_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
