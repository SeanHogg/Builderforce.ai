...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: BuilderForceAgentsConfig | null = null;
  ...
  get(key: string, cfg: BuilderForceAgentsConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: BuilderForceAgentsConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: BuilderForceAgentsConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: BuilderForceAgentsConfig): void {
