...
let ensureBuilderForceAgentsCliOnPath: typeof import("./path-env.js").ensureBuilderForceAgentsCliOnPath;

describe("ensureBuilderForceAgentsCliOnPath", () => {
  const envKeys = [
    "PATH",
    "BUILDERFORCE_AGENTS_PATH_BOOTSTRAPPED",
    "BUILDERFORCE_AGENTS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureBuilderForceAgentsCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling builderforce exists", () => {
    const tmp = abs("/tmp/builderforce-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "builderforce");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.BUILDERFORCE_AGENTS_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.BUILDERFORCE_AGENTS_PATH_BOOTSTRAPPED = "1";
    ensureBuilderForceAgentsCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/builderforce-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "builderforce");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/builderforce-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "builderforce");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "builderforce");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/builderforce-path/case-linuxbrew");
