...
export function rejectEgressTarget(rawUrl: string | undefined): string | null {
 ...
 */
export async function performHostEgress(req: HostEgressRequest): Promise<HostEgressResponseFrame> {
  const rejection = rejectEgressTarget(req.url);
  if (rejection) {
