...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-exec-approvals-"));
}
...
describe("resolve exec approvals defaults", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    const prevBuilderForceAgentsHome = process.env.BUILDERFORCE_AGENTS_HOME;
    try {
      process.env.BUILDERFORCE_AGENTS_HOME = dir;
      expect(path.normalize(resolveExecApprovalsPath())).toBe(
        path.normalize(path.join(dir, ".builderforce", "exec-approvals.json")),
      );
      expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
        path.normalize(path.join(dir, ".builderforce", "exec-approvals.sock")),
      );
    } finally {
      if (prevBuilderForceAgentsHome === undefined) {
        delete process.env.BUILDERFORCE_AGENTS_HOME;
      } else {
        process.env.BUILDERFORCE_AGENTS_HOME = prevBuilderForceAgentsHome;
      }
    }
  });
});
...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    try {
      process.env.BUILDERFORCE_AGENTS_HOME = dir;
      const approvalsPath = path.join(dir, ".builderforce", "exec-approvals.json");
