...
import { CORE_PACKAGE_NAME, isCorePackageName, stripPackagePrefix } from "./package-names.js";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "builderforce.mjs");
    ...
    if (!doctorEntryExists) {
      steps.push({
        name: "builderforce doctor entry",
    ...
    const doctorStep = await runStep(
      step("builderforce doctor", doctorArgv, gitRoot, { BUILDERFORCE_AGENTS_UPDATE_IN_PROGRESS: "1" }),
    );
  ...
  if (globalManager) {
    const packageName = (await readPackageName(pkgRoot)) ?? CORE_PACKAGE_NAME;
