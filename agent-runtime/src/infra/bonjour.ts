...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "BuilderForceAgents";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(BuilderForceAgents\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.BUILDERFORCE_AGENTS_MDNS_HOSTNAME?.trim() ||
    process.env.BUILDERFORCE_AGENTS_MDNS_HOSTNAME?.trim() ||
    "builderforce";
  const hostname =
    hostnameRaw
