...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("builderforce-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-builderforce-archive.tgz");
  ...
  it("rejects unsupported archive extensions", async () => {
    const dir = await createTempDir("builderforce-install-source-utils-");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using the final non-empty stdout line", async () => {
    const cwd = await createTempDir("builderforce-install-source-utils-");
    runCommandWithTimeoutMock.mockResolvedValue({
      stdout: "npm notice created package\nbuilderforce-plugin-1.2.3.tgz\n",
      stderr: "",
      code: 0,
      signal: null,
      killed: false,
    ...
    const result = await packNpmSpecToArchive({
      spec: "builderforce-plugin@1.2.3",
      timeoutMs: 1000,
      cwd,
    });
    ...
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "builderforce-plugin-1.2.3.tgz"),
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "builderforce-plugin@1.2.3", "--ignore-scripts"],
