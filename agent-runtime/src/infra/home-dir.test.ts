...
describe("resolveEffectiveHomeDir", () => {
  it("prefers BUILDERFORCE_AGENTS_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/builderforce-home"),
    );
  });
  ...
  it("expands BUILDERFORCE_AGENTS_HOME when set to ~", () => {
    const env = {
      BUILDERFORCE_AGENTS_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for BUILDERFORCE_AGENTS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { BUILDERFORCE_AGENTS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when BUILDERFORCE_AGENTS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ BUILDERFORCE_AGENTS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { BUILDERFORCE_AGENTS_HOME: "/srv/builderforce-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/builderforce-home")}/x`);
  });
