...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__builderforce_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "builderforce-root");
...
describe("resolveBuilderForceAgentsPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const pkgRoot = path.join(project, "node_modules", "builderforce");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "builderforce" }));
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "builderforce");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "builderforce.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "builderforce" }));

    expect(resolveBuilderForceAgentsPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveBuilderForceAgentsPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-builderforce package roots", async () => {
    const { resolveBuilderForceAgentsPackageRootSync } = await import("./builderforce-root.js");

    const pkgRoot = fx("not-builderforce");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-builderforce" }));
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveBuilderForceAgentsPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
