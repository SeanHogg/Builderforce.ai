...
import { resolveBuilderForceAgentsPackageRoot, resolveBuilderForceAgentsPackageRootSync } from "./builderforce-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveBuilderForceAgentsPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "builderforce" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveBuilderForceAgentsPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // builderforce.mjs or dist/<bundle>.js
