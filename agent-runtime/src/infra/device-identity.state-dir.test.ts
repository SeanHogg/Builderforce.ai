...
describe("device identity state dir defaults", () => {
  ...
  it("writes the default identity file under BUILDERFORCE_AGENTS_STATE_DIR", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-identity-state-"));
