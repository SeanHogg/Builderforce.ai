...
type EnsureBuilderForceAgentsPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureBuilderForceAgentsPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `builderforce` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "builderforce");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "builderforce"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `builderforce` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureBuilderForceAgentsCliOnPath(opts: EnsureBuilderForceAgentsPathOpts = {}) {
  if (isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_PATH_BOOTSTRAPPED)) {
    return;
  }
