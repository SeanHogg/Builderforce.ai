...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-heartbeat-suite-"));
});
...
describe("runHeartbeatOnce", () => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("builderforce-hb");
  ...
  it("skips heartbeat when HEARTBEAT.md is effectively empty (saves API calls)", async () => {
    ...
    try {
      ...
      await fs.mkdir(path.join(workspaceDir, ".builderforce"), { recursive: true });
