...
export function triggerBuilderForceAgentsRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.BUILDERFORCE_AGENTS_SYSTEMD_UNIT,
        process.env.BUILDERFORCE_AGENTS_PROFILE,
      );
  ...
  const label =
    process.env.BUILDERFORCE_AGENTS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.BUILDERFORCE_AGENTS_PROFILE);
