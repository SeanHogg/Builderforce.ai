...
describe("update-startup", () => {
  ...
  let resolveBuilderForceAgentsPackageRoot: (typeof import("./builderforce-root.js"))["resolveBuilderForceAgentsPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "BUILDERFORCE_AGENTS_STATE_DIR");
    prevStateDir = process.env.BUILDERFORCE_AGENTS_STATE_DIR;
    process.env.BUILDERFORCE_AGENTS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveBuilderForceAgentsPackageRoot } = await import("./builderforce-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveBuilderForceAgentsPackageRoot).mockResolvedValue("/opt/builderforce");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/builderforce",
      installKind: "package",
      packageManager: "npm",
