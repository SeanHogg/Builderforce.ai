...
import { POSIX_BUILDERFORCE_AGENTS_TMP_DIR, resolvePreferredBuilderForceAgentsTmpDir } from "./tmp-builderforce-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredBuilderForceAgentsTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `builderforce-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredBuilderForceAgentsTmpDir({
    accessSync,
...
describe("resolvePreferredBuilderForceAgentsTmpDir", () => {
  it("prefers /tmp/builderforce when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_BUILDERFORCE_AGENTS_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/builderforce when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_BUILDERFORCE_AGENTS_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/builderforce when /tmp/builderforce is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/builderforce when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/builderforce is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/builderforce is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/builderforce is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
