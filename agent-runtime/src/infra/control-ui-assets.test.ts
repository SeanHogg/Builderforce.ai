...
vi.mock("./builderforce-root.js", () => ({
  resolveBuilderForceAgentsPackageRoot: vi.fn(async () => null),
  resolveBuilderForceAgentsPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveBuilderForceAgentsPackageRoot when available", async () => {
    const builderforceRoot = await import("./builderforce-root.js");
    const { resolveControlUiDistIndexPath } = await import("./control-ui-assets.js");

    const pkgRoot = abs("fixtures/builderforce");
    (
      builderforceRoot.resolveBuilderForceAgentsPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/builderforce"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "builderforce" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "builderforce.mjs"))).resolves.toBe(
  ...
  it("returns null when fallback package name does not match", async () => {
    ...
    const root = abs("fixtures/not-builderforce");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    const pkgRoot = abs("fixtures/builderforce-bundle");
    (
      builderforceRoot.resolveBuilderForceAgentsPackageRootSync as unknown as ReturnType<typeof vi.fn>
