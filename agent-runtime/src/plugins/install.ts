...
async function ensureBuilderForceAgentsExtensions(manifest: PackageManifest) {
  ...
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing builderforce.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json builderforce.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureBuilderForceAgentsExtensions(manifest);
  } catch (err) {
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "builderforce security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "builderforce security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(params: {
  ...
  return await withTempDir("builderforce-plugin-", async (tmpDir) => {
...
export async function installPluginFromNpmSpec(params: {
  ...
  return await withTempDir("builderforce-npm-pack-", async (tmpDir) => {
