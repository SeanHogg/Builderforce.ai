...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `builderforce-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        builderforce: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      builderforce: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.builderforce/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without builderforce.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@builderforce/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toMatch(/(?:builderforce|builderforce)\.extensions/);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@builderforce/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@builderforce/voice-call@0.0.1",
    });
