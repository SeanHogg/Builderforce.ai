...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `builderforce-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "builderforce.plugin.json"), JSON.stringify(manifest), "utf-8");
}
