...
const fixtureRoot = path.join(os.tmpdir(), `builderforce-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.BUILDERFORCE_AGENTS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "builderforce.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.BUILDERFORCE_AGENTS_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadBuilderForceAgentsPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.BUILDERFORCE_AGENTS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadBuilderForceAgentsPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadBuilderForceAgentsPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
