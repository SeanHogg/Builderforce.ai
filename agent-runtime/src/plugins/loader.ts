...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: BuilderForceAgentsPluginDefinition;
  register?: BuilderForceAgentsPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as BuilderForceAgentsPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as BuilderForceAgentsPluginDefinition;
...
export function loadBuilderForceAgentsPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverBuilderForceAgentsPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "@seanhogg/builderforce-agents/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "@seanhogg/builderforce-agents/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: BuilderForceAgentsPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as BuilderForceAgentsPluginModule;
    } catch (err) {
