...
export const PLUGIN_MANIFEST_FILENAME = "builderforce.plugin.json";
...
// package.json "builderforce" metadata (used for onboarding/catalog)
...
export type BuilderForceAgentsPackageManifest = {
  extensions?: string[];
...
export type PackageManifest = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<AnyManifestKey, BuilderForceAgentsPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): BuilderForceAgentsPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
  ...
  for (const legacyKey of LEGACY_MANIFEST_KEYS) {
    const legacy = (manifest as Record<string, unknown>)[legacyKey] as
      | BuilderForceAgentsPackageManifest
      | undefined;
    if (legacy !== undefined) {
      return legacy;
    }
