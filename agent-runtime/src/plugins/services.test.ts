...
import type { BuilderForceAgentsPluginService, BuilderForceAgentsPluginServiceContext } from "./types.js";
...
function createRegistry(services: BuilderForceAgentsPluginService[]) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: BuilderForceAgentsPluginServiceContext[] = [];

    const serviceA: BuilderForceAgentsPluginService = {
      id: "service-a",
      start: (ctx) => {
        starts.push("a");
        contexts.push(ctx);
    ...
    const serviceB: BuilderForceAgentsPluginService = {
      id: "service-b",
      start: (ctx) => {
        starts.push("b");
        contexts.push(ctx);
    ...
    const serviceC: BuilderForceAgentsPluginService = {
      id: "service-c",
      start: (ctx) => {
        starts.push("c");
        contexts.push(ctx);
