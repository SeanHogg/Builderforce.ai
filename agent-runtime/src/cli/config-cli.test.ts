...
import type { ConfigFileSnapshot, BuilderForceAgentsConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `builderforce config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<(cfg: BuilderForceAgentsConfig) => Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: BuilderForceAgentsConfig) => mockWriteConfigFile(cfg),
}));
...
function buildSnapshot(params: {
  resolved: BuilderForceAgentsConfig;
...
function setSnapshot(resolved: BuilderForceAgentsConfig, config: BuilderForceAgentsConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: BuilderForceAgentsConfig = {
        agents: {
      ...
      const runtimeMerged: BuilderForceAgentsConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
