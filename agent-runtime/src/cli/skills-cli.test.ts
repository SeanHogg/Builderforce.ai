...
describe("skills-cli", () => {
  describe("formatSkillsList", () => {
    ...
    it("allows overriding the CLI hint via config", () => {
      const report = createMockReport([]);
      const cfg: BuilderForceAgentsConfig = { skills: { registry: { cli: "npx openagentNode" } } };
      const output = formatSkillsList(report, { config: cfg });
      expect(output).toContain("npx openagentNode");
  ...
  describe("formatSkillInfo", () => {
    ...
    it("skill info hint respects config override", () => {
      const report = createMockReport([]);
      const cfg: BuilderForceAgentsConfig = { skills: { registry: { cli: "openagentNode-cli" } } };
      const output = formatSkillInfo(report, "unknown-skill", { config: cfg });
      expect(output).toContain("openagentNode-cli");
      expect(output).not.toContain("builderforce.ai/marketplace");
    });
  ...
  describe("formatSkillsCheck", () => {
    it("shows summary of skill status", () => {
      ...
      expect(output2).toContain("openagentNode-cli");
      expect(output2).not.toContain("npx agenthub");
    });
  });
  ...
  describe("integration: loads real skills from bundled directory", () => {
    ...
    beforeAll(async () => {
      envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_BUNDLED_SKILLS_DIR"]);
      tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-skills-test-"));
      tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-bundled-skills-test-"));
      process.env.BUILDERFORCE_AGENTS_BUNDLED_SKILLS_DIR = tempBundledDir;
