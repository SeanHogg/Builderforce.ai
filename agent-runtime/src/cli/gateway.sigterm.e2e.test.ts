...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "builderforce-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      BUILDERFORCE_AGENTS_NO_RESPAWN: "1",
      BUILDERFORCE_AGENTS_STATE_DIR: stateDir,
      BUILDERFORCE_AGENTS_SKIP_CHANNELS: "1",
      BUILDERFORCE_AGENTS_SKIP_GMAIL_WATCHER: "1",
      BUILDERFORCE_AGENTS_SKIP_CRON: "1",
      BUILDERFORCE_AGENTS_SKIP_BROWSER_CONTROL_SERVER: "1",
      BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "builderforce-entry-bootstrap.mjs");
