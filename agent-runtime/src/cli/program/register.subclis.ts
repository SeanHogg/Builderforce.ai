...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.BUILDERFORCE_AGENTS_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<BuilderForceAgentsConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live BuilderForceAgents docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  {
    name: "agentbot",
    description: "Legacy agentbot command aliases",
    hasSubcommands: true,
    register: async (program) => {
      const mod = await import("../agentbot-cli.js");
      mod.registerAgentbotCli(program);
    },
  },
  {
    name: "pairing",
  ...
  {
    name: "plugins",
    description: "Manage BuilderForceAgents plugins and extensions",
    hasSubcommands: true,
    register: async (program) => {
