...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = new Command();
    registerCoreCliCommands(program, testProgramContext, ["node", "builderforce", "doctor"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = new Command();
    registerCoreCliCommands(program, testProgramContext, ["node", "builderforce", "vitest"]);

    const prevArgv = process.argv;
    process.argv = ["node", "builderforce", "status"];
    try {
      program.exitOverride();
      await program.parseAsync(["node", "builderforce", "status"]);
    } finally {
