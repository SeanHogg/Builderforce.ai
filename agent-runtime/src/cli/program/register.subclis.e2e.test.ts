...
describe("registerSubCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.BUILDERFORCE_AGENTS_DISABLE_LAZY_SUBCOMMANDS;
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    process.argv = ["node", "builderforce", "acp"];
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    process.argv = ["node", "builderforce"];
    const program = new Command();
    registerSubCliCommands(program, process.argv);
    ...
    expect(names).toContain("agentbot");
    expect(registerAcpCli).not.toHaveBeenCalled();
  });

  it("re-parses argv for lazy subcommands", async () => {
    process.argv = ["node", "builderforce", "nodes", "list"];
    const program = new Command();
    program.name("builderforce");
    registerSubCliCommands(program, process.argv);
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    process.argv = ["node", "builderforce", "acp", "--help"];
    const program = new Command();
    program.name("builderforce");
    registerSubCliCommands(program, process.argv);
    ...
    await program.parseAsync(["node", "builderforce", "acp"], { from: "user" });
    expect(registerAcpCli).toHaveBeenCalledTimes(1);
    expect(acpAction).toHaveBeenCalledTimes(1);
  });
});
