...
${formatHelpExamples([
  ['builderforce agent --to +15555550123 --message "status update"', "Start a new session."],
  ['builderforce agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['builderforce agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.builderforce.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.builderforce.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "builderforce agents set-identity --identity-file ~/.builderforce/workspace/IDENTITY.md --agent main",
