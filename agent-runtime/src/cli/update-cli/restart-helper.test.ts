...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'builderforce-gateway.service'");
    ...
    it("uses BUILDERFORCE_AGENTS_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const scriptPath = await prepareRestartScript({
        BUILDERFORCE_AGENTS_PROFILE: "default",
        BUILDERFORCE_AGENTS_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.builderforce.gateway'");
      expect(content).toContain('rm -f "$0"');
    ...
    it("uses BUILDERFORCE_AGENTS_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const scriptPath = await prepareRestartScript({
        BUILDERFORCE_AGENTS_PROFILE: "default",
        BUILDERFORCE_AGENTS_LAUNCHD_LABEL: "com.custom.builderforce",
      });

      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.builderforce'");
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "BuilderForceAgents Gateway"');
      expect(content).toContain('schtasks /Run /TN "BuilderForceAgents Gateway"');
      // Batch self-cleanup
    ...
    it("uses BUILDERFORCE_AGENTS_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const scriptPath = await prepareRestartScript({
        BUILDERFORCE_AGENTS_PROFILE: "default",
        BUILDERFORCE_AGENTS_WINDOWS_TASK_NAME: "BuilderForceAgents Gateway (custom)",
      });

      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain('schtasks /End /TN "BuilderForceAgents Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "BuilderForceAgents Gateway (custom)"');
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const scriptPath = await prepareRestartScript({
        BUILDERFORCE_AGENTS_PROFILE: "production",
      });

      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain("builderforce-gateway-production.service");
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const scriptPath = await prepareRestartScript({
        BUILDERFORCE_AGENTS_PROFILE: "staging",
      });

      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain("gui/502/ai.builderforce.staging");
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain('schtasks /End /TN "BuilderForceAgents Gateway (production)"');
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const scriptPath = await prepareRestartScript({
        BUILDERFORCE_AGENTS_PROFILE: "it's-a-test",
      });
    ...
    it("rejects unsafe batch profile names on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = await prepareRestartScript({
        BUILDERFORCE_AGENTS_PROFILE: "test&whoami",
      });
