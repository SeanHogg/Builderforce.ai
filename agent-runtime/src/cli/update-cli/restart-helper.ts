...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = env.BUILDERFORCE_AGENTS_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.BUILDERFORCE_AGENTS_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.BUILDERFORCE_AGENTS_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.BUILDERFORCE_AGENTS_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.BUILDERFORCE_AGENTS_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.BUILDERFORCE_AGENTS_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `builderforce-restart-${timestamp}.sh`;
...
`;
      ...
      filename = `builderforce-restart-${timestamp}.bat`;
