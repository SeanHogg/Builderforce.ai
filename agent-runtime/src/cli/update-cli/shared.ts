...
export function resolveGitInstallDir(): string {
  const override = process.env.BUILDERFORCE_AGENTS_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveBuilderForceAgentsPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", BUILDERFORCE_AGENTS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `BUILDERFORCE_AGENTS_GIT_DIR points at a non-git directory: ${params.dir}. Set BUILDERFORCE_AGENTS_GIT_DIR to an empty folder or a builderforce checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`BUILDERFORCE_AGENTS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "builderforce.mjs");
