...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: BuilderForceAgentsConfig; warnings: string[] } {
...
function enablePluginInConfig(config: BuilderForceAgentsConfig, pluginId: string): BuilderForceAgentsConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.builderforce.ai/cli/plugins")}\n`,
    );
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: BuilderForceAgentsConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.builderforce.ai/plugin");
      lines.push("");
