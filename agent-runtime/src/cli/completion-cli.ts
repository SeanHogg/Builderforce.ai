...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# BuilderForceAgents Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(builderforce completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# BuilderForceAgents Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(builderforce completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $BUILDERFORCE_AGENTS_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "builderforce") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c builderforce -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'builderforce gateway' and not 'builderforce other gateway', we need to check the sequence.
