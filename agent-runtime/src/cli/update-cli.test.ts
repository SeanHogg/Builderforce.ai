...
import type { BuilderForceAgentsConfig, ConfigFileSnapshot } from "../config/types.builderforce.js";
...
vi.mock("../infra/builderforce-root.js", () => ({
  resolveBuilderForceAgentsPackageRoot: vi.fn(),
}));
...
const { resolveBuilderForceAgentsPackageRoot } = await import("../infra/builderforce-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-update-tests-"));
  });
  ...
  const baseConfig = {} as BuilderForceAgentsConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/builderforce-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveBuilderForceAgentsPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("builderforce-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveBuilderForceAgentsPackageRoot).mockReset();
    ...
    vi.mocked(resolveBuilderForceAgentsPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("builderforce");
    readPackageVersion.mockResolvedValue("1.0.0");
    resolveGlobalManager.mockResolvedValue("npm");
    serviceLoaded.mockResolvedValue(false);
    prepareRestartScript.mockResolvedValue("/tmp/builderforce-restart-test.sh");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("BuilderForceAgents update status");
  });
  ...
  it("uses stored beta channel when configured", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...baseSnapshot,
      config: { update: { channel: "beta" } } as BuilderForceAgentsConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "git",
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveBuilderForceAgentsPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "npm",
      steps: [],
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    const envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_UPDATE_IN_PROGRESS"]);
    const randomSpy = vi.spyOn(Math, "random").mockReturnValue(0);
    try {
      delete process.env.BUILDERFORCE_AGENTS_UPDATE_IN_PROGRESS;
      vi.mocked(runGatewayUpdate).mockResolvedValue(mockResult);
      vi.mocked(runDaemonRestart).mockResolvedValue(true);
      ...
      expect(process.env.BUILDERFORCE_AGENTS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("builderforce-update-wizard");
    const envSnapshot = captureEnv(["BUILDERFORCE_AGENTS_GIT_DIR"]);
    try {
      setTty(true);
      process.env.BUILDERFORCE_AGENTS_GIT_DIR = tempDir;
