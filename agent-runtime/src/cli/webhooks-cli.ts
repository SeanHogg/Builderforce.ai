...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.builderforce.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + BuilderForceAgents hooks")
    ...
    .option("--hook-url <url>", "BuilderForceAgents hook URL")
    .option("--hook-token <token>", "BuilderForceAgents hook token")
