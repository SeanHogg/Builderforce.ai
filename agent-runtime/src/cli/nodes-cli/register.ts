    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["builderforce nodes status", "List known nodes with live status."],
          ["builderforce nodes pairing pending", "Show pending node pairing requests."],
          ['builderforce nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["builderforce nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.builderforce.ai/cli/nodes")}\n`,
    );
