...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    BUILDERFORCE_AGENTS_STATE_DIR: process.env.BUILDERFORCE_AGENTS_STATE_DIR,
    BUILDERFORCE_AGENTS_CONFIG_PATH: process.env.BUILDERFORCE_AGENTS_CONFIG_PATH,
    BUILDERFORCE_AGENTS_GATEWAY_PORT: process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT,
    BUILDERFORCE_AGENTS_PROFILE: process.env.BUILDERFORCE_AGENTS_PROFILE,
  };

  beforeEach(() => {
    process.env.BUILDERFORCE_AGENTS_STATE_DIR = "/tmp/builderforce-cli-state";
    process.env.BUILDERFORCE_AGENTS_CONFIG_PATH = "/tmp/builderforce-cli-state/builderforce.json";
    delete process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT;
    delete process.env.BUILDERFORCE_AGENTS_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.BUILDERFORCE_AGENTS_STATE_DIR !== undefined) {
      process.env.BUILDERFORCE_AGENTS_STATE_DIR = originalEnv.BUILDERFORCE_AGENTS_STATE_DIR;
    } else {
      delete process.env.BUILDERFORCE_AGENTS_STATE_DIR;
    }

    if (originalEnv.BUILDERFORCE_AGENTS_CONFIG_PATH !== undefined) {
      process.env.BUILDERFORCE_AGENTS_CONFIG_PATH = originalEnv.BUILDERFORCE_AGENTS_CONFIG_PATH;
    } else {
      delete process.env.BUILDERFORCE_AGENTS_CONFIG_PATH;
    }

    if (originalEnv.BUILDERFORCE_AGENTS_GATEWAY_PORT !== undefined) {
      process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT = originalEnv.BUILDERFORCE_AGENTS_GATEWAY_PORT;
    } else {
      delete process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT;
    }

    if (originalEnv.BUILDERFORCE_AGENTS_PROFILE !== undefined) {
      process.env.BUILDERFORCE_AGENTS_PROFILE = originalEnv.BUILDERFORCE_AGENTS_PROFILE;
    } else {
      delete process.env.BUILDERFORCE_AGENTS_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        BUILDERFORCE_AGENTS_PROFILE: "dev",
        BUILDERFORCE_AGENTS_STATE_DIR: "/tmp/builderforce-daemon-state",
        BUILDERFORCE_AGENTS_CONFIG_PATH: "/tmp/builderforce-daemon-state/builderforce.json",
        BUILDERFORCE_AGENTS_GATEWAY_PORT: "19001",
      },
