...
function getSkillsHint(config?: BuilderForceAgentsConfig): string {
...
function appendSkillsHint(output: string, json?: boolean, config?: BuilderForceAgentsConfig): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("builderforce skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendSkillsHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("builderforce skills list")}\` to see available skills.`,
      opts.json,
      opts.config,
    );
  }
