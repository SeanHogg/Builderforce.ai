...
function formatSourceLabel(source: string, workspaceDir: string, agentId: string): string {
  if (source === "memory") {
    return shortenHomeInString(
      `memory (${path.join(workspaceDir, ".builderforce", "MEMORY.md")} + ${path.join(
        workspaceDir,
        ".builderforce",
        "memory",
      )}${path.sep}*.md)`,
...
async function scanMemoryFiles(
  ...
  const [legacyMemoryDir, builderforceMemoryDir] = getDefaultMemoryDirs(workspaceDir);
  ...
  let dirChecked = "";
  // Check both memory/ and .builderforce/memory/ (canonical)
  const dirsToCheck = [memoryDir, builderforceMemoryDir];
  ...
  if (dirReadable === null && dirChecked === "") {
    ...
    issues.push(
      `memory directories missing (${shortenHomePath(memoryDir)} and ${shortenHomePath(builderforceMemoryDir)})`,
    );
  }
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["builderforce memory status", "Show index and provider status."],
          ["builderforce memory index --force", "Force a full reindex."],
          ['builderforce memory search --query "deployment notes"', "Search indexed memory entries."],
          ["builderforce memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.builderforce.ai/cli/memory")}\n`,
    );
