...
describe("registerQrCli", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("BUILDERFORCE_AGENTS_GATEWAY_TOKEN", "");
    vi.stubEnv("BUILDERFORCE_AGENTS_GATEWAY_TOKEN", "");
    vi.stubEnv("BUILDERFORCE_AGENTS_GATEWAY_PASSWORD", "");
    vi.stubEnv("BUILDERFORCE_AGENTS_GATEWAY_PASSWORD", "");
  });
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("builderforce devices approve <requestId>");
  });
