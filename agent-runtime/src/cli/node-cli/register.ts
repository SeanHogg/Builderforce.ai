    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "builderforce node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["builderforce node status", "Check node host service status."],
          ["builderforce node install", "Install the node host service."],
          ["builderforce node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.builderforce.ai/cli/node")}\n`,
    );
