...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of BuilderForce Agents")).toBe("friends-of-builderforce-agents");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of BuilderForce Agents"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-builderforce-agents" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-builderforce-agents" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of BuilderForce Agents"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-builderforce-agents");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-builderforce-agents": { slug: "friends-of-builderforce-agents" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of BuilderForce Agents"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["builderforce-agents-dm"],
        channelId: "1",
        channelName: "BuilderForce Agents DM",
        channelSlug: "builderforce-agents-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
