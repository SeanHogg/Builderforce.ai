...
describe("agent components", () => {
  const createCfg = (): BuilderForceAgentsConfig => ({}) as BuilderForceAgentsConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): BuilderForceAgentsConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/builderforce-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/builderforce",
    });
  });
});
