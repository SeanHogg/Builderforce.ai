...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("builderforce");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const builderforce = ctx.forProfile("builderforce");

    const opened = await builderforce.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("builderforce")?.lastTargetId).toBe("CREATED");
  });
});
