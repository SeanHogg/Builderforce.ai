...
vi.mock("./chrome.js", () => ({
  resolveBuilderForceAgentsUserDataDir: vi.fn(() => "/tmp/builderforce-test/builderforce/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveBuilderForceAgentsUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "builderforce-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveBuilderForceAgentsUserDataDir).mockReturnValue(userDataDir);
