...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".builderforce-profile-decorated");
}
 ...
 */
export function decorateBuilderForceAgentsProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_BUILDERFORCE_AGENTS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_BUILDERFORCE_AGENTS_BROWSER_COLOR).toUpperCase();
