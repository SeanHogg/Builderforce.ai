...
function withLoopbackBrowserAuthImpl(
  ...
  if (
    ...
    headers.has("x-builderforce-password") ||
    headers.has("x-builderforce-password")
  ) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-builderforce-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-builderforce-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the BuilderForceAgents browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the BuilderForceAgents browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
