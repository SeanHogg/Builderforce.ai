...
const RELAY_AUTH_HEADER = "x-builderforce-relay-token";
...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN?.trim() || process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
function deriveDeterministicRelayAuthToken(port: number): string | null {
  ...
  return createHash("sha256")
    .update(`builderforce-relay:${port}:`)
    .update(gatewayToken)
    .digest("base64url");
}
...
async function looksLikeBuilderForceAgentsRelay(baseUrl: string): Promise<boolean> {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/BuilderForceAgents-Extension-Relay",
          revision: "0",
          userAgent: "BuilderForceAgents-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "BuilderForceAgents/extension-relay",
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikeBuilderForceAgentsRelay(info.baseUrl))) {
