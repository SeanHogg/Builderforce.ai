...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/builderforce" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          builderforce: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchBuilderForceAgentsChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveBuilderForceAgentsUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopBuilderForceAgentsChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT;
    ...
    state.prevGatewayToken = process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
    } else {
      process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD;
    } else {
      process.env.BUILDERFORCE_AGENTS_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
