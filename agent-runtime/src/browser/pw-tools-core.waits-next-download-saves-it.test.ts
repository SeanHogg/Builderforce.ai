...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredBuilderForceAgentsTmpDir: vi.fn(() => "/tmp/builderforce"),
}));
vi.mock("../infra/tmp-builderforce-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredBuilderForceAgentsTmpDir.mockReturnValue("/tmp/builderforce");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredBuilderForceAgentsTmpDir.mockReturnValue("/tmp/builderforce-preferred");
    ...
    const expectedRootedDownloadsDir = path.join(
      path.sep,
      "tmp",
      "builderforce-preferred",
      "downloads",
    );
    const expectedDownloadsTail = `${path.join("tmp", "builderforce-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredBuilderForceAgentsTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "builderforce-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "builderforce-preferred", "downloads")}${path.sep}`),
    );
  });
