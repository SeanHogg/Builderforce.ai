import type { BrowserConfig, BrowserProfileConfig, BuilderForceAgentsConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_BUILDERFORCE_AGENTS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "builderforce" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_BUILDERFORCE_AGENTS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "builderforce" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_BUILDERFORCE_AGENTS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_BUILDERFORCE_AGENTS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an BuilderForceAgents browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for builderforce/builderforce CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: BuilderForceAgentsConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_BUILDERFORCE_AGENTS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "builderforce";
