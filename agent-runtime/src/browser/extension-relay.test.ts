...
describe("chrome extension relay server", () => {
  ...
  it("derives relay auth headers from gateway token for loopback URLs", async () => {
    ...
    process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN = "test-gateway-token";
    try {
      const headers = getChromeExtensionRelayAuthHeaders(`http://127.0.0.1:${port}`);
      expect(Object.keys(headers)).toContain("x-builderforce-relay-token");
      expect((headers["x-builderforce-relay-token"] ?? "").length).toBeGreaterThan(20);
    } finally {
      if (prev === undefined) {
        delete process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN;
      } else {
        process.env.BUILDERFORCE_AGENTS_GATEWAY_TOKEN = prev;
      }
    }
  });
  ...
  it("does not swallow EADDRINUSE when occupied port is not an builderforce relay", async () => {
    const port = await getFreePort();
