...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.BUILDERFORCE_AGENTS_TTS_PREFS;
      process.env.BUILDERFORCE_AGENTS_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.BUILDERFORCE_AGENTS_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: BuilderForceAgentsConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
