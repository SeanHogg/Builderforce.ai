...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".builderforce");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.BUILDERFORCE_AGENTS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$BUILDERFORCE_AGENTS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via BUILDERFORCE_AGENTS_STATE_DIR.
