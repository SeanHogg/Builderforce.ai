...
export function resolveBuilderForceAgentsManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type BuilderForceAgentsManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
): BuilderForceAgentsManifestRequires | undefined {
  const requiresRaw =
...
export function resolveBuilderForceAgentsManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export type ParsedBuilderForceAgentsManifestInstallBase = {
...
export function parseBuilderForceAgentsManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedBuilderForceAgentsManifestInstallBase | undefined {
  ...
  const spec: ParsedBuilderForceAgentsManifestInstallBase = {
    raw,
    kind,
  };
