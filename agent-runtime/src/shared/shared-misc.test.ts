...
describe("shared/frontmatter", () => {
  ...
  test("resolveBuilderForceAgentsManifestBlock parses JSON5 metadata and picks builderforce block", () => {
    const frontmatter = {
      metadata: "{ builderforce: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveBuilderForceAgentsManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveBuilderForceAgentsManifestBlock returns undefined for invalid input", () => {
    expect(resolveBuilderForceAgentsManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveBuilderForceAgentsManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveBuilderForceAgentsManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
