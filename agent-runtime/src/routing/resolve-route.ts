...
function listAgents(cfg: BuilderForceAgentsConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: BuilderForceAgentsConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: BuilderForceAgentsConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<BuilderForceAgentsConfig, EvaluatedBindingsCache>();
