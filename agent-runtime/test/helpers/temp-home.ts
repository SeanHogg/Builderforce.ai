...
type EnvSnapshot = {
  ...
  builderforceHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    builderforceHome: process.env.BUILDERFORCE_AGENTS_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("BUILDERFORCE_AGENTS_HOME", snapshot.builderforceHome);
  restoreKey("BUILDERFORCE_AGENTS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked BUILDERFORCE_AGENTS_HOME.
  delete process.env.BUILDERFORCE_AGENTS_HOME;
  process.env.BUILDERFORCE_AGENTS_STATE_DIR = path.join(base, ".builderforce");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "builderforce-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".builderforce", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
