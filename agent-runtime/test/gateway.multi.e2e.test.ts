...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `builderforce-e2e-${name}-`));
  const configDir = path.join(homeDir, ".builderforce");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          BUILDERFORCE_AGENTS_SKIP_BROWSER_CONTROL_SERVER: "1",
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-builderforce-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-builderforce-token": gwB.hookToken },
        ),
      ]);
