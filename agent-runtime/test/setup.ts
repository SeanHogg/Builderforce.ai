...
process.env.BUILDERFORCE_AGENTS_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime BuilderForceAgents modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: BuilderForceAgentsConfig) => {
    ...
    resolveAccount: (cfg: BuilderForceAgentsConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: BuilderForceAgentsConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
