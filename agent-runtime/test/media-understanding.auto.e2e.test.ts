...
import type { BuilderForceAgentsConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("builderforce-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("builderforce-bin-sherpa-");
      const modelDir = await makeTempDir("builderforce-sherpa-model-");
      ...
      const cfg: BuilderForceAgentsConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("builderforce-bin-whispercpp-");
      const modelDir = await makeTempDir("builderforce-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("builderforce-bin-gemini-");
      ...
      const cfg: BuilderForceAgentsConfig = { tools: { media: { image: {} } } };
