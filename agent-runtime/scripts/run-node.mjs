...
const shouldBuild = (deps) => {
  if (deps.env.BUILDERFORCE_AGENTS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.BUILDERFORCE_AGENTS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[builderforce] ${message}\n`);
};

const runBuilderForceAgents = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["builderforce.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runBuilderForceAgents(deps);
  }
