...
INSTALL_URL="${BUILDERFORCE_AGENTS_INSTALL_URL:-https://builderforce.ai/install.sh}"
SMOKE_PREVIOUS_VERSION="${BUILDERFORCE_AGENTS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${BUILDERFORCE_AGENTS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="builderforce"
PACKAGE_NAME="${BUILDERFORCE_AGENTS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (BUILDERFORCE_AGENTS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${BUILDERFORCE_AGENTS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${BUILDERFORCE_AGENTS_INSTALL_LATEST_OUT:-}"
fi
