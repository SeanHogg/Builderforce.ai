...
import type { ApiContributor, Entry, MapConfig, User } from "./update-agenttributors.types.js";

const REPO = "builderforce/builderforce";
const PER_LINE = 10;

const mapPath = resolve("scripts/agenttributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing agenttributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README agenttributors: ${entries.length} entries`);
