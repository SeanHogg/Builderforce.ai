...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "agentlog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
agentlog - BuilderForceAgents Logging Utility

USAGE:
    agentlog [OPTIONS]

DESCRIPTION:
    View BuilderForceAgents logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    BuilderForceAgents logs flow through the macOS unified log (subsystem: ai.builderforce).
...
QUICK START:
    agentlog -n 100             Show last 100 lines from all components
    agentlog -f                 Follow logs in real-time
    agentlog -e                 Show only errors
    agentlog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    agentlog                   Show last 50 lines from past 5 minutes (default)
    agentlog -f                Stream logs continuously
    agentlog -n 100            Show last 100 lines
    agentlog -e                Show only recent errors
    agentlog -l 30m -n 200     Show last 200 lines from past 30 minutes
    agentlog -c ServerManager  Show recent ServerManager logs
    agentlog -s "fail"         Search for "fail" in recent logs
    agentlog --server -e       Show recent server errors
    agentlog -f -d             Stream debug logs continuously
