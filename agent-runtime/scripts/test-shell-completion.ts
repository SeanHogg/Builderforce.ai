 ...
 * `builderforce update`. Use it to verify the completion installation flow
...
\`builderforce update\`, \`builderforce doctor\`, and \`builderforce onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# BuilderForceAgents Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`builderforce completion --install\` later to enable.`));
    return;
  }
