...
# BuilderForceAgents Auth Widget for Termux
...
SERVER="${BUILDERFORCE_AGENTS_SERVER:-${BUILDERFORCE_AGENTS_SERVER:-l36}}"

# Check auth status
termux-toast "Checking BuilderForceAgents auth..."

STATUS=$(ssh "$SERVER" '$HOME/builderforce/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/builderforce/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|BUILDERFORCE_AGENTS_EXPIRING|BUILDERFORCE_AGENTS_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/builderforce/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|BUILDERFORCE_AGENTS_EXPIRED|BUILDERFORCE_AGENTS_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: BuilderForceAgents won't work until re-auth"
                ;;
        esac
        ;;
