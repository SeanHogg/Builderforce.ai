#!/bin/bash
# Setup BuilderForceAgents Auth Management System
...
echo "=== BuilderForceAgents Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'builderforce-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  BuilderForceAgents message: Send warning via BuilderForceAgents itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/builderforce-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/builderforce-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/builderforce-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now builderforce-auth-monitor.timer
...
echo "   ssh phone 'chmod +x ~/.shortcuts/BuilderForceAgents*'"
echo ""
...
echo "  - Auth monitor: systemctl --user status builderforce-auth-monitor.timer"
echo ""
echo "Quick commands:"
