...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/builderforce/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "BuilderForceAgents Re-Auth" -c "After getting key, run: ssh $SERVER '~/builderforce/scripts/mobile-reauth.sh'" --id builderforce-auth
        ;;
    *)
