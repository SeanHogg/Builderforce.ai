...
function runTests() {
  const isolatedLock =
    process.env.BUILDERFORCE_AGENTS_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `builderforce-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      BUILDERFORCE_AGENTS_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
