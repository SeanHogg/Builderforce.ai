...
DMG_TEMP="$(mktemp -d /tmp/builderforce-dmg.XXXXXX)"
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/builderforce-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/builderforce-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/builderforce-dmg-limits.txt
