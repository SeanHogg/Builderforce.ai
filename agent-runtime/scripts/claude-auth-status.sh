...
# Checks both Claude Code and BuilderForceAgents auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
BUILDERFORCE_AGENTS_AUTH="$HOME/.builderforce/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    builderforce models status --json 2>/dev/null || true
}
...
check_builderforce_auth() {
    ...
    if [ ! -f "$BUILDERFORCE_AGENTS_AUTH" ]; then
        echo "MISSING"
        return 1
    fi
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$BUILDERFORCE_AGENTS_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    builderforce_status=$(check_builderforce_auth 2>/dev/null || true)

    claude_expires=0
    builderforce_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        builderforce_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        builderforce_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$BUILDERFORCE_AGENTS_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$builderforce_status" \
        --arg be "$builderforce_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            builderforce: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$builderforce_status" == EXPIRED* ]] || [[ "$builderforce_status" == MISSING* ]]; then
        echo "BUILDERFORCE_AGENTS_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$builderforce_status" == EXPIRING* ]]; then
        echo "BUILDERFORCE_AGENTS_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "BuilderForceAgents Auth (~/.builderforce/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$BUILDERFORCE_AGENTS_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$BUILDERFORCE_AGENTS_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'builderforce doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active builderforce >/dev/null 2>&1; then
    echo -e "BuilderForceAgents service: ${GREEN}running${NC}"
else
    echo -e "BuilderForceAgents service: ${RED}NOT running${NC}"
fi
