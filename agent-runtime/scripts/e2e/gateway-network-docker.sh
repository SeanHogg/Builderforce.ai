...
IMAGE_NAME="builderforce-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="builderforce-net-e2e-$$"
GW_NAME="builderforce-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "BUILDERFORCE_AGENTS_GATEWAY_TOKEN=$TOKEN" \
	  -e "BUILDERFORCE_AGENTS_SKIP_CHANNELS=1" \
	  -e "BUILDERFORCE_AGENTS_SKIP_GMAIL_WATCHER=1" \
	  -e "BUILDERFORCE_AGENTS_SKIP_CRON=1" \
	  -e "BUILDERFORCE_AGENTS_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
