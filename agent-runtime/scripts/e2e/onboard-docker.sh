...
IMAGE_NAME="builderforce-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/builderforce-bin:$PATH"
  mkdir -p /tmp/builderforce-bin
  cat > /tmp/builderforce-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/builderforce-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$BUILDERFORCE_AGENTS_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/builderforce-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/builderforce-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$BUILDERFORCE_AGENTS_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/builderforce-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    send "" 2.0
	  ...
	  run_case_local_basic() {
	    ...
	    export HOME="$home_dir"
	    mkdir -p "$HOME"
	    node "$BUILDERFORCE_AGENTS_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$HOME/.builderforce/workspace"
    config_path="$HOME/.builderforce/builderforce.json"
    sessions_dir="$HOME/.builderforce/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    export HOME="$home_dir"
	    ...
	    node "$BUILDERFORCE_AGENTS_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    export HOME="$home_dir"
    mkdir -p "$HOME/.builderforce"
    # Seed a remote config to exercise reset path.
	    cat > "$HOME/.builderforce/builderforce.json" <<'"'"'JSON'"'"'
{
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$BUILDERFORCE_AGENTS_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$BUILDERFORCE_AGENTS_ENTRY\" configure --section skills" send_skills_flow
