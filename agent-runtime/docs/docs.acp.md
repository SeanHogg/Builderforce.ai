# BuilderForce Agents ACP Bridge

This document describes how the BuilderForce Agents ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`builderforce acp` exposes an ACP agent over stdio and forwards prompts to a running
BuilderForce Agents Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
In Zed, open the Agent panel and select “BuilderForce Agents ACP” to start a thread.

## Execution Model

- ACP client spawns `builderforce acp` and speaks ACP messages over stdio.
...
builderforce acp --reset-session
```
...
`builderforce acp` resolves the Gateway URL and auth from CLI flags or config:
