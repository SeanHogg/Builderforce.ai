...
- `save_session_handoff` agent tool — writes `.builderforce/sessions/<id>.yaml` with summary, decisions, next steps, open questions, and artifacts. Registered in `createBuilderForce AgentsTools()`.
- TUI loads the latest handoff on session start and shows "Resuming from session …" with summary, decisions, and next steps. Silent no-op when `.builderforce/` does not exist.
- `/handoff` slash command — prompts the connected agent to produce and save a session handoff.

**Workflow Persistence**

- `saveWorkflowState` / `loadWorkflowState` / `listIncompleteWorkflowIds` in `project-context.ts` — serialize workflows to `.builderforce/sessions/workflow-<id>.yaml` after every task state change.
...
- `KnowledgeLoopService` — subscribes to `onAgentEvent`; writes timestamped activity entries (files created/edited, tools used) to `.builderforce/memory/YYYY-MM-DD.md` after each run; auto-syncs the full `.builderforce/` directory to builderforce.ai.
- `syncBuilderForce AgentsDirectory()` extracted as a standalone reusable export — callable any time, not just on gateway startup.
- `appendKnowledgeMemory()` added to `project-context.ts`.
- `project_knowledge` tool: new `"memory"` query type reads the last 7 `.builderforce/memory/*.md` files. Included in `"all"`.

**Agent-to-Agent Mesh Delegation**

- `GET /api/agents/fleet` (builderforce.ai) — agent-authenticated fleet discovery; returns all agents in the tenant with online status and capabilities.
- `PATCH /api/agents/:id/heartbeat` now accepts `{ capabilities: string[] }` and persists to a new `capabilities` column (migration `0007`). Heartbeat sends `["chat","tasks","relay","remote-dispatch"]`.
- `POST /api/agents/:id/forward` — source-agent-authenticated dispatch; delivers JSON payload to target agent via `AgentNodeRelayDO`.
- `AgentLinkRelayService` handles `remote.task` messages by dispatching them as local `chat.send`.
- `agent_fleet` agent tool — queries fleet, returns capabilities, hints at `remote:<id>` role format.
- `dispatchToRemoteAgentNode()` in `remote-subagent.ts` — POSTs to `/forward`, returns `{ status: "accepted" | "rejected" }`.
- Orchestrator: `setRemoteDispatchOptions()` + `remote:<agentNodeId>` role routing — bypasses local `spawnSubagentDirect`, calls `dispatchToRemoteAgentNode` instead.
- `GET /api/tenants/:id/agents` now reads real capabilities from DB; `capabilitySummary.remoteDispatch` is `true` only when online and reporting the capability.

Docs: https://docs.builderforce.ai
...
- UI: update control UI metadata, icons, and colors to BuilderForce Agents branding — use `builderforce.png` logo across favicon/Apple touch icons, add SEO metadata (`og:image`, `application-name`), and replace red accent tokens (#ff5c5c, #dc2626) with blue (#4d9eff, #3b82f6) to align with builderforce.ai design system.
- TUI: add `/init` slash command to initialize BuilderForce Agents project locally (creates `.builderforce/context.yaml`, `architecture.md`, `rules.yaml`), preventing collision with gateway session reset and providing clear separation between local project init and session management.
...
- CI/Containers: set `BUILDERFORCE_AGENTS_A2UI_SKIP_MISSING=1` for Docker build and npm publish workflow paths that run prepack builds without A2UI sources.
...
- CI/Release: align release workflow build env with CI by setting `BUILDERFORCE_AGENTS_A2UI_SKIP_MISSING=1` for `pnpm build` steps.
- Build: harden `scripts/bundle-a2ui.sh` for Windows Git Bash/NVM environments and robust source-presence detection.
- Tests: restore legacy hook manifest compatibility (`openagentNode` key), make slash-command docs test resilient to docs-site layout, and fix legacy state-dir symlink migration test setup.
...
- Docs: fix broken internal links in `builderforce.md` and `phase2.md`.
- Docs: rewrite `builderforce-overview.md` as a standalone BuilderForce Agents overview.

## 2026.2.22

### Changes

- Skills: remove bundled `food-order` skill from this repo; manage/install it from AgentHub instead.
...
- **BREAKING:** tool-failure replies now hide raw error details by default. BuilderForce Agents still sends a failure summary, but detailed error suffixes (for example provider/runtime messages and local path fragments) now require `/verbose on` or `/verbose full`.
...
- Gateway/Restart: fix restart-loop edge cases by keeping `builderforce.mjs -> dist/entry.js` bootstrap detection explicit, reacquiring the gateway lock for in-process restart fallback paths, and tightening restart-loop regression coverage. (#23416) Thanks @jeffwnli.
...
- Security/Audit: add `builderforce security audit` detection for open group policies that expose runtime/filesystem tools without sandbox/workspace guards (`security.exposure.open_groups_with_runtime_or_fs`).
...
- Security/Gateway: emit a startup security warning when insecure/dangerous config flags are enabled (including `gateway.controlUi.dangerouslyDisableDeviceAuth=true`) and point operators to `builderforce security audit`.
...
- Telegram/Network: default Node 22+ DNS result ordering to `ipv4first` for Telegram fetch paths and add `BUILDERFORCE_AGENTS_TELEGRAM_DNS_RESULT_ORDER`/`channels.telegram.network.dnsResultOrder` overrides to reduce IPv6-path fetch failures. (#5405) Thanks @Glucksberg.
...
- Plugins/CLI: make `builderforce plugins enable` and plugin install/link flows update allowlists via shared plugin-enable policy so enabled plugins are not left disabled by allowlist mismatch. (#23190) Thanks @downwind7agentNoded-ctrl.
...
- Security/Audit: add `builderforce security audit` finding `gateway.nodes.allow_commands_dangerous` for risky `gateway.nodes.allowCommands` overrides, with severity upgraded to critical on remote gateway exposure.
...
- Security/Config: block prototype-key traversal during config merge patch and legacy migration merge helpers (`__proto__`, `constructor`, `prototype`) to prevent prototype pollution during config mutation flows. (#22968) Thanks @Agentborn.
...
- Security/Discord: add `builderforce security audit` warnings for name/tag-based Discord allowlist entries (DM allowlists, guild/channel `users`, and pairing-store entries), highlighting slug-collision risk while keeping name-based matching supported, and canonicalize resolved Discord allowlist names to IDs at runtime without rewriting config files.
...
- Providers: fix circular chunk dependency in build output that could cause `[builderforce] Failed to start CLI: TypeError: __exportAll is not a function` when the Copilot provider is involved. The GitHub Copilot token module now lazy‑imports JSON‑file helpers.
...
- Branding: complete rename from `builderforce` to `builderforce` across all runtime log prefixes, agent identity defaults, project manifest keys, and package description.

### Fixes

- CLI: fix `[builderforce]` prefix in CLI error messages and process title; the CLI now correctly reports errors as `[builderforce]`. Fix `DEFAULT_CLI_NAME`, update completion and update-command defaults to `builderforce`.
- Agents: fix default identity fallback prefix from `[builderforce]` to `[builderforce]` in agent identity, model selection warnings, session transcript repair, and web auto-reply message prefix.
- Infra: fix `[builderforce]` prefix in unhandled rejection handler, fatal error, and shell env fallback log messages.
- macOS: fix `[builderforce]` prefix in relay and gateway daemon error messages.
- Config: update `PROJECT_NAME` manifest key from `builderforce` to `builderforce`; existing configs using the `builderforce` key remain compatible via `LEGACY_PROJECT_NAMES`.
...
- Gateway/CLI: add paired-device hygiene flows with `device.pair.remove`, plus `builderforce devices remove` and guarded `builderforce devices clear --yes [--pending]` commands for removing paired entries and optionally rejecting pending requests. (#20057) Thanks @mbelinky.
...
- Gateway/Daemon: forward `TMPDIR` into installed service environments so macOS LaunchAgent gateway runs can open SQLite temp/journal files reliably instead of failing with `SQLITE_CANTOPEN`. (#20512) Thanks @Agentborn.
...
- BuilderForce AgentsKit/Protocol: preserve JSON boolean literals (`true`/`false`) when bridging through `AnyCodable` so Apple client RPC params no longer re-encode booleans as `1`/`0`. Thanks @mbelinky.
...
- Browser/Relay: reuse an already-running extension relay when the relay port is occupied by another BuilderForce Agents process, while still failing on non-relay port collisions to avoid masking unrelated listeners. (#20035) Thanks @mbelinky.
...
- iOS/Signing: restore local auto-selected signing-team overrides during iOS project generation by wiring `.local-signing.xcconfig` into the active signing config and emitting `BUILDERFORCE_AGENTS_DEVELOPMENT_TEAM` in local signing setup. (#19993) Thanks @ngutman.
...
- Cron/CLI: add deterministic default stagger for recurring top-of-hour cron schedules (including 6-field seconds cron), auto-migrate existing jobs to persisted `schedule.staggerMs`, and add `builderforce cron add/edit --stagger <duration>` plus `--exact` overrides for per-job timing control.
...
- Docker: add optional `BUILDERFORCE_AGENTS_INSTALL_BROWSER` build arg to preinstall Chromium + Xvfb in the Docker image, avoiding runtime Playwright installs. (#18449)
...
- Gateway/Presence: centralize presence snapshot broadcasts and unify runtime version precedence (`BUILDERFORCE_AGENTS_VERSION` > `BUILDERFORCE_AGENTS_SERVICE_VERSION` > `npm_package_version`) so self-presence and websocket `hello-ok` report consistent versions.
...
- Models CLI: validate catalog entries in `builderforce models set`. (#18129) Thanks @carrotRakko.
...
- Gateway/HTTP: preserve unbracketed IPv6 `Host` headers when normalizing requests. (#18061) Thanks @Agentborn.
...
- BuilderForce AgentsKit/iOS ChatUI: accept canonical session-key completion events for local pending runs and preserve message IDs across history refreshes, preventing stuck "thinking" state and message flicker after gateway replies. (#18165) Thanks @mbelinky.
...
- iOS/Signing: auto-select local Apple Development team during iOS project generation/build, prefer the canonical BuilderForce Agents team when available, and support local per-machine signing overrides without committing team IDs. (#18421) Thanks @ngutman.
...
- Discord: skip auto-thread creation in forum/media/voice/stage channels and keep group session last-route metadata fresh to avoid invalid thread API errors and lost follow-up sends. (#18098) Thanks @Agentborn.
...
- CLI/Pairing: make `builderforce qr --remote` prefer `gateway.remote.url` over tailscale/public URL resolution and register the `builderforce agentbot qr` legacy alias path. (#18091)
- CLI/QR: restore fail-fast validation for `builderforce qr --remote` when neither `gateway.remote.url` nor tailscale `serve`/`funnel` is configured, preventing unusable remote pairing QR flows. (#18166) Thanks @mbelinky.
...
- CLI/Doctor: ensure `builderforce doctor --fix --non-interactive --yes` exits promptly after completion so one-shot automation no longer hangs. (#18502)
- CLI/Doctor: auto-repair `dmPolicy="open"` configs missing wildcard allowlists and write channel-correct repair paths (including `channels.googlechat.dm.allowFrom`) so `builderforce doctor --fix` no longer leaves Google Chat configs invalid after attempted repair. (#18544)
...
- Gateway/Update: prevent restart crash loops after failed self-updates by restarting only on successful updates, stopping early on failed install/build steps, and running `builderforce doctor --fix` during updates to sanitize config. (#18131) Thanks @RamiNoodle733.
...
- CLI/Daemon: warn when a gateway restart sees a stale service token so users can reinstall with `builderforce gateway install --force`, and skip drift warnings for non-gateway service restarts. (#18018)
...
- CLI/Status: fix `builderforce status --all` token summaries for bot-token-only channels so Mattermost/Zalo no longer show a bot+app warning. (#18527) Thanks @echo931.
- CLI/Configure: make the `/model picker` allowlist prompt searchable with tokenized matching in `builderforce configure` so users can filter huge model lists by typing terms like `gpt-5.2 openai/`. (#19010) Thanks @bjesuiter.
- CLI/Message: preserve `--components` JSON payloads in `builderforce message send` so Discord component payloads are no longer dropped. (#18222) Thanks @saurabhchopade.
...
- Models/CLI: sync auth-profiles credentials into agent `auth.json` before registry availability checks so `builderforce models list --all` reports auth correctly for API-key/token providers, normalize provider-id aliases when bridging credentials, and skip expired token mirrors. (#18610, #18615)
...
- Gateway/Auth: trim whitespace around trusted proxy entries before matching so configured proxies with stray spaces still authorize. (#18084) Thanks @Agentborn.
...
- Security/Sessions: create new session transcript JSONL files with user-only (`0o600`) permissions and extend `builderforce security audit --fix` to remediate existing transcript file permissions.
...
- Telegram: add poll sending via `builderforce message poll` (duration seconds, silent delivery, anonymity controls). (#16209) Thanks @robbyczgw-cla.
- Slack/Discord: add `dmPolicy` + `allowFrom` config aliases for DM access control; legacy `dm.policy` + `dm.allowFrom` keys remain supported and `builderforce doctor --fix` can migrate them.
...
- CLI/Plugins: ensure `builderforce message send` exits after successful delivery across plugin-backed channels so one-shot sends do not hang. (#16491) Thanks @yinghaosang.
- CLI/Plugins: run registered plugin `gateway_stop` hooks before `builderforce message` exits (success and failure paths), so plugin-backed channels can clean up one-shot CLI resources. (#16580) Thanks @gumadeiras.
...
- TUI/Gateway: resolve local gateway target URL from `gateway.bind` mode (tailnet/lan) instead of hardcoded localhost so `builderforce tui` connects when gateway is non-loopback. (#16299) Thanks @cortexuvula.
- TUI: honor explicit `--session <key>` in `builderforce tui` even when `session.scope` is `global`, so named sessions no longer collapse into shared global history. (#16575) Thanks @cinqu.
...
- Memory/QMD: make `builderforce memory index` verify and print the active QMD index file path/size, and fail when QMD leaves a missing or zero-byte index artifact after an update. (#16775) Thanks @Shunamxiao.
...
- Media/Security: allow local media reads from BuilderForce Agents state `workspace/` and `sandboxes/` roots by default so generated workspace media can be delivered without unsafe global path bypasses. (#15541) Thanks @lanceji.
...
- Security/Hooks: restrict hook transform modules to `~/.builderforce/hooks/transforms` (prevents path traversal/escape module loads via config). Config note: `hooks.transformsDir` must now be within that directory. Thanks @akhmittra.
...
- Telegram/Security: require numeric Telegram sender IDs for allowlist authorization (reject `@username` principals), auto-resolve `@username` to IDs in `builderforce doctor --fix` (when possible), and warn in `builderforce security audit` when legacy configs contain usernames. Thanks @vincentkoc.
...
- macOS: hard-limit unkeyed `builderforce://agent` deep links and ignore `deliver` / `to` / `channel` unless a valid unattended key is provided. Thanks @Cillian-Collins.
- Scripts/Security: validate GitHub logins and avoid shell invocation in `scripts/update-agenttributors.ts` to prevent command injection via malicious commit records. Thanks @scanleale.
...
- Install: add optional Podman-based setup: `setup-podman.sh` for one-time host setup (builderforce user, image, launch script, systemd quadlet), `run-builderforce-podman.sh launch` / `launch setup`; systemd Quadlet unit for builderforce user service; docs for rootless container, builderforce user (subuid/subgid), and quadlet (troubleshooting). (#16273) Thanks @DarwinsBuddy.
...
- Config/State: removed legacy `.builderforce` auto-detection/migration and `builderforce.json` config candidates. If you still have state/config under `~/.builderforce`, move it to `~/.builderforce` (recommended) or set `BUILDERFORCE_AGENTS_STATE_DIR` / `BUILDERFORCE_AGENTS_CONFIG_PATH` explicitly.

### Fixes

- Gateway/Auth: add trusted-proxy mode hardening follow-ups by keeping `BUILDERFORCE_AGENTS_GATEWAY_*` env compatibility, auto-normalizing invalid setup combinations in interactive `gateway configure` (trusted-proxy forces `bind=lan` and disables Tailscale serve/funnel), and suppressing shared-secret/rate-limit audit findings that do not apply to trusted-proxy deployments. (#15940) Thanks @nickytonline.
...
- OpenAI Codex/Auth: bridge BuilderForce Agents OAuth profiles into `pi` `auth.json` so model discovery and models-list registry resolution can use Codex OAuth credentials. (#15184) Thanks @loiie45e.
...
- CLI/Completion: route plugin-load logs to stderr and write generated completion scripts directly to stdout to avoid `source <(builderforce completion ...)` corruption. (#15481) Thanks @arosstale.
...
- Security/Browser: constrain `POST /trace/stop`, `POST /wait/download`, and `POST /download` output paths to BuilderForce Agents temp roots and reject traversal/escape paths.
...
- Security/Browser: confine `POST /hooks/file-chooser` upload paths to an BuilderForce Agents temp uploads root and reject traversal/escape paths. Thanks @1seal.
...
- Security/Onboarding: clarify multi-user DM isolation remediation with explicit `builderforce config set session.dmScope ...` commands in security audit, doctor security, and channel onboarding guidance. (#13129) Thanks @VintLin.
...
- Config: preserve `${VAR}` env references when writing config files so `builderforce config set/apply/patch` does not persist secrets to disk. Thanks @thewilloftheshadow.
...
- CLI/Plugins: add `builderforce plugins uninstall <id>` with `--dry-run`, `--force`, and `--keep-files` options, including safe uninstall path handling and plugin uninstall docs. (#5985) Thanks @JustasMonkev.
- CLI: add `builderforce logs --local-time` to display log timestamps in local timezone. (#13818) Thanks @xialonglee.
...
- Logging/CLI: use local timezone timestamps for console prefixing, and include `±HH:MM` offsets when using `builderforce logs --local-time` to avoid ambiguity. (#14771) Thanks @0xRaini.
...
- Gateway/Control UI: resolve missing dashboard assets when `builderforce` is installed globally via symlink-based Node managers (nvm/fnm/n/Homebrew). (#14919) Thanks @aynorica.
...
- Logging/Browser: fall back to `os.tmpdir()/builderforce` for default log, browser trace, and browser download temp paths when `/tmp/builderforce` is unavailable.
...
- Heartbeat: filter noise-only system events so scheduled reminder notifications do not fire when cron runs carry only heartbeat markers. (#13317) Thanks @pvtagentNoden.
- Signal: render mention placeholders as `@uuid`/`@phone` so mention gating and BuilderForce Agents targeting work. (#2013) Thanks @alexgleason.
...
- Feishu plugin packaging: remove `workspace:*` `builderforce` dependency from `extensions/feishu` and sync lockfile for install compatibility. (#14423) Thanks @jackcooper2015.
...
- Update/Daemon: fix post-update restart compatibility by generating `dist/cli/daemon-cli.js` with alias-aware exports from hashed daemon bundles, preventing `registerDaemonCli` import failures during `builderforce update`.
...
- Docker: add AgentNodeDock shell helpers for Docker workflows. (#12817) Thanks @Olshansk.
...
- Paths: add `BUILDERFORCE_AGENTS_HOME` for overriding the home directory used by internal path resolution. (#12091) Thanks @sebslight.
...
- CLI: make `builderforce plugins list` output scannable by hoisting source roots and shortening bundled/global/workspace plugin paths.
...
- Paths: structurally resolve `BUILDERFORCE_AGENTS_HOME`-derived home paths and fix Windows drive-letter handling in tool meta shortening. (#12125) Thanks @mcaxtr.
...
- Memory/QMD: run boot refresh in background by default, add configurable QMD maintenance timeouts, retry QMD after fallback failures, and scope QMD queries to BuilderForce Agents-managed collections. (#9690, #9705, #10042) Thanks @vignesh07.
...
- State dir: honor `BUILDERFORCE_AGENTS_STATE_DIR` for default device identity and canvas storage paths. (#4824) Thanks @kossoy.
...
- macOS: honor Nix-managed defaults suite (`ai.builderforce.mac`) for nixMode to prevent onboarding from reappearing after bundle-id churn. (#12205) Thanks @joshp123.
...
- Update: avoid wiping prebuilt Control UI assets during dev auto-builds (`tsdown --no-clean`), run update doctor via `builderforce.mjs`, and auto-restore missing UI assets after doctor. (#10146) Thanks @gumadeiras.
...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (builderforce-cn).
...
- Docs: finish renaming the QMD memory docs to reference the BuilderForce Agents state dir.
...
- Skills: update session-logs paths from .builderforce to .builderforce. (#4502)
...
- Build: align npm tar override and bin metadata so the `builderforce` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update BuilderForce Agents branding + install links.
...
- Rebrand: rename the npm package/CLI to `builderforce`, add a `builderforce` compatibility shim, and move extensions to the `@builderforce/*` scope.
...
- macOS: finish BuilderForce Agents app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Docs: update exe.dev install instructions. (#https://github.com/seanhogg/builderforce/pull/3047) Thanks @zackerthescar.
...
- Skills: update session-logs paths to use ~/.builderforce. (#4502) Thanks @bonald.
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. https://docs.builderforce.ai/providers/ollama https://docs.builderforce.ai/providers/venice
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. https://docs.builderforce.ai/tts
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. https://docs.builderforce.ai/tools/exec-approvals https://docs.builderforce.ai/tools/slash-commands
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. https://docs.builderforce.ai/channels/telegram

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. https://docs.builderforce.ai/tts
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. https://docs.builderforce.ai/tts
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. https://docs.builderforce.ai/channels/telegram
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. https://docs.builderforce.ai/tools/web
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. https://docs.builderforce.ai/tools/exec-approvals https://docs.builderforce.ai/tools/slash-commands
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @Glucksberg.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). https://docs.builderforce.ai/diagnostics/flags
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. https://docs.builderforce.ai/bedrock
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. https://docs.builderforce.ai/channels/bluebubbles
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) https://docs.builderforce.ai/channels/signal
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. https://docs.builderforce.ai/tts
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. https://docs.builderforce.ai/gateway/tools-invoke-http-api
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. https://docs.builderforce.ai/gateway/heartbeat
- Deploy: add Fly.io deployment support + guide. (#1570) https://docs.builderforce.ai/platforms/fly
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. https://docs.builderforce.ai/channels/tlon

### Changes

- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. https://docs.builderforce.ai/multi-agent-sandbox-tools
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. https://docs.builderforce.ai/bedrock
- CLI: add `builderforce system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.builderforce.ai/cli/system
- CLI: add live auth probes to `builderforce models status` for per-profile verification. (commit 40181afde) https://docs.builderforce.ai/cli/models
- CLI: restart the gateway by default after `builderforce update`; add `--no-restart` to skip it. (commit 2c85b1b40)
- Browser: add node-host proxy auto-routing for remote gateways (configurable per gateway/node). (commit c3cb26f7c)
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. https://docs.builderforce.ai/tools/llm-task
...
- Docs: add cron vs heartbeat decision guide (with Lobster workflow notes). (#1533) Thanks @JustYannicc. https://docs.builderforce.ai/automation/cron-vs-heartbeat
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. https://docs.builderforce.ai/gateway/heartbeat
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `builderforce models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Doctor: honor BUILDERFORCE_AGENTS_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. https://docs.builderforce.ai/cli/agents https://docs.builderforce.ai/web/control-ui
...
- Highlight: Lobster optional plugin tool for typed workflows + approval gates. https://docs.builderforce.ai/tools/lobster
- Lobster: allow workflow file args via `argsJson` in the plugin tool. https://docs.builderforce.ai/tools/lobster
...
- CLI: flatten node service commands under `builderforce node` and remove `service node` docs.
- CLI: move gateway service commands under `builderforce gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `builderforce update wizard` for interactive channel selection and restart prompts. https://docs.builderforce.ai/cli/update
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. https://docs.builderforce.ai/gateway/troubleshooting
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking

- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. https://docs.builderforce.ai/web/control-ui#insecure-http
...
- Control UI: add copy-as-markdown with error feedback. (#1345) https://docs.builderforce.ai/web/control-ui
- Control UI: drop the legacy list view. (#1345) https://docs.builderforce.ai/web/control-ui
- TUI: add syntax highlighting for code blocks. (#1200) https://docs.builderforce.ai/tui
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) https://docs.builderforce.ai/tui
- TUI: add a searchable model picker for quicker model selection. (#1198) https://docs.builderforce.ai/tui
- TUI: add input history (up/down) for submitted messages. (#1348) https://docs.builderforce.ai/tui
- ACP: add `builderforce acp` for IDE integrations. https://docs.builderforce.ai/cli/acp
- ACP: add `builderforce acp client` interactive harness for debugging. https://docs.builderforce.ai/cli/acp
- Skills: add download installs with OS-filtered options. https://docs.builderforce.ai/tools/skills
- Skills: add the local sherpa-onnx-tts skill. https://docs.builderforce.ai/tools/skills
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. https://docs.builderforce.ai/concepts/memory
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. https://docs.builderforce.ai/concepts/memory
- Memory: add OpenAI batch indexing for embeddings when configured. https://docs.builderforce.ai/concepts/memory
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. https://docs.builderforce.ai/concepts/memory
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). https://docs.builderforce.ai/concepts/memory
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. https://docs.builderforce.ai/concepts/memory
- Memory: add `--verbose` logging for memory status + batch indexing details. https://docs.builderforce.ai/concepts/memory
- Memory: add native Gemini embeddings provider for memory search. (#1151) https://docs.builderforce.ai/concepts/memory
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) https://docs.builderforce.ai/tools/browser
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) https://docs.builderforce.ai/channels/nostr
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) https://docs.builderforce.ai/channels/matrix
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) https://docs.builderforce.ai/channels/slack
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) https://docs.builderforce.ai/channels/telegram
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) https://docs.builderforce.ai/plugins/zalouser
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) https://docs.builderforce.ai/plugins/manifest
- Plugins: move channel catalog metadata into plugin manifests. (#1290) https://docs.builderforce.ai/plugins/manifest
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) https://docs.builderforce.ai/plugins/manifest
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) https://docs.builderforce.ai/web/control-ui
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) https://docs.builderforce.ai/gateway/configuration
- Plugins: add plugin slots with a dedicated memory slot selector. https://docs.builderforce.ai/plugins/agent-tools
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). https://docs.builderforce.ai/channels/bluebubbles
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. https://docs.builderforce.ai/channels/zalo
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. https://docs.builderforce.ai/plugins/zalouser
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. https://docs.builderforce.ai/plugins/agent-tools
- Plugins: auto-enable bundled channel/provider plugins when configuration is present.
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `builderforce update`.
- Plugins: share npm plugin update logic between `builderforce update` and `builderforce plugins update`.
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. https://docs.builderforce.ai/reference/api-usage-costs
- Security: warn when <=300B models run without sandboxing while web tools are enabled. https://docs.builderforce.ai/cli/security
- Exec: add host/security/ask routing for gateway + node exec. https://docs.builderforce.ai/tools/exec
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). https://docs.builderforce.ai/tools/exec
- Exec approvals: migrate approvals to `~/.builderforce/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.builderforce.ai/tools/exec-approvals
- Nodes: add headless node host (`builderforce node start`) for `system.run`/`system.which`. https://docs.builderforce.ai/cli/node
- Nodes: add node daemon service install/status/start/stop/restart. https://docs.builderforce.ai/cli/node
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) https://docs.builderforce.ai/concepts/session
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. https://docs.builderforce.ai/tools/subagents
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. https://docs.builderforce.ai/concepts/groups
- Models: add Qwen Portal OAuth provider support. (#1120) https://docs.builderforce.ai/providers/qwen
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. https://docs.builderforce.ai/start/onboarding
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. https://docs.builderforce.ai/start/onboarding
- Docs: refresh Android node discovery docs for the Gateway WS service type. https://docs.builderforce.ai/platforms/android
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) https://docs.builderforce.ai/bedrock
- Docs: clarify WhatsApp voice notes. https://docs.builderforce.ai/channels/whatsapp
- Docs: clarify Windows WSL portproxy LAN access notes. https://docs.builderforce.ai/platforms/windows
- Docs: refresh bird skill install metadata and usage notes. (#1302) https://docs.builderforce.ai/tools/browser-login
...
- Build: use tsgo for dev/watch builds by default (opt out with `BUILDERFORCE_AGENTS_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `builderforce doctor --fix` to repair, then update plugins (`builderforce plugins update`) if you use any.

### Fixes

- Discovery: shorten Bonjour DNS-SD service type to `_builderforce-gw._tcp` and update discovery clients/docs.
...
- CLI: keep `builderforce logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `builderforce memory status`.
...
- Memory: probe sqlite-vec availability in `builderforce memory status`.
...
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) — thanks @ThomsenDrake. https://docs.builderforce.ai/hooks
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks. https://docs.builderforce.ai/nodes/media-understanding
- Plugins: add Zalo Personal plugin (`@builderforce/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.builderforce.ai/plugins/zalouser
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) — thanks @timolins. https://docs.builderforce.ai/providers/vercel-ai-gateway
- Sessions: add `session.identityLinks` for cross-platform DM session li nking. (#1033) — thanks @thewilloftheshadow. https://docs.builderforce.ai/concepts/session
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) — thanks @YuriNachos. https://docs.builderforce.ai/tools/web

### Breaking

- **BREAKING:** `builderforce message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `builderforce hooks` is now `builderforce webhooks`; hooks live under `builderforce hooks`. https://docs.builderforce.ai/cli/webhooks
- **BREAKING:** `builderforce plugins install <path>` now copies into `~/.builderforce/extensions` (use `--link` to keep path-based loading).
...
- Tools: add `exec` PTY support for interactive sessions. https://docs.builderforce.ai/tools/exec
...
- Directory: unify `builderforce directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `builderforce.hooks` manifests and `builderforce hooks install/update`.
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental “open” behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/…` tokens correctly. https://docs.builderforce.ai/gateway/security
- Security: redact sensitive text in gateway WS logs.
- Tools: cap pending `exec` process output to avoid unbounded buffers.
- CLI: speed up `builderforce sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `builderforce models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@builderforce/msteams` via `builderforce plugins install @builderforce/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `builderforce-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `builderforce security audit`.
- Apps: store node auth tokens encrypted (Keychain/SecurePrefs).
- Daemon: share profile/state-dir resolution across service helpers and honor `BUILDERFORCE_AGENTS_STATE_DIR` for Windows task scripts.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `builderforce models status`, and update docs.
- CLI: add `--json` output for `builderforce daemon` lifecycle/install commands.
...
- Fix: make `builderforce update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `builderforce configure`, even if no other sections are chosen.
...
- Security: expanded `builderforce security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `builderforce security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `builderforce browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.builderforce-dev`).
...
- Memory: new `builderforce memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.builderforce/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `builderforce dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `builderforce doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Onboarding/Auth: honor `BUILDERFORCE_AGENTS_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) — thanks @roshanasingh4.
...
- Gateway/CLI: honor `BUILDERFORCE_AGENTS_LAUNCHD_LABEL` / `BUILDERFORCE_AGENTS_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `builderforce doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `builderforce plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `builderforce` CLI when missing in local mode; install via `builderforce.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch builderforce.ai scripts and run install sh/cli with onboarding skipped.
- Installer UX: support `BUILDERFORCE_AGENTS_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `builderforce.extensions`; load plugin packages from config dirs; extract archives without system tar.
- Config: expand `~` in `BUILDERFORCE_AGENTS_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) — thanks @pasogott.
...
- CLI: `builderforce status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `builderforce update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `builderforce reset` and `builderforce uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `builderforce gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `builderforce status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `builderforce sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: respect `BUILDERFORCE_AGENTS_STATE_DIR` for node pairing + voice wake settings storage. (#664) — thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`BUILDERFORCE_AGENTS_LIVE_GATEWAY_IMAGE_PROBE=1`).
- CLI: `builderforce sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “BuilderForce Agents” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `builderforce update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/builderforce`. (#660)
...
- CLI: `builderforce message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `builderforce models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.builderforce/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- Gateway/Daemon/Doctor: atomic config writes; repair gateway service entrypoint + install switches; non-interactive legacy migrations; systemd unit alignment + KillMode=process; node bridge keepalive/pings; Launch at Login persistence; bundle BuilderForce AgentsKit resources + Swift 6.2 compat dylib; relay version check + remove smoke test; regen Swift GatewayModels + keep agent provider string; cron jobId alias + channel alias migration + main session key normalization; heartbeat Telegram accountId resolution; avoid WhatsApp fallback for internal runs; gateway listener error wording; serveBaseUrl param; honor gateway --dev; fix wide-area discovery updates; align agents.defaults schema; provider account metadata in daemon status; refresh Carbon patch for gateway fixes; restore doctor prompter initialValue handling.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `builderforce pairing list <provider>` + `builderforce pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `builderforce logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- **Docs:** new FAQ/AgentHub/config examples/showcase entries and clarified auth, sandbox, and systemd docs.
...
- Env: load global `$BUILDERFORCE_AGENTS_STATE_DIR/.env` (`~/.builderforce/.env`) as a fallback after CWD `.env`.
...
- Commands: unify /status (inline) and command auth across providers; group bypass for authorized control commands; remove Discord /builderforce slash handler.
- CLI: run `builderforce agent` via the Gateway by default; use `--local` to force embedded mode.
