...
import {
  ...
  isWSL2Sync,
  type BuilderForceAgentsPluginApi,
  type ProviderAuthContext,
} from "@seanhogg/builderforce-agents/plugin-sdk";

const CLIENT_ID = process.env.BUILDERFORCE_AGENTS_GOOGLE_OAUTH_CLIENT_ID?.trim() ?? "";
const CLIENT_SECRET = process.env.BUILDERFORCE_AGENTS_GOOGLE_OAUTH_CLIENT_SECRET?.trim() ?? "";
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>BuilderForceAgents Antigravity OAuth</title>
  </head>
  <body>
    <main>
...
function buildAuthUrl(params: { challenge: string; state: string }): string {
  if (!CLIENT_ID) {
    throw new Error("Missing BUILDERFORCE_AGENTS_GOOGLE_OAUTH_CLIENT_ID");
  }
...
async function exchangeCode(params: {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) {
    throw new Error(
      "Missing Google OAuth credentials. Set BUILDERFORCE_AGENTS_GOOGLE_OAUTH_CLIENT_ID and BUILDERFORCE_AGENTS_GOOGLE_OAUTH_CLIENT_SECRET.",
    );
  }
