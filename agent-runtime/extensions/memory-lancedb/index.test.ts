...
const liveEnabled = HAS_OPENAI_KEY && process.env.BUILDERFORCE_AGENTS_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
