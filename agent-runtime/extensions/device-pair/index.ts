...
import { approveDevicePairing, listDevicePairing } from "@seanhogg/builderforce-agents/plugin-sdk";
...
function resolveGatewayPort(cfg: BuilderForceAgentsPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.BUILDERFORCE_AGENTS_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveScheme(
  cfg: BuilderForceAgentsPluginApi["config"],
...
async function resolveTailnetHost(api: BuilderForceAgentsPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: BuilderForceAgentsPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: BuilderForceAgentsPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the BuilderForceAgents iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the BuilderForceAgents iOS app:",
            "",
            "```",
            qrAscii,
            "```",
