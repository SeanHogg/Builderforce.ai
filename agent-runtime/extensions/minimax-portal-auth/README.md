# MiniMax OAuth (BuilderForceAgents plugin)
...
```bash
builderforce plugins enable minimax-portal-auth
```
...
```bash
builderforce models auth login --provider minimax-portal --set-default
```
