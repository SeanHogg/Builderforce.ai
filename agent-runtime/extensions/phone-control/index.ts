...
import type { BuilderForceAgentsPluginApi, BuilderForceAgentsPluginService } from "@seanhogg/builderforce-agents/plugin-sdk";
...
function normalizeDenyList(cfg: BuilderForceAgentsPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: BuilderForceAgentsPluginApi["config"]): string[] {
...
): BuilderForceAgentsPluginApi["config"] {
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
...
export default function register(api: BuilderForceAgentsPluginApi) {
  ...
  const timerService: BuilderForceAgentsPluginService = {
