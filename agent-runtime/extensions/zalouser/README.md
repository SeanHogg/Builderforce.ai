...
BuilderForceAgents extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
builderforce message send --channel zalouser --target <threadId> --message "Hello from BuilderForceAgents!"
```
...
```bash
builderforce channels login --channel zalouser              # Login via QR
builderforce channels login --channel zalouser --account work
...
builderforce directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
builderforce message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
builderforce message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `builderforce channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
builderforce channels login --channel zalouser --account work
builderforce message send --channel zalouser --account work --target <id> --message "Hello"
