import type { ChannelAccountSnapshot, ChannelStatusIssue } from "@seanhogg/builderforce-agents/plugin-sdk";
    ...
    if (!configured) {
      if (isMissingZca(lastError)) {
        ...
        issues.push({
          ...
          fix: "Run: builderforce channels login --channel zalouser",
        });
      }
      continue;
    }
