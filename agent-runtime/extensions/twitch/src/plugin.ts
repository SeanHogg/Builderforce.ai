/**
 * Twitch channel plugin for BuilderForceAgents.
 *
...
import { buildChannelConfigSchema } from "@seanhogg/builderforce-agents/plugin-sdk";
 ...
 * for BuilderForceAgents. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: BuilderForceAgentsConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: BuilderForceAgentsConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: BuilderForceAgentsConfig): boolean => {
