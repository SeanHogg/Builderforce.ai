...
import type { ReplyPayload, BuilderForceAgentsConfig } from "@seanhogg/builderforce-agents/plugin-sdk";
import { createReplyPrefixOptions } from "@seanhogg/builderforce-agents/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // BuilderForceAgentsConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as BuilderForceAgentsConfig;
