...
function resolveBuilderForceAgentsRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.BUILDERFORCE_AGENTS_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["builderforce"]) {
  ...
  throw new Error("Unable to resolve core root. Set BUILDERFORCE_AGENTS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveBuilderForceAgentsRoot(), "dist", "extensionAPI.js");
