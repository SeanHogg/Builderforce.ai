...
Official Voice Call plugin for **BuilderForceAgents**.
...
Docs: `https://docs.builderforce.ai/plugins/voice-call`
Plugin system: `https://docs.builderforce.ai/plugin`

## Install (local dev)

### Option A: install via BuilderForceAgents (recommended)
...
```bash
mkdir -p ~/.builderforce/extensions
cp -R extensions/voice-call ~/.builderforce/extensions/voice-call
cd ~/.builderforce/extensions/voice-call && pnpm install
```
...
```bash
builderforce voicecall call --to "+15555550123" --message "Hello from BuilderForceAgents"
