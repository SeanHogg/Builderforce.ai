...
export async function handleNextcloudTalkInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as BuilderForceAgentsConfig,
    surface: CHANNEL_ID,
  });
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as BuilderForceAgentsConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as BuilderForceAgentsConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(
    config as BuilderForceAgentsConfig,
  );
