...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "builderforce-nostr-"));
  ...
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        const override =
          stateEnv.BUILDERFORCE_AGENTS_STATE_DIR?.trim() || stateEnv.BUILDERFORCE_AGENTS_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".builderforce");
      },
    },
  } as PluginRuntime);
  try {
