...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["builderforcePollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["builderforce", "pollId"]) ??
    readNestedString(value, ["builderforce", "poll", "id"]) ??
    readNestedString(value, ["data", "builderforcePollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "builderforce", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          builderforcePollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "builderforce poll vote",
          displayText: "Vote recorded",
          value: { builderforcePollId: pollId, pollId },
        },
      },
    ],
  };
