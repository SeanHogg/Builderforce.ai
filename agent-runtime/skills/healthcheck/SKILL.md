---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for BuilderForceAgents deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, BuilderForceAgents cron scheduling for periodic checks, or version status checks on a machine running BuilderForceAgents (laptop, workstation, Pi, VPS).
author: builderForceAgents
author-url: https://builderforce.ai
---

# BuilderForceAgents Host Hardening

## Overview

Assess and harden the host running BuilderForceAgents, then align it to a user-defined risk tolerance without breaking access. Use BuilderForceAgents security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim BuilderForceAgents changes the host firewall, SSH, or OS updates; it does not.
...
5. BuilderForceAgents gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if BuilderForceAgents can access sensitive data.
...
### 2) Run BuilderForceAgents security audits (read-only)

As part of the default read-only checks, run `builderforce security audit --deep`. Only offer alternatives if the user requests them:

1. `builderforce security audit` (faster, non-probing)
2. `builderforce security audit --json` (structured output)

Offer to apply BuilderForceAgents safe defaults (numbered):

1. `builderforce security audit --fix`

Be explicit that `--fix` only tightens BuilderForceAgents defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check BuilderForceAgents version/update status (read-only)

As part of the default read-only checks, run `builderforce update status`.
...
- Credential hygiene notes (location of BuilderForceAgents creds, prefer disk encryption)
...
- BuilderForceAgents security audit (re-run)
...
After BuilderForceAgents install or first hardening pass, run at least one baseline audit and version check:

- `builderforce security audit`
- `builderforce security audit --deep`
- `builderforce update status`

Ongoing monitoring is recommended. Use the BuilderForceAgents cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `builderforce cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `builderforce update status`
...
Before creating, `builderforce cron list` and match on exact `name`. If found, `builderforce cron edit <id> ...`.
If not found, `builderforce cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `builderforce update status` (preferred for source checkouts and channels)
2. `npm view builderforce version` (published npm version)

## BuilderForceAgents command accuracy

Use only supported commands and flags:

- `builderforce security audit [--deep] [--fix] [--json]`
- `builderforce status` / `builderforce status --deep`
- `builderforce health --json`
- `builderforce update status`
- `builderforce cron add|list|runs|run`

Do not invent CLI flags or imply BuilderForceAgents enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by BuilderForceAgents compaction:
