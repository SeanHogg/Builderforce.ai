...
export class CreationCanvasPanel extends WebviewPanelBase<CanvasInbound> {
  ...
  protected renderHtml(_webview: vscode.Webview): string {
    ...
    </style></head><body><div class="bar"><strong id="title">${h(text.creationSession)}</strong><span id="presence"></span><button id="file">${h(text.file)}</button><button id="selection">${h(text.selection)}</button><button id="diagnostics">${h(text.problems)}</button><button id="repo">${h(text.repository)}</button><button id="terminal">${h(text.terminal)}</button><button id="preview">${h(text.preview)}</button><button id="refresh">${h(text.refresh)}</button><button id="web">${h(text.web)}</button></div><aside class="palette"><b>${h(text.addToCanvas)}</b>${paletteHtml}</aside><aside class="comments"><b>${h(text.comments)}</b><small id="comment-scope"> · ${h(text.session)}</small><ul></ul><form><input placeholder="${h(text.commentPlaceholder)}"><button>${h(text.add)}</button></form></aside><div id="canvas"><div class="world"><svg class="edges" width="3000" height="2000"></svg><div id="objects"></div><div class="empty">${h(text.loading)}</div></div></div><form class="composer"><input placeholder="${h(text.promptPlaceholder)}"><button>${h(text.send)}</button></form><script nonce="${nonce}">
