-- 0006 — attribution + correlation on the OPERATIONAL copy of `llm_failover_log`.
--
...
-- api/migrations/0946_llm_failover_attribution.sql. Both copies of this table are
-- live (the writer falls back to the primary database when the operational secret
-- is unbound), so a column added to only one of them is absent exactly where the
-- rows land. This is the track the rows actually land on in a deployment that
-- binds NEON_TRANSACTIONAL_DATABASE_URL — i.e. production.
