...
// =========================================================================
// The root entity. `DOMAIN_MANIFEST` names `connection` as this seat’s rootKind
// and these three tables ARE it; they were declared in `platform.ts` while this
// module held only the satellites (drive, mailbox, extension, LTI). Moved here
// so the seat can be reviewed on its own — and so `growth.ts`, which reached into
// `platform.ts` for exactly one table, stops crossing that boundary at all.
//
// `created_by_user_id` travelled as a plain id: identity’s `users` is not this
// domain’s to import (§3), and the FK is declared in migration 0410.
// =========================================================================

// ---------------------------------------------------------------------------
// Connector platform (migration 0410)
// ---------------------------------------------------------------------------

/**
 * A TENANT-AUTHORED connector definition.
 *
 * Built-in connectors are NOT rows here — they ship as code in
 * `application/connectors/defaults/` so a fix reaches every tenant with the
 * deploy instead of needing a data migration. This table holds only what a
 * customer authored (in the builder, or by importing an OpenAPI spec), and the
 * registry merges the two.
 *
 * `manifest` is the validated {@link ConnectorManifest} — parsed through
 * `parseConnectorManifest` on every write, never trusted on read.
 */
export const connectors = pgTable('connectors', {
  id:            uuid('id').primaryKey().defaultRandom(),
  tenantId:      integer('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  /** Catalog key, unique within the tenant and never a built-in key. */
  connectorKey:  varchar('connector_key', { length: 64 }).notNull(),
  /** Denormalised from the manifest so the catalog list needn't parse JSON per row. */
  name:          varchar('name', { length: 255 }).notNull(),
  category:      varchar('category', { length: 32 }).notNull().default('other'),
  icon:          varchar('icon', { length: 16 }).notNull().default('🔌'),
  /** 'draft' — editable, not advertised to agents. 'published' — advertised. */
  status:        varchar('status', { length: 16 }).notNull().default('draft'),
  manifest:      jsonb('manifest').notNull(),
  /** Bumped on every manifest write; lets a connection pin what it was built against. */
  version:       integer('version').notNull().default(1),
  createdByUserId: varchar('created_by_user_id', { length: 36 }),
  createdAt:     timestamp('created_at').notNull().defaultNow(),
  updatedAt:     timestamp('updated_at').notNull().defaultNow(),
}, (t) => [
  unique('uq_connectors_tenant_key').on(t.tenantId, t.connectorKey),
  index('idx_connectors_tenant_status').on(t.tenantId, t.status),
]);

/**
 * One configured, credentialed instance of a connector — "our production Slack".
 *
 * Points at a connector by KEY (not by row id) because the key resolves against
 * built-ins and tenant connectors alike; that is what lets a tenant connect Slack
 * without a `connectors` row existing at all.
 */
export const connectorConnections = pgTable('connector_connections', {
  id:            uuid('id').primaryKey().defaultRandom(),
  tenantId:      integer('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  /** Built-in key ('slack') or a tenant connector's key. */
  connectorKey:  varchar('connector_key', { length: 64 }).notNull(),
  /** Display label — distinguishes two connections to the same system. */
  name:          varchar('name', { length: 255 }).notNull(),
  /** AES-GCM per-tenant encrypted JSON of the manifest's auth fields. */
  credentialsEnc: text('credentials_enc').notNull(),
  iv:            varchar('iv', { length: 64 }).notNull(),
  /** Overrides the manifest baseUrl (self-hosted GitLab, a sandbox tenant). */
  baseUrlOverride: varchar('base_url_override', { length: 500 }),
  ...
  unique('uq_connector_connections_tenant_key_name').on(t.tenantId, t.connectorKey, t.name),
  index('idx_connector_connections_tenant').on(t.tenantId, t.enabled),
]);

/**
 * One row per outbound connector call — the audit trail for "what did the agent
 * actually do in our Salesforce?".
 *
 * Deliberately records the request SHAPE (action, status, duration) and never the
 * request body: a connector body routinely carries customer PII and the auth
 * values are secret. `error` holds the upstream message, truncated.
 */
export const connectorCallLogs = pgTable('connector_call_logs', {
  id:           bigserial('id', { mode: 'number' }).primaryKey(),
  tenantId:     integer('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  connectionId: uuid('connection_id').references(() => connectorConnections.id, { onDelete: 'cascade' }),
  connectorKey: varchar('connector_key', { length: 64 }).notNull(),
  ...
  error:        text('error'),
  /** Who triggered it — 'agent', 'user', or a test run from the builder. */
  actorKind:    varchar('actor_kind', { length: 16 }).notNull().default('agent'),
  createdAt:    timestamp('created_at').notNull().defaultNow(),
}, (t) => [
  index('idx_connector_call_logs_tenant_time').on(t.tenantId, t.createdAt),
  index('idx_connector_call_logs_connection').on(t.connectionId, t.createdAt),
]);
...
export const mailboxConnections = pgTable('mailbox_connections', {
   ...
   *  silently re-arms sending. See migrations 0414 and 1182. */
