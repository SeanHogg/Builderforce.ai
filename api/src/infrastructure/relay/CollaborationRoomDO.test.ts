...
function makeMockState(): DurableObjectState & { sockets: MockWebSocket[]; stored: Map<string, unknown> } {
  ...
  return {
    ...
    id: { toString: () => 'test-room' } as unknown as DurableObjectId,
    waitUntil: vi.fn(),
...
describe('CollaborationRoomDO construction', () => {
  it('creates an instance without throwing', () => {
    expect(() => new CollaborationRoomDO(makeMockState())).not.toThrow();
  });
...
describe('the JSON protocol the terminal and presence surfaces use', () => {
  ...
  beforeEach(() => {
    state = makeMockState();
    room = new CollaborationRoomDO(state);
