export type AgentHostMachineProfileInput = {
  machineName?: string;
  machineIp?: string;
...
export function normalizeMachineProfile(value: unknown): AgentHostMachineProfileInput | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return null;
  }
  const raw = value as Record<string, unknown>;
  const profile: AgentHostMachineProfileInput = {};
