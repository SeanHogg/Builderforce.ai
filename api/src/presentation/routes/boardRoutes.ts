...
export function createBoardRoutes(db: Db): Hono<HonoEnv> {
  ...
  router.post('/', async (c) => {
    ...
    //
    // The Neon HTTP driver has no transaction support — findOrCreateBoard uses a
    // compensating delete (roll the board back if the lane seed fails) so the
    // board-with-default-lanes invariant holds without explicit DB transactions.
