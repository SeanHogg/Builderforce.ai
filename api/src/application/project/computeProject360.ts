...
import { agentHosts, executions, ideAgents, tasks, users } from '../../infrastructure/database/schema';
 ...
 * round-trip so every surface (the VS Code native panel today; the web app later)
 * renders the SAME numbers instead of each re-deriving a score that drifts.
 *
 ...
 * estimation, and the non-terminal executions that tell us who is actually working
 * right now vs idle (and why).
 *
...
/** Action a gap button performs — mapped to a VS Code command by the panel host.
 *  `board`/`approvals` are argless; `brain` seeds the shared Brain with a task the
 *  agent can actually do; `run-task`/`open-task` carry a task descriptor. */
export interface Project360Action {
  kind: 'board' | 'approvals' | 'brain' | 'run-task' | 'open-task';
  label: string;
  /** Seed text for `brain`. */
  text?: string;
  /** Task descriptor for `run-task` / `open-task`. */
...
export interface Project360 {
  ...
  /** Flattened, severity-sorted missing items — the "improve health" checklist. */
  gaps: Project360Gap[];
  workforce: Project360Member[];
  generatedAt: string;
}

/** The task-status/goal aggregates the projects-list builder already computed for
 *  this project. Passed in so we don't re-count the same rows (DRY + one fewer query). */
...
/** Canonical done/terminal status sets (kept in sync with the projects-list builder). */
...
function ownerOf(t: {
  ...
  // Precedence human > host > cloud — the same identity resolution the workforce
  // metrics use, so "who owns this" never disagrees between surfaces.
...
export async function computeProject360(
  ...
  // Non-terminal executions on this project's tasks = who/what is live right now.
  // `paused` = a run waiting on a human (ask_human / approval) — awaiting, not working.
  ...
  // Per-task classifications the aggregates don't carry.
  ...
  // "Assignable" open work = open, not blocked (a blocked task not being assigned
  // isn't the staffing problem — the block is).
  ...
  for (const e of activeRows) {
    ...
    // A running exec beats a paused one if a task somehow has both.
  ...
  push(
    ...
    hasData && completed === 0
      ? [gap('progress', 'medium', 'Nothing completed yet', boardAction('Open board'))]
      : [],
  );
  ...
  push(
    ...
    overdue > 0
      ? [
          gap(
            'timeliness',
            overdue >= 3 ? 'high' : 'medium',
            `${overdue} task${overdue === 1 ? '' : 's'} overdue`,
            brainAction(
              'Re-plan with Brain',
              `${overdue} tasks in the "${pj}" project are overdue. Review each overdue task, then either update its due date to something realistic or break it into smaller tasks. Summarise the new plan.`,
            ),
          ),
        ]
      : [],
  ...
  push(
    ...
    blocked > 0
      ? [
          gap(
            'flow',
            blocked >= 2 ? 'high' : 'medium',
            `${blocked} task${blocked === 1 ? '' : 's'} blocked`,
            brainAction(
              'Unblock with Brain',
              `${blocked} tasks in the "${pj}" project are blocked. Investigate each blocked task, identify what's blocking it, and propose a concrete way to unblock it. Where you can act, do it.`,
            ),
          ),
        ]
      : [],
    ...
    open > 0 && inProgressTasks.length === 0 && activeRuns === 0
      ? [
          gap(
            ...
            brainAction(
              'Start next task',
              `The "${pj}" project has open work but nothing in progress. Pick the highest-priority ready task, explain why, and start it.`,
            ),
          ),
        ]
      : [],
  ...
  if (!hasGoals)
    directionGaps.push(
      gap(
        'direction',
        'high',
        'No goal or OKR linked',
        brainAction(
          'Define objectives',
          `Help me define 1–3 clear objectives / OKRs for the "${pj}" project and link the key tasks to them so the work has a measurable direction.`,
        ),
      ),
    );
  if (!agg.hasArchitecturePrd)
    directionGaps.push(
      gap(
        ...
        brainAction(
          'Draft architecture',
          `Analyse the "${pj}" project's codebase and produce an architecture PRD: key components, data flow, and the main technical decisions.`,
        ),
      ),
    );
  push(
    ...
    'planning',
    directionScore,
    hasGoals ? (agg.hasArchitecturePrd ? 'Goals + architecture set' : 'Goals set') : 'Direction undefined',
    directionGaps,
  );
  ...
  push(
    ...
    notStarted.length === 0
      ? 'Backlog clear'
      : `${notStarted.length - unestimated.length}/${notStarted.length} backlog tasks estimated`,
    unestimated.length > 0
      ? [
          gap(
            'readiness',
            unestimated.length >= 5 ? 'high' : 'medium',
            `${unestimated.length} backlog task${unestimated.length === 1 ? '' : 's'} unestimated`,
            brainAction(
              'Groom backlog',
              `Review the un-estimated backlog tasks in the "${pj}" project. Add a story-point estimate to each, and mark the well-defined ones as ready.`,
            ),
          ),
        ]
      : [],
  ...
  push(
    ...
    assignable.length === 0
      ? 'No open work'
      : `${assignable.length - unassigned.length}/${assignable.length} open tasks owned`,
    unassigned.length > 0
      ? [
          gap(
            'staffing',
            unassigned.length >= 3 ? 'high' : 'medium',
            `${unassigned.length} open task${unassigned.length === 1 ? '' : 's'} unassigned`,
            brainAction(
              'Assign owners',
              `Assign an owner to each unassigned open task in the "${pj}" project, matching the work to the right teammate or agent by skill and availability.`,
            ),
          ),
        ]
      : [],
  ...
  if (thin.length > 0)
    defGaps.push(
      gap(
        'definition',
        thin.length >= 4 ? 'medium' : 'low',
        `${thin.length} task${thin.length === 1 ? '' : 's'} lack detail`,
        brainAction(
          'Flesh out tasks',
          `Several open tasks in the "${pj}" project have thin or missing descriptions. Flesh each out with scope and acceptance criteria so an agent can execute it unattended.`,
        ),
      ),
    );
  if (unownedInProgress.length > 0)
    defGaps.push(
      gap(
        'definition',
        'medium',
        `${unownedInProgress.length} in-progress task${unownedInProgress.length === 1 ? '' : 's'} have no owner`,
        boardAction('Review on board'),
      ),
    );
  push(
    'definition',
    ...
    'team',
    definition,
    thin.length === 0 ? 'Tasks well-defined' : `${thin.length} thin`,
    defGaps,
  );
  ...
  // Overall — weighted so shipping (progress) and unblocked flow count for more.
  ...
  // ---- Workforce (who's working / idle and why) -------------------------------
  const severityRank: Record<Project360Gap['severity'], number> = { high: 3, medium: 2, low: 1 };
  const allGaps = dims
    .flatMap((d) => d.gaps)
    .sort((a, b) => severityRank[b.severity] - severityRank[a.severity]);

  // Resolve names for every owner + assigned host in one pass (no N+1).
  ...
  const [userRows, hostRows, cloudRows] = await Promise.all([
    userIds.size
      ? db.select({ id: users.id, displayName: users.displayName, username: users.username, email: users.email })
  ...
  // Aggregate the best status per owner across their open tasks.
  const rank: Record<Project360Member['status'], number> = {
    working: 5, awaiting: 4, blocked: 3, idle: 2, available: 1,
  };
  ...
  const consider = (o: Owner, m: Omit<Project360Member, 'ref' | 'kind' | 'name'>) => {
    ...
    const next: Project360Member = { ref: o.ref, kind: o.kind, name: nameOf(o), ...m };
  ...
  for (const t of openTasks) {
    ...
    if (live && !live.paused) consider(o, { status: 'working', reason: `Running ${t.key ?? t.title}`, taskId: t.id, taskKey: t.key, taskTitle: t.title });
    else if (live && live.paused) consider(o, { status: 'awaiting', reason: `Awaiting input on ${t.key ?? t.title}`, taskId: t.id, taskKey: t.key, taskTitle: t.title });
    else if (t.status === 'blocked') consider(o, { status: 'blocked', reason: `Blocked on ${t.key ?? t.title}`, taskId: t.id, taskKey: t.key, taskTitle: t.title });
    else if (t.status === 'in_progress') consider(o, { status: 'idle', reason: `In progress, no active run: ${t.key ?? t.title}`, taskId: t.id, taskKey: t.key, taskTitle: t.title });
    else consider(o, { status: 'idle', reason: `Assigned ${t.key ?? t.title}, not started`, taskId: t.id, taskKey: t.key, taskTitle: t.title });
  }
  // The project's assigned agent host, if it isn't already carrying a task.
  if (agg.assignedAgentHost) {
    const ref = `host:${agg.assignedAgentHost.id}`;
    if (!byOwner.has(ref))
      byOwner.set(ref, { ref, kind: 'host', name: agg.assignedAgentHost.name, status: 'available', reason: 'Assigned to project, no active task' });
  }

  const statusRank = rank;
  const workforce = [...byOwner.values()].sort((a, b) => statusRank[b.status] - statusRank[a.status] || a.name.localeCompare(b.name));
  ...
  return {
    ...
    overall,
    counts: {
      total, completed, open, blocked, overdue,
