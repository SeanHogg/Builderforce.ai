/**
 * IDE starter-template SEEDING — the R2 side of the scaffolds.
 *
 ...
 * run-only defaults that were never persisted). This module owns the decision of
 * WHEN to seed, so the creation routes ([projectRoutes]) and the lazy self-heal
 * on file-list ([ideRoutes]) share one implementation instead of duplicating the
 * gate logic.
 *
 * The scaffold CONTENT and the modality→scaffold map live in
 * `@builderforce/ide-templates`, because the browser needs the same answers for
 * the Run fallback and cannot import a module that talks to R2. Keeping the
 * content here as well is exactly what shipped "Web + Mobile" projects with no
 * files at all.
 *
...
export function scaffoldForProject(project: SeedableProject): Record<string, string> | null {
  return templateByName(project.template) ?? scaffoldForModality(project.modality);
}
...
export function templateForProject(project: SeedableProject): Record<string, string> | null {
  const explicit = templateByName(project.template);
...
export function templateLooksUnseeded(objects: TemplateObject[]): boolean {
  return !objects.some((o) => o.size > 0 && isScaffoldPath(o.path));
}
