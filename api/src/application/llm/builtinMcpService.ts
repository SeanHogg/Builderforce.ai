...
/** Sentinel extensionId the gateway routes to this in-process catalog. Defined in
 *  {@link ./toolNaming} beside `advertisedName` — the two answer the same question
 *  ("how is a platform tool addressed?") and a caller that needs one usually needs
 *  the other, without importing this module's whole dependency graph. */
export { BUILTIN_EXTENSION_ID } from './toolNaming';
...
const CATALOG: BuiltinTool[] = [
  ...
  { tool: 'initiatives.list', mutates: false, description: 'List initiatives (programs of work under a portfolio; compact: id/portfolioId/name/status/owner/dates + snippet), capped by limit (default 50, max 200).', parameters: obj({ limit: N }), run: async (ctx, a) => { const seg = await resolveSegment(ctx.db, ctx.tenantId); const rows = await ctx.db.select().from(initiatives).where(and(eq(initiatives.tenantId, ctx.tenantId), eq(initiatives.segmentId, seg))).orderBy(desc(initiatives.updatedAt)).limit(LIST_MAX_LIMIT); return listEnvelope('initiatives', rows.map((r) => compactRow(r as unknown as Record<string, unknown>, INITIATIVE_LIST_FIELDS, 'description')), clampLimit(a.limit)); } },
  {
    ...
    description: 'Create an initiative under a portfolio (pass portfolioId). startDate and targetDate are REQUIRED (YYYY-MM-DD): an initiative with no window renders permanently unscoped on the planning spine, because a brand-new initiative has no children for its dates to be derived from.',
    parameters: obj({ name: S, description: S, status: S, portfolioId: S, startDate: S, targetDate: S }, ['name', 'startDate', 'targetDate']),
    run: async (ctx, a) => {
      const name = str(a.name).trim(); if (!name) throw new Error('name is required');
      // Enforced in the RUNNER as well as the schema: a model can omit a "required"
      // field, and the whole point of this rule is that no path may create a
      // container with no window (the same guard the REST tracker applies).
      const initStart = dt(a.startDate); const initTarget = dt(a.targetDate);
      if (!initStart || !initTarget) throw new Error('startDate and targetDate are required (YYYY-MM-DD)');
      const seg = await resolveSegment(ctx.db, ctx.tenantId);
      const [row] = await ctx.db.insert(initiatives).values({ tenantId: ctx.tenantId, segmentId: seg, name, description: a.description != null ? str(a.description) : null, ...(a.status != null ? { status: str(a.status) } : {}), portfolioId: a.portfolioId != null ? str(a.portfolioId) : null, startDate: initStart, targetDate: initTarget }).returning();
      await bumpPmo(ctx);
      return row;
    },
  },
  ...
  { tool: 'objectives.list', mutates: false, description: 'List OKR objectives — the strategic goals on the Portfolio ▸ OKRs tab, NOT board Epics (compact: id/title/status/period/owner + snippet), capped by limit (default 50, max 200).', parameters: obj({ limit: N }), run: async (ctx, a) => { const seg = await resolveSegment(ctx.db, ctx.tenantId); const rows = await ctx.db.select().from(objectives).where(and(eq(objectives.tenantId, ctx.tenantId), eq(objectives.segmentId, seg))).orderBy(desc(objectives.updatedAt)).limit(LIST_MAX_LIMIT); return listEnvelope('objectives', rows.map((r) => compactRow(r as unknown as Record<string, unknown>, OBJECTIVE_LIST_FIELDS, 'description')), clampLimit(a.limit)); } },
  {
    ...
    description: 'Create an OKR Objective — a strategic, qualitative goal (e.g. "Unlock recurring revenue"). This populates the Portfolio ▸ OKRs tab. Do NOT model OKRs as board Epics. SCOPE the objective by passing exactly one of projectId (a goal FOR a specific project — this is what satisfies that project\'s "Direction" / "goal or OKR linked" health check), initiativeId, or portfolioId (omit all three for a workspace/org-level objective). Then add measurable targets with key_results.create and link the delivering epics/initiatives with objectives.add_link. status: active|achieved|missed|archived; period is an optional label like "2026-Q2". startDate and endDate are REQUIRED (YYYY-MM-DD) — an objective with no window renders permanently unscoped on the planning spine, because a brand-new objective has no key results or linked work for its dates to be derived from. Idempotent: an objective with the same title already in this workspace is returned ({ deduped: true, … }) instead of duplicated.',
    parameters: obj({ title: S, description: S, period: S, status: { type: 'string', enum: ['active', 'achieved', 'missed', 'archived'] }, projectId: N, portfolioId: S, initiativeId: S, startDate: S, endDate: S }, ['title', 'startDate', 'endDate']),
    run: async (ctx, a) => {
      const title = str(a.title).trim(); if (!title) throw new Error('title is required');
      // Enforced in the RUNNER as well as the schema — a model can omit a "required"
      // field, and no path may create a container with no window (same guard as the
      // REST tracker). Checked BEFORE the dedupe read so a bad call costs no query.
      const objStart = dt(a.startDate); const objEnd = dt(a.endDate);
      if (!objStart || !objEnd) throw new Error('startDate and endDate are required (YYYY-MM-DD)');
      const seg = await resolveSegment(ctx.db, ctx.tenantId);
      ...
      const [row] = await ctx.db.insert(objectives).values({
        ...
        startDate: objStart, endDate: objEnd,
      }).returning();
      await bumpPmo(ctx);
      return row;
    },
  ...
  { tool: 'prompts.browse_public', mutates: false, description: 'Browse the public prompt gallery (published prompts across all workspaces).', parameters: obj({ q: S, category: S, limit: N }), run: (ctx, a) => ctx.db.select({ ...getTableColumns(promptLibraryEntries), tenantId: promptLibraryEntries.tenantId }).from(promptLibraryEntries).where(eq(promptLibraryEntries.visibility, 'public')).orderBy(desc(promptLibraryEntries.starCount)).limit(clampLimit(a.limit)) },
  {
  ...
  { tool: 'chats.runs', mutates: false, description: "Every execution started against a ticket this chat links, newest first — the agent that actually RAN it, the run status, whether it produced anything, and `submittedBy`: WHICH pathway started it (`user:<id>` = a human pressed Run, `system:lane-auto` = board autonomy, `system:coordinator` = the manager's pass). Use it to tell 'the work is running' from 'the work was filed and nobody started it' — an empty result on a chat that has agents and linked tickets means nothing was ever dispatched, not that the runtime failed. Each run also names WHERE it executed — `executor` is 'container' (a real shell and a local clone, so the run could build/type-check/test), 'durable' (serverless and shell-LESS: it edits over the git API and provably could NOT build), or 'github_actions'; `hostName` names an on-prem machine instead. A chat whose runs were all 'durable' produced code nothing compiled.", parameters: obj({ chatId: N, limit: N }, ['chatId']), run: async (ctx, a) => { const r = await readChatRunHistoryForCaller(ctx.db, ctx.tenantId, num(a.chatId), ctx.userId ?? null, a.limit != null ? num(a.limit) : undefined); if (!r) throw new Error('Chat not found'); return r; } },
  {
  ...
  { tool: 'chats.invite_agent', mutates: true, description: 'Invite an agent into a Brain chat as a participant (agentRef = cloud agent id / workforce ref). Once invited it can be tagged to take action; use chats.dispatch_agent to have it execute a linked ticket.', parameters: obj({ chatId: N, agentRef: S, agentKind: S, role: S }, ['chatId', 'agentRef']), run: async (ctx, a) => { const svc = new ChatTicketService(ctx.db, ctx.env as Env); const r = await svc.inviteAgent(ctx.tenantId, num(a.chatId), ctx.userId ?? null, { agentRef: str(a.agentRef), agentKind: a.agentKind != null ? str(a.agentKind) : undefined, role: a.role != null ? str(a.role) : undefined }); if ('error' in r) throw new Error(r.error); return r; } },
  {
    ...
    description: "Tag an invited agent to EXECUTE: assign it to a task/epic (typically one linked to the chat — see chats.list_tickets) and start a run immediately, joining it to this chat so it narrates here and can be steered mid-run with executions.post_message. Returns the started execution. Only task/epic tickets are runnable. Use this for work the CURRENT session cannot do itself — a long or repetitive job, or work that must run elsewhere; if you have the workspace file tools and could make the change in a few calls, make it instead of hiring an agent to make it. On a lifecycle-managed board the stage's authorized role decides who executes, so the agent that actually runs may differ from `agentRef` — the response says which one it is. The PERSON in this chat directing the dispatch is the authority: their dispatch overrides every governance gate (an unstaffed lane, a managed stage with no bound role, a human gate, a tripped failure breaker) — the run is recorded under their name (`overrode` names the gate) and cannot advance the lane on its own. Only a billing entitlement refusal (cloud-run or token allowance) stands: read it and report it, do not retry the identical dispatch.",
  ...
  { tool: 'audit.list', mutates: false, description: 'List activity/audit events for the workspace (who did what, to what, when), optionally filtered by verb (e.g. "task.created", "user.login") / targetType (e.g. "task", "deployment").', parameters: obj({ limit: N, verb: S, targetType: S }), run: (ctx, a) => { const conds: SQL[] = [eq(activityLog.tenantId, ctx.tenantId)]; if (a.verb != null) conds.push(eq(activityLog.verb, str(a.verb))); if (a.targetType != null) conds.push(eq(activityLog.targetType, str(a.targetType))); return ctx.db.select().from(activityLog).where(and(...conds)).orderBy(desc(activityLog.id)).limit(clampLimit(a.limit)); } },
  ...
  { tool: 'skills_marketplace.list', mutates: false, description: 'Browse published marketplace skills (public).', parameters: obj({ category: S, q: S, limit: N }), run: (ctx, a) => ctx.db.select().from(marketplaceSkills).where(a.category != null ? and(eq(marketplaceSkills.published, true), eq(marketplaceSkills.category, str(a.category))) : eq(marketplaceSkills.published, true)).orderBy(desc(marketplaceSkills.downloads)).limit(clampLimit(a.limit)) },

  // ---- Artifact assignments (skill/persona → scope; 'content' retired in 0982). Tenant-scoped (NO segment_id);
  //       composite PK (tenantId, artifactType, artifactSlug, scope, scopeId). ----
  { tool: 'artifact_assignments.list', mutates: false, description: 'List artifacts (skill/persona) assigned to a scope.', parameters: obj({ scope: S, scopeId: N, artifactType: { type: 'string', enum: ['skill', 'persona', 'agent'] } }, ['scope', 'scopeId']), run: (ctx, a) => { const conds: SQL[] = [eq(artifactAssignments.tenantId, ctx.tenantId), eq(artifactAssignments.scope, str(a.scope) as never), eq(artifactAssignments.scopeId, num(a.scopeId))]; if (a.artifactType != null) conds.push(eq(artifactAssignments.artifactType, str(a.artifactType) as never)); return ctx.db.select().from(artifactAssignments).where(and(...conds)).limit(200); } },
  {
    tool: 'artifact_assignments.assign', mutates: true,
    description: 'Attach a skill or persona artifact to a scope.',
    parameters: obj({ artifactType: { type: 'string', enum: ['skill', 'persona', 'agent'] }, artifactSlug: S, scope: S, scopeId: N, config: S }, ['artifactType', 'artifactSlug', 'scope', 'scopeId']),
    run: async (ctx, a) => {
  ...
  //
  // The same money rule, one level down — and on most networks the AD SET is where the
  // daily budget actually lives, so creating one is exactly as expensive a mistake as
  // creating a campaign. Every write here goes through the manager-gated route rather
  // than calling `adSetService` directly, for the reason stated above.
  //
  // They exist so an agent says the same words the canvas form says. `adTargeting.ts`
  // is ONE vocabulary on purpose: if the panel offers "placements" and an agent had to
  // invent its own name for the same idea, the two would drift, and the failure that
  // follows is a budget spent correctly on people nobody asked for.
  {
    tool: 'ads.list_ad_sets', mutates: false,
    description: 'READ the ad sets on one ad account — the level BENEATH a campaign, where the audience and (on most networks) the daily budget live. Each comes back with its `targeting` in the shared vocabulary (countries, ageMin/ageMax, genders, interests, placements, devices), the network\'s own spec verbatim in `nativeTargeting` for audiences that vocabulary cannot name, plus status, bid and budget in CENTS. Pass `campaignId` (the network\'s own campaign id from ads.list_campaigns) for just one campaign\'s sets. Identify the account with `connectionId`, or `network` when the workspace has one account there.',
    parameters: obj({ connectionId: S, network: S, campaignId: S }, []),
    run: (ctx, a) => {
      const query = new URLSearchParams();
      if (a.connectionId != null) query.set('connectionId', str(a.connectionId));
      if (a.network != null) query.set('network', str(a.network));
      if (a.campaignId != null) query.set('campaignId', str(a.campaignId));
      const qs = query.toString();
      return replayRoute(ctx, 'GET', qs ? `/api/ads/adsets?${qs}` : '/api/ads/adsets');
    },
  },
  {
    tool: 'ads.create_ad_set', mutates: true,
    description: 'Create an ad set under an existing campaign — this is how an AUDIENCE is named. THIS SPENDS REAL MONEY: on most networks the daily budget lives here, not on the campaign. Created PAUSED unless you pass `launch: true`. `objective` is the PARENT campaign\'s objective carried down (several networks declare it at this level) and must be one that network supports. `targeting` is an object using ONLY these keys: countries (ISO 3166-1 alpha-2, e.g. ["US","GB"]), ageMin, ageMax (whole numbers 13-65), genders (["male"] or ["female"] — omit for everyone), interests (phrases), placements (feed, search, stories, video, audience_network), devices (mobile, desktop, tablet). A dimension the network cannot place is REFUSED BY NAME rather than dropped, so read the refusal instead of retrying without it. Budgets and bids are decimals in the account currency.',
    parameters: obj({
      campaignId: S, name: S, objective: S, connectionId: S, network: S,
      targeting: { type: 'object' }, dailyBudget: N, bid: N,
      startsAt: S, endsAt: S, launch: B,
    }, ['campaignId', 'name', 'objective']),
    run: (ctx, a) => replayRoute(ctx, 'POST', '/api/ads/adsets', {
      campaignId: str(a.campaignId),
      name: str(a.name),
      objective: str(a.objective),
      ...(a.connectionId != null ? { connectionId: str(a.connectionId) } : {}),
      ...(a.network != null ? { network: str(a.network) } : {}),
      ...(a.targeting != null ? { targeting: a.targeting } : {}),
      ...(a.dailyBudget != null ? { dailyBudget: num(a.dailyBudget) } : {}),
      ...(a.bid != null ? { bid: num(a.bid) } : {}),
      ...(a.startsAt != null ? { startsAt: str(a.startsAt) } : {}),
      ...(a.endsAt != null ? { endsAt: str(a.endsAt) } : {}),
      ...(a.launch === true ? { launch: true } : {}),
    }),
  ...
  {
    tool: 'ads.update_ad_set', mutates: true,
    description: 'Change an ad set — rename it, re-budget it, RE-TARGET it, pause it or resume it. THIS MOVES MONEY: `status: "active"` starts spending, and the budget here is usually the one that governs. `targeting` REPLACES the whole spec rather than merging into it (every network replaces here), so send every dimension you still want — omitting one DROPS it and widens the audience. Identify the set by the network\'s own id (`externalId` from ads.list_ad_sets) plus the account.',
    parameters: obj({
      externalId: S, connectionId: S, network: S, name: S, status: S,
      targeting: { type: 'object' }, dailyBudget: N, bid: N,
    }, ['externalId']),
    run: (ctx, a) => replayRoute(ctx, 'PATCH', `/api/ads/adsets/${encodeURIComponent(str(a.externalId))}`, {
      ...(a.connectionId != null ? { connectionId: str(a.connectionId) } : {}),
      ...(a.network != null ? { network: str(a.network) } : {}),
      ...(a.name != null ? { name: str(a.name) } : {}),
      ...(a.status != null ? { status: str(a.status) } : {}),
  ...
  {
    tool: 'ads.list_ads', mutates: false,
    description: 'READ the ads — the creatives a person actually sees — on one ad account, optionally only those inside one ad set (`adSetId`, the network\'s own id from ads.list_ad_sets). Each comes back with its headline, body, call to action and the destination URL AS IT WENT LIVE, which is UTM-tagged and therefore not byte-for-byte what was submitted.',
    parameters: obj({ connectionId: S, network: S, adSetId: S }, []),
    run: (ctx, a) => {
      const query = new URLSearchParams();
      if (a.connectionId != null) query.set('connectionId', str(a.connectionId));
      if (a.network != null) query.set('network', str(a.network));
      if (a.adSetId != null) query.set('adSetId', str(a.adSetId));
      const qs = query.toString();
      return replayRoute(ctx, 'GET', qs ? `/api/ads/ads?${qs}` : '/api/ads/ads');
    },
  },
  {
    tool: 'ads.create_ad', mutates: true,
    description: 'Create one ad inside an ad set. THIS SPENDS REAL MONEY once launched; created PAUSED unless you pass `launch: true`. Do NOT tag `destinationUrl` yourself — utm_source, utm_medium, utm_campaign and utm_content are stamped server-side from the campaign\'s own stored tag before any network sees the URL, and the tag actually applied comes back as `utmCampaign`. A utm_* parameter you wrote deliberately is kept rather than overwritten. Networks that can only promote EXISTING content (Pinterest, X) need `creativeRef` — the id of the post or pin — instead of new copy; ads.list_accounts says which ones.',
    parameters: obj({
      adSetId: S, name: S, connectionId: S, network: S,
      headline: S, body: S, callToAction: S, destinationUrl: S, creativeRef: S, launch: B,
    }, ['adSetId', 'name']),
    run: (ctx, a) => replayRoute(ctx, 'POST', '/api/ads/ads', {
      adSetId: str(a.adSetId),
      name: str(a.name),
      ...(a.connectionId != null ? { connectionId: str(a.connectionId) } : {}),
      ...(a.network != null ? { network: str(a.network) } : {}),
      ...(a.headline != null ? { headline: str(a.headline) } : {}),
      ...(a.body != null ? { body: str(a.body) } : {}),
      ...(a.callToAction != null ? { callToAction: str(a.callToAction) } : {}),
      ...(a.destinationUrl != null ? { destinationUrl: str(a.destinationUrl) } : {}),
      ...(a.creativeRef != null ? { creativeRef: str(a.creativeRef) } : {}),
      ...(a.launch === true ? { launch: true } : {}),
    }),
  },
  {
    tool: 'ads.update_ad', mutates: true,
    description: 'Rename an ad, PAUSE it or resume it. THIS MOVES MONEY when resuming. Deliberately narrow: on every one of these networks a live ad\'s copy and destination are IMMUTABLE — a changed creative is re-reviewed, so it is a NEW ad (ads.create_ad), not an edit. Identify it by the network\'s own id (`externalId` from ads.list_ads) plus the account.',
    parameters: obj({ externalId: S, connectionId: S, network: S, name: S, status: S }, ['externalId']),
    run: (ctx, a) => replayRoute(ctx, 'PATCH', `/api/ads/ads/${encodeURIComponent(str(a.externalId))}`, {
      ...(a.connectionId != null ? { connectionId: str(a.connectionId) } : {}),
      ...(a.network != null ? { network: str(a.network) } : {}),
      ...(a.name != null ? { name: str(a.name) } : {}),
      ...(a.status != null ? { status: str(a.status) } : {}),
  ...
  { tool: 'marketplace_stats.get_stats', mutates: false, description: 'Likes/installs for artifacts.', parameters: obj({ type: { type: 'string', enum: ['skill', 'persona', 'agent'] }, slugs: { type: 'array', items: S } }, ['type', 'slugs']), run: (ctx, a) => { const slugs = Array.isArray(a.slugs) ? a.slugs.map(str) : []; const q = new URLSearchParams({ type: str(a.type), slugs: slugs.join(',') }); return replayRoute(ctx, 'GET', `/api/marketplace-stats/stats?${q.toString()}`); } },
  ...
  { tool: 'marketplace.publish_ticket', mutates: true, description: 'Publish a work item (a board ticket — e.g. a Product brief or a Design gig authored by the Product Manager / Designer agent) to the Gig Marketplace so freelancers can estimate, bid, and be hired. The scope (title/description → requirements) is derived from the ticket, so you can publish with just a ticketId. Optional: postingType (project_bid|design|fte), engagementType (fixed_bid|hourly|fte), a requirements override, discipline + specialty, experienceLevel (entry|intermediate|expert), projectLength (lt_1_month|1_3_months|3_6_months|gt_6_months|ongoing), visibility (public|private), and the money — rateMinCents/rateMaxCents for hourly work (a per-hour band) or budgetTotalCents for fixed-price work (a whole-job total); a total on hourly work is refused. Returns { jobId }.', parameters: obj({ ticketId: N, postingType: S, engagementType: S, requirements: S, discipline: S, specialty: S, experienceLevel: S, projectLength: S, rateMinCents: N, rateMaxCents: N, budgetTotalCents: N, visibility: S }, ['ticketId']), run: (ctx, a) => replayRoute(ctx, 'POST', '/api/marketplace/publish', { ticketId: num(a.ticketId), postingType: a.postingType != null ? str(a.postingType) : undefined, engagementType: a.engagementType != null ? str(a.engagementType) : undefined, requirements: a.requirements != null ? str(a.requirements) : undefined, discipline: a.discipline != null ? str(a.discipline) : undefined, specialty: a.specialty != null ? str(a.specialty) : undefined, experienceLevel: a.experienceLevel != null ? str(a.experienceLevel) : undefined, projectLength: a.projectLength != null ? str(a.projectLength) : undefined, rateMinCents: a.rateMinCents != null ? num(a.rateMinCents) : undefined, rateMaxCents: a.rateMaxCents != null ? num(a.rateMaxCents) : undefined, budgetTotalCents: a.budgetTotalCents != null ? num(a.budgetTotalCents) : undefined, visibility: a.visibility != null ? str(a.visibility) : undefined }) },
  ...
  { tool: 'jobs.create', mutates: true, description: 'Create a Marketplace job posting directly (not tied to a board ticket) — e.g. an FTE Job Posting (postingType:"fte") candidates interview for, or a standalone gig. For a gig derived from an existing work item, prefer marketplace.publish_ticket. requirements = the acceptance criteria a proposal is AI-evaluated against. Money follows the SHAPE: hourly work takes rateMinCents/rateMaxCents (a per-hour band), fixed-price work takes budgetTotalCents (a whole-job total) — sending a total on hourly work is refused. specialty is a sub-category of discipline. Passing sourceTicketId creates or REOPENS that ticket\u2019s one posting rather than minting a second (same service as marketplace.publish_ticket).', parameters: obj({ title: S, description: S, requirements: S, discipline: S, specialty: S, skills: { type: 'array', items: S }, postingType: S, engagementType: S, experienceLevel: S, projectLength: S, projectId: N, rateMinCents: N, rateMaxCents: N, budgetTotalCents: N, sourceTicketId: N, visibility: S }, ['title']), run: (ctx, a) => replayRoute(ctx, 'POST', '/api/jobs', { title: str(a.title), description: a.description != null ? str(a.description) : undefined, requirements: a.requirements != null ? str(a.requirements) : undefined, discipline: a.discipline != null ? str(a.discipline) : undefined, specialty: a.specialty != null ? str(a.specialty) : undefined, skills: Array.isArray(a.skills) ? a.skills.map(str) : undefined, postingType: a.postingType != null ? str(a.postingType) : undefined, engagementType: a.engagementType != null ? str(a.engagementType) : undefined, experienceLevel: a.experienceLevel != null ? str(a.experienceLevel) : undefined, projectLength: a.projectLength != null ? str(a.projectLength) : undefined, projectId: a.projectId != null ? num(a.projectId) : undefined, rateMinCents: a.rateMinCents != null ? num(a.rateMinCents) : undefined, rateMaxCents: a.rateMaxCents != null ? num(a.rateMaxCents) : undefined, budgetTotalCents: a.budgetTotalCents != null ? num(a.budgetTotalCents) : undefined, sourceTicketId: a.sourceTicketId != null ? num(a.sourceTicketId) : undefined, visibility: a.visibility != null ? str(a.visibility) : undefined }) },
...
function buildCtx(
  ...
  return {
    ...
    // Creation attribution for tickets minted through MCP (`tasks.create`,
    // `tickets.from_delta`). This is also what canonicalizes `target_type` to `'task'`:
    // the MCP writer used to record `'tasks'`, so the lifecycle ledger read one shape and
    // silently missed the other (103 plural rows against 8 singular).
    tasks: new TaskService(taskRepo, projectRepo, undefined, undefined, undefined, undefined, undefined, taskCreatedHook(db, opts?.env)),
    env: opts?.env,
...
/**
 * The two curated agent allowlists moved to {@link ./cloudAgentToolset} — pure id
 * lists that anything reading a tool SURFACE needs without dragging in this whole
 * module (the same carve-out {@link ./toolNaming} got, for the same reason).
