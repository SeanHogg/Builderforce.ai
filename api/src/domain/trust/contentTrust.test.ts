...
import { isHumanOrExternalOutputTool, secretLeakReasons, trustNotice } from './contentTrust';
describe('content trust', () => {
  ...
  it('recognizes human and connector output seams', () => {
    expect(isHumanOrExternalOutputTool('connector_call')).toBe(true);
    expect(isHumanOrExternalOutputTool('read_file')).toBe(false);
  });
});
