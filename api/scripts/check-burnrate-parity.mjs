...
const TRANSFORM = {
  ...
  // Found by DOING the merge (PRD 19 §9.7), not by policy v1. A strict
  // duplicate of a Builderforce owner that is already feature-reached and
  // strictly richer, so giving it a feature path would create a SECOND
  // answer to a question the platform already answers — the outcome §2 exists
  // to prevent. Its rows transform onto the existing owner.
  // (`kanban_columns` and `release_plans` were the other two; migration 1160
  // folded them into `swimlanes` / `product_releases` and dropped them, so their
  // map rows read `merged` and they are no longer register targets at all.)
