...
| #66 | Fix the agent execution | ready | bug | critical | d02ff7ee | — | Title: `fix`; Seed task #66; Core execution path defect | Agent runtime timeout errors; PR #28 |
| #90 | Fix the build | in_progress | bug | critical | d02ff7ee | — | Title: `fix` + `build`; Seed task #90; CI build breakage (Frontend typecheck) | **Potential duplicate of #57** — same title "Fix the build" (Frontend vs API) |
...
| #68 | `{"error":"No transactions support in neon-http driver"}` | ready | bug | medium (inferred) | 658608ba | #138 | Title: `error`; 500 Internal Server Error | Boards API POST fails; blocks board creation |
...
| #354 | Helcim checkout: recurring billing schedule not created | ready | bug | high (inferred) | 6e83f382 | #134 | Description: `needs to be fixed`; Missing feature = billing defect | One-time charges only; blocks recurring revenue |
...
| #353 | Agent channels endpoint always returns `[]` (stub, no DB) | ready | ambiguous | high (inferred) | No bug keyword; endpoint returns empty array — may be intentional stub | Empty response from production endpoint may mask a regression |
| #355 | Helcim webhook mapping is a placeholder | ready | ambiguous | high (inferred) | No bug keyword in title; body describes incorrect mapping | `APPROVED` webhooks incorrectly map to `subscription.activated` — defect if data is live |
