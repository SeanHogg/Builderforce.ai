...
- **studio-embedded has no component test harness, so the two-pass preview bug shipped uncaught.** This pass fixed a real bug in [StudioPanel.tsx](studio-embedded/src/components/StudioPanel.tsx): in the "Refined" two-pass quality tier, `onFrame` fires for both the draft pass and the refinement pass, so `previewFrames` accumulated 2N bitmaps — and since [video-engine.ts](studio/src/engine/video-engine.ts) `.close()`s the N draft bitmaps after refining, `VideoPreview`'s thumbnail loop would `drawImage()` a closed bitmap and throw, leaving the Refined tier with no thumbnail strip. Fixed by collapsing `previewFrames` onto `generated.frames` after generation. Per the repo's regression-test rule the invariant ("after generate, previewFrames === final frame set, contains no closed bitmaps") should be locked, but [studio-embedded/package.json](studio-embedded/package.json) runs `vitest run --passWithNoTests` with no jsdom + React Testing Library setup, so there is no layer to write the test in. Fixing (add `jsdom` + `@testing-library/react` to studio-embedded and a StudioPanel two-pass test that asserts the frame-set invariant) unblocks: locking this and future preview/lifecycle regressions in the embedded panel that engine-level unit tests can't reach.
- **`onSaveVersion` persists `model`, not the resolved quality tier.** [StudioPanel.tsx](studio-embedded/src/components/StudioPanel.tsx) `VideoVersionParams` records the raw `model` state field, but in simple mode the engine is actually created from `resolveQualityTier(quality)` (`tier.primary` + optional `tier.refinement`), and `model` is only consulted when Advanced is open. So a version saved in simple "Refined" mode persists whatever stale `model` default was in state, not `lcm-tiny-sd → lcm-dreamshaper-v7` — the saved params can't reproduce the generation. Fixing (persist `quality` + `showAdvanced`, or persist the resolved `{primary, refinement}` pair, and replay through the same resolver) unblocks: faithful version reproduction and a correct version-history audit trail.
