...
- **Product & Agile net-new features are the remaining large frontier (not built).** Wired Product/Agile widgets resurface EXISTING app features (tasks→backlog/kanban, Brain→ideas, PRDs→prd). The rest are net-new with NO backing: Product — `mvp`, `validation`, `roadmap`, `release-planning`, `changelog`, `feature-flags`, `feature-roi`, `business-value`; Agile — `poker`, `retros`, `sprints`, `velocity`, `capacity`, `cost`, `feature-scoring`. These need the **`work_items` unified spine** (locked decision — `tasks`/`specs` become specializations), new schema, the `/v1` API, realtime rooms (poker/retros), and AI features (roadmap/validation) — a multi-migration body comparable to everything built so far. Fixing (build the `work_items` spine first, then each feature's schema + API + surface) unblocks: the Product + Agile embed surfaces.
