...
- **SDK 0.8.0 not yet published to npm — the publish workflow is failing with a 404 PUT.** Latest version live on npm is `0.7.0`; the 0.9.0 attempt by [release.yml](.github/workflows/release.yml) → [publish-if-new.mjs](sdk/scripts/publish-if-new.mjs) errored with `404 Not Found - PUT https://registry.npmjs.org/@seanhogg%2fbuilderforce-sdk`, which is npm's misleading rendering of a publish-permission failure (registry hides "no access" behind 404 to avoid leaking package existence). The workflow authenticates via the long-lived `NPM_TOKEN` automation secret — likely root cause is the token being rotated / expired / scoped without `@seanhogg/builderforce-sdk` write access, OR Trusted Publishing was enabled on npmjs.com without removing the `NODE_AUTH_TOKEN` env from the workflow step. SDK 0.8.0 (this pass — adds top-level `vendor` + `model` on `BuilderforceApiError` and `resolvedVendor` in `_builderforce`) will remain stuck on the publishing path until this is unblocked. Fixing (1) regenerate `NPM_TOKEN` via `npm token create --type=automation` with `@seanhogg` scope write access and update the repo secret, OR (2) complete the Trusted Publishing migration on npmjs.com and drop the `NODE_AUTH_TOKEN` from [release.yml:138](.github/workflows/release.yml#L138) so OIDC takes over) unblocks: customers picking up gateway features as soon as they land instead of being pinned to 0.7.0 forever, AND the consumer-side `inferVendorFromModel` prefix-heuristic cleanup that depends on the new error fields.
