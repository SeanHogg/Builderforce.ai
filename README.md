...
- **No vendor-level cooldown — saturated OpenRouter key cools per-model only, cascade burns the whole free pool one 429 at a time.** [cooldownStore.ts](api/src/infrastructure/auth/cooldownStore.ts) keys cooldowns by `(vendor, model)`. When OpenRouter's free-tier API key is rate-limited globally, every model on that key returns 429 in rapid succession — the gateway's cascade walks all 20+ OpenRouter free models, each gets its own 429 + cooldown write, total ~20× round trips of wasted budget per request. Fixing (track a parallel `cooldown:vendor:<vendor>` key that cools the ENTIRE vendor when N≥3 models from the same vendor 429 within a sliding window, e.g. 60s — chain composer skips the vendor block wholesale until the vendor cooldown lifts) unblocks: customers getting fast cross-vendor failover instead of cascade-walking a single saturated key, and dramatic reduction in 429 events surfaced in [/admin?tab=usage](frontend/src/app/admin/page.tsx) failover table when a single upstream is throttled.
