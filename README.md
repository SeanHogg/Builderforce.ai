...
- **All-vendor saturation still produces a 503 when every configured vendor is wholesale-cooled at the same time.** [cooldownStore.ts](api/src/infrastructure/auth/cooldownStore.ts) now tracks `vendor_cooldown:<vendor>` alongside per-model cooldowns (1 auth failure or 3 transient failures within 60s trips the vendor), and [FREE_MODEL_POOL / PRO_PAID_MODEL_POOL](api/src/application/llm/LlmProxyService.ts) now compose from every registered vendor via `modelsByTier` so Cerebras / Ollama / NVIDIA free models are first-class in the cascade rather than appearing only via a single cross-vendor fallback. The residual case: when free vendors are all wholesale-cooled simultaneously, free-tier callers see a clean 503 instead of a slow walk — but there's no automatic paid fallback. Fixing (allow free-plan tenants to opt-in to a paid last-resort tail via `body.allowPaidFallback: true`, billed against a configurable per-tenant cap) unblocks: zero `AI_RATE_LIMITED` escapes for tenants willing to spend on overflow.
