...
- **`@seanhogg/builderforce-sdk` first publish needs the `NPM_TOKEN` secret added to the repo.** [release.yml `publish-sdk` job](.github/workflows/release.yml) runs on every push to `main` and idempotently publishes via [sdk/scripts/publish-if-new.mjs](sdk/scripts/publish-if-new.mjs) — but it 401s until you add `NPM_TOKEN` (npm automation token) at *Settings → Secrets and variables → Actions*. Once that's set, every version-bump commit lands on npm without manual `npm publish`. Until then tenant apps consume via `npm install file:../Builderforce.ai/sdk` or git URL.
- **`Idempotency-Key` dedup TTL not enforced.** SDK sends the header; gateway logs to `llm_usage_log.idempotency_key`; but no actual dedup happens — a retry within TTL still produces a fresh vendor call and a fresh charge. Need a KV-backed cache (Cloudflare Workers KV or a Durable Object) keyed by `(tenantId, idempotencyKey)` that returns the cached response body + status within ~10 min. Header acceptance is safe to ship today; tenants just shouldn't rely on it for billing-critical retries yet. Fixing unblocks safe automatic retries for cron-driven enrichment jobs.
