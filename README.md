...
- **Cloudflare Pages `cf-build` chain has not been validated against the `/studio` route's WASM/ORT footprint.** [frontend/package.json](frontend/package.json) `cf-build` runs `next-on-pages` then `scripts/strip-worker-assets.js` to keep the Worker bundle under 3 MiB by stripping ORT/WASM from the worker (these get served as static assets instead). The strip script was written when only Transformers.js was a consumer; now `onnxruntime-web` + `@huggingface/transformers` are both imported through the studio chunk on `/studio`, plus `mp4-muxer` ships fresh WASM. The local `next build` succeeds (verified 362 KB First Load JS for `/studio`) but the cf-build pipeline could either (a) leave studio's ORT WASM in the worker and breach the 3 MiB cap, or (b) over-strip and break the studio at runtime. Fixing (run `npm run cf-build` in CI, check the resulting `_worker.js` size, extend `strip-worker-assets.js` if needed to recognise the studio's bundled paths) unblocks: confidence that the deployed `/studio` actually works at edge, not just locally.
