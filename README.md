...
- **Board column → swimlane mapping is a name/key heuristic, not a stored relation.** The task board's kanban columns are the fixed `TaskStatus` set (`BOARD_STATUSES` in [TaskMgmtContent.tsx](frontend/src/components/TaskMgmtContent.tsx)), but swimlanes are user-created with arbitrary `key`/`name` and no FK back to a status (`swimlanes` table in [schema.ts](api/src/infrastructure/database/schema.ts), created via [boardRoutes.ts](api/src/presentation/routes/boardRoutes.ts) with no default seeding). To show "which agents are configured on which swimlane" in each column header, `agentsByStatus` matches a lane to a status by `normalizeKey(lane.key|lane.name) === normalizeKey(status|STATUS_LABELS[status])` ([useBoardConfig.ts](frontend/src/components/board/useBoardConfig.ts) `normalizeKey`). A board whose lanes are named off-status (e.g. "Implement", "Review-2") will simply not surface its agents on any column. Fixing (add a nullable `status`/`mapped_status` column to `swimlanes`, set it in the board-config UI, and key `agentsByStatus` off it — or drive the kanban columns from swimlanes directly instead of the fixed status set) unblocks: reliable agent-per-lane display for arbitrarily-named boards.
