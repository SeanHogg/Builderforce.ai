...
- **Cron jobs are project-scoped, not per-assigned-agent — the "different cron per agent" UX isn't fully storable.** This pass moved Cron out of its own project tab into the **Agent / Capabilities** tab ([AgentCapabilitiesContent.tsx](frontend/src/components/AgentCapabilitiesContent.tsx) renders [CronJobsContent](frontend/src/components/CronJobsContent.tsx) once an agent is assigned) and removed the standalone **Cron** + **Instances** tabs ([ProjectDetailsPanel.tsx](frontend/src/components/ProjectDetailsPanel.tsx)). But [cron_jobs](api/src/infrastructure/database/schema.ts) is keyed by `(agent_host_id, project_id)` only — there is no `project_agent_id` / `swimlane_agent_assignment_id` column — so every assigned agent in a project shares the same project cron list (the per-agent label is cosmetic). Fixing (add a nullable `project_agent_id` FK to `cron_jobs` + thread it through `cronApi.list/create` + the cron poller's filter) unblocks: genuinely distinct schedules per assigned agent.
