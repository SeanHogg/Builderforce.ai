...
- **`@seanhogg/builderforce-studio` first-publish requires either an `NPM_TOKEN` secret or pre-registering trusted publishing on npmjs.com.** [`.github/workflows/publish-npm-package.yml`](.github/workflows/publish-npm-package.yml) now supports BOTH auth modes (NPM_TOKEN takes precedence; OIDC otherwise), so either path works. Pick one operator action: (a) Add `NPM_TOKEN` to GitHub repo secrets — fastest, one-time, but a long-lived token in secrets; can be removed after first publish if OIDC is then configured. (b) Pre-register trusted publishing at https://www.npmjs.com → Add Package → `@seanhogg/builderforce-studio` → Add trusted publisher (GitHub Actions / SeanHogg / Builderforce.ai / release.yml / `publish-studio`) — zero-secrets but requires the unpublished-package OIDC registration flow. Fixing (operator picks one) unblocks: the studio actually gets published, third-party consumers can `npm install @seanhogg/builderforce-studio`.
