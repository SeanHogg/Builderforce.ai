...
- **Frame interpolation has no LEARNED (RIFE/FILM) backend — two real backends ship, the learned one is the optional upgrade.** This pass added a pluggable interpolation seam with two working backends ([GenerateOptions.interpolationBackend](studio/src/types.ts)): `latent-slerp` ([frame-interpolator.ts](studio/src/engine/frame-interpolator.ts)) and `motion` ([motion-interpolator.ts](studio/src/engine/motion-interpolator.ts) — block optical-flow estimate + motion-compensated warp, real per-block displacement, no external weights). What's still absent is a *learned* flow model (RIFE/FILM) behind the same `estimateBlockMotion`/`interpolateFrames` signature: block matching is coarser than a trained network on large/occluded motion. Fixing (add a `RifeInterpolator` ONNX backend — fetched via [weight-cache.ts](studio/src/engine/weight-cache.ts) like the diffusion models, with init-time I/O contract verification and graceful fallback to `motion`) unblocks: best-in-class interpolation quality on hard motion. Lower priority now that a real motion backend exists.
- **Motion-backend tweens carry the slerp latent (not their warped pixels) into the refinement pass.** In [video-engine.ts](studio/src/engine/video-engine.ts) `produceClip`, when `interpolationBackend: 'motion'` AND a refinement model is wired, each tween's DISPLAYED pixels are the motion warp but its entry in `latents[]` is the slerp latent (motion warps are pixel-space and have no latent). So the rare motion+refine+interpolation combo re-renders tweens from the slerp latent in the refinement pass, discarding the motion warp. Acceptable today (refinement is a detail pass and motion+refine is uncommon), but it means motion interpolation and refinement don't compose faithfully. Fixing (VAE-encode the warped tween pixels to a latent for refinement, or skip refinement on motion tweens and keep the warp) unblocks: motion interpolation + Refined tier composing correctly.
- **Cinematic storyboard editing is field-level only — can't add/remove/reorder shots or edit the character bible.** [StoryboardEditor.tsx](studio-embedded/src/components/StoryboardEditor.tsx) now lets the user edit each shot's prompt / camera / frame count before render and shows post-render VLM validation badges, but there's no affordance to insert a shot, delete a shot, reorder shots, or edit a character's locked `appearance`. A director who wants an extra cutaway or a wardrobe change must re-plan. Fixing (add/remove/drag-reorder controls on the shot list + an editable character-bible section) unblocks: full director control over the plan without round-tripping the LLM.
