...
**npm publishing:** the [release.yml](.github/workflows/release.yml) `package-npm` job builds, tests, and version-gates all public packages in one runner before deployment. Each package is published with provenance only when its `package.json` version is absent from npm, making re-runs idempotent. Auth uses npm Trusted Publishing OIDC (with `NPM_TOKEN` as an optional bootstrap fallback). Configure each package's trusted publisher for Organization `SeanHogg`, Repository `Builderforce.ai`, and Workflow `release.yml`.
