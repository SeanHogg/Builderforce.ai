...
- **Per-vendor-call timeout is a fixed constant (25s).** [vendors/types.ts](api/src/application/llm/vendors/types.ts) hard-codes `DEFAULT_VENDOR_CALL_TIMEOUT_MS = 25_000` in `fetchWithVendorTimeout`. That covers most use cases (chat, classification, structured) but is wrong for long-form scenarios that legitimately take >25s for a single vendor (`career_360`, full article generation). Today such requests will time out per-vendor and burn through the whole chain even when the vendor would have completed. Fixing (per-request override via `body._builderforce: { vendorTimeoutMs }` or per-use-case profile) unblocks: long-form workloads riding on free-tier vendors that have higher latency variance.
...
- **Superadmin `tokenDailyLimitOverride` mutations are not audit-logged.** [PATCH /api/admin/tenants/:id/token-limit-override](api/src/presentation/routes/adminRoutes.ts) lets a superadmin set a tenant's daily token cap to unlimited (-1), a custom integer, or null (revert to plan default), but writes the change directly without inserting a row into `admin_audit_log`. Other superadmin mutations (impersonation, role changes, module assignments) record actor + before/after in [adminAuditLog](api/src/infrastructure/database/schema.ts) — this one does not. Fixing (insert an `admin_audit_log` row keyed by actor user id + tenant id + before/after value) unblocks: forensic trail for "who removed the cap on tenant X and when."
...
- **Frontend types for the LLM model-status endpoints understate the real response shape.** [adminApi.ts:46-52](frontend/src/lib/adminApi.ts#L46-L52) `LlmModelStatus` omits `vendor: VendorId`, which the backend includes ([LlmProxyService.status()](api/src/application/llm/LlmProxyService.ts)). [builderforceApi.ts:1018](frontend/src/lib/builderforceApi.ts#L1018) `llmApi.models()` types the response as `{ data: Array<{ id, object }> }` but the actual `/llm/v1/models` route ([llmRoutes.ts:632](api/src/presentation/routes/llmRoutes.ts#L632)) returns a union of `{ configured: false, models: string[] }` (no API key) OR `{ configured: true, data: Array<{ model, preferred, available, cooldownUntil?, vendor }> }`. Callers that try to read `vendor` get a typing-vs-runtime mismatch. Admin Usage's AI Analyze button currently sidesteps this by deriving vendor from id prefix instead of trusting the field. Fixing (widen `LlmModelStatus` to include `vendor`, change `llmApi.models()` return type to the union) unblocks: callers using the catalog without prefix-parsing workarounds, and a vendor breakdown in /admin?tab=usage's chart legend.
