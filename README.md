...
- **The agent API-key prefix `clk_` still encodes the retired "claw" brand.** [HashService.generateApiKey](api/src/infrastructure/auth/HashService.ts) mints agent keys as `clk_*`, and the auth router branches on `token.startsWith('clk_')` ([llmRoutes.ts](api/src/presentation/routes/llmRoutes.ts)) with `keyResolutionCache` keyed by `'clk'`. It is a live, user-visible brand leak (the key string a registered agent holds literally starts with "clk"). NOT renamed this pass because existing deployed agents hold `clk_` keys and the prefix is load-bearing for routing — a blind swap breaks live auth. Fixing safely (mint new keys with a BuilderForce-aligned prefix, e.g. `bfa_`, AND accept BOTH `clk_`/`bfa_` in `llmRoutes` + `keyResolutionCache` during a migration window, then drop `clk_` once all keys rotate) unblocks: no "claw" in any agent credential. Part of the broader CoderClaw→BuilderForce rebrand tracked below.
- **Marketing `FEATURES` is duplicated and the canonical list is under-rendered.** The new auto-assign / agents-ship-code capabilities were added to BOTH the canonical [content.ts `FEATURES`](frontend/src/lib/content.ts) and the separate hardcoded array in [agents/page.tsx](frontend/src/app/agents/page.tsx) (which renders all of them) because the two are not unified (DRY gap). Worse, the only consumer of `content.FEATURES` ([RegisterPageClient.tsx](frontend/src/app/register/RegisterPageClient.tsx)) renders `FEATURES.slice(0, 6)`, so any feature past index 6 (incl. the two just added) is canonical-but-invisible there. Fixing (have agents/page.tsx import `content.FEATURES` via a shared `Feature`-shaped adapter incl. icons, and drop or raise the register-page slice) unblocks: one source of truth for marketing features and the new capabilities showing everywhere.
- **README top-of-file marketing prose + the "Integration with CoderClaw" / self-bootstrap sections still say "CoderClaw"/"Claw" and link `coderclaw.ai`.** Lines ~13/21/44/55–63/316–355 etc. predate the rename and are tracked by the in-progress rebrand section below (DNS 301 + prose sweep are listed follow-ups). Not swept this pass to avoid conflicting with that effort. Fixing (finish the prose sweep + `coderclaw.ai`→`builderforce.ai/agents` links as part of the tracked rebrand) unblocks: a brand-consistent README.
- **Pre-existing TS build break in [frontend/src/app/projects/page.tsx](frontend/src/app/projects/page.tsx) — `DeleteProjectDialog` referenced but not defined.** `tsc --noEmit` fails with `TS2552: Cannot find name 'DeleteProjectDialog'` (line ~440) plus an implicit-`any` on the `project` param of its render callback. Unrelated to the cloud-coding work (surfaced while typechecking the swimlane target-picker change); the component is used but never imported/defined. Fixing (define or import the dialog, type the callback param) unblocks: a clean frontend typecheck/build for the projects page.
