...
- **Cross-vendor coverage depends on operational key binding — vendor-level cooldown can short-circuit a saturated OpenRouter key but only helps when another vendor's key is configured.** [cooldownStore.ts](api/src/infrastructure/auth/cooldownStore.ts) now tracks `vendor_cooldown:<vendor>` alongside per-model cooldowns: 1 auth failure (401/403) or 3 transient failures (429/5xx/408) within 60s trip the vendor wholesale, and [buildCandidateChain](api/src/application/llm/LlmProxyService.ts) skips every model owned by a cooled vendor in one pass. The remaining gap is the recovery target — when CEREBRAS / NVIDIA / OLLAMA keys aren't bound on a tenant's path, vendor cooldown still produces a clean 503 instead of a slow walk, but there's nowhere to land. Fixing (require at least one cross-vendor key bound for any tenant on the free plan, or fall the gateway's own `OPENROUTER_API_KEY_PRO` paid pool in as a last-resort tail for free tenants when the free vendor is wholesale-cooled) unblocks: customers seeing zero `AI_RATE_LIMITED` escapes when the free pool's single upstream is throttled.
