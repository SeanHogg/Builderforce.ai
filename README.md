...
- **Tenant JWTs with `sa: true` aren't checked against `authTokens` revocation in `llmRoutes`.** [requireTenantAccess](api/src/presentation/routes/llmRoutes.ts) verifies the signature via `verifyJwt` but does not consult the `authTokens` table to honor revocation — unlike [webAuthMiddleware](api/src/presentation/middleware/webAuthMiddleware.ts) and [authMiddleware](api/src/presentation/middleware/authMiddleware.ts), which both check by `jti`. After this release, tenant JWTs can carry `sa: true` (added so platform admins bypass plan caps from the admin UI), so a revoked superadmin still grants gateway bypass for up to the JWT's 1-hour TTL even after revocation flows through `admin/audit_log`. Fixing (mirror the revocation check from `webAuthMiddleware` into `requireTenantAccess`, or accept the 1h staleness as the worst case across all tenant-JWT auth — same staleness already exists for non-`sa` claims like role) unblocks: immediate effect when a superadmin is revoked, regardless of which token type they were using. Note: same `sa` claim on the WebJWT already has a 24h staleness window, so this is not a regression — but the gateway path is the most-billable surface, so it's the highest-value place to add the revocation check.
