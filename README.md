...
- **No CI-level e2e covers the studio's actual in-browser ONNX execution — two invariants are unit-tested, the rest needs a real GPU browser.** Two layers are now locked by unit tests: (1) the ORT WASM/JS version-match ([onnx-runtime-config.test.ts](studio/src/engine/onnx-runtime-config.test.ts), guard for `_OrtGetInputOutputMetadata is not a function`); (2) the per-model UNet input contract ([diffusion-engine.test.ts](studio/src/engine/diffusion-engine.test.ts), guard for `input 'X' is missing in 'feeds'`) — every name in a model's `unetInputNames` must have a registered builder in `UNET_INPUT_BUILDERS`, and LCM models must declare `timestep_cond`. The layers still needing a real WebGPU browser: (a) dtype precision (`int32` vs `int64` for `input_ids` and `timestep`); (b) exact UNet output name (we currently pick the first Float32 output); (c) the LCM `timestep_cond` sinusoidal embedding values match the model's training; (d) the CLIP-L tokenizer's pad-id assumption holds for SD-Turbo; (e) the LCM UNet 1.7GB fp32 + external data actually fits 6GB VRAM. These need a Playwright job on a GPU runner. Fixing unblocks: confirmed end-to-end image fidelity, not just "pipeline resolves without throwing" + unit-tested contracts.
