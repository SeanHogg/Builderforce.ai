...
- **Governance compliance schema STARTED ([0057](api/migrations/0057_governance_compliance.sql)); API + surfaces + remaining tables open.** Landed + applied + verified (drift 69 tables, trigger default-fill confirmed): `soc_controls`, `soc_evidence`, `security_vendors`, `security_incidents` — all `(tenant_id, segment_id)`-scoped with the 0056 default-segment trigger (doc 07 SEC-1/2/3). **Still open:** (a) the remaining doc-07 tables — `pii_data_assets`, `security_dpas`, `security_trainings`, `compliance_events`, `data_subject_requests`, `data_suppression_list`, `access_reviews`, `vulnerability_scans`+`findings`, `security_audit_log`; (b) the `/v1/governance/*` (or `/api/governance/*`) routes (Segment-scoped, audited per doc 07 §4); (c) frontend surface components (Soc2/Vendors/Incidents…) + embed wiring (flip `available:true`). Until (b)+(c), the `soc2`/`vendors`/`incidents` embed views still render the scaffold. Fixing unblocks: the first real Security-pillar embedded widgets.
