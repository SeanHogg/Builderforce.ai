...
- **Video continuity is anchor-latent + per-frame noise blend; there is no trained motion model, so frames stay coherent but the scene does not "evolve" temporally.** [studio/src/engine/video-engine.ts](studio/src/engine/video-engine.ts) now samples one anchor latent per clip and blends each frame as `sqrt(1-motion) * anchor + sqrt(motion) * frameNoise` (default `motionAmount = 0.15`). This delivers locked color palette + locked composition across frames — the bug a user reported as "each picture is a unique interpretation of the prompt vs the progression of the animation". The remaining gap: scene PROGRESSION (camera move, object trajectory, action over time) requires either (a) an AnimateDiff-style motion module wired into the UNet (significant — needs a trained motion-LoRA and an extra cross-frame attention pass per step), or (b) img2img-style frame warping where frame N+1 starts from frame N's decoded latent re-noised partway through the schedule (cheaper — Deforum / Stable WarpFusion pattern, fits the existing engine). The [Mamba state bias path](studio/src/engine/mamba-coherence.ts) is already wired and could, in principle, drive (b) by accumulating frame deltas into a "motion vector" applied to the next anchor — but the current `advanceState()` is the CPU placeholder noted elsewhere in this register. Fixing (option b is the practical pick — re-noise the previous frame's decoded latent at a partial timestep instead of sampling fresh anchor noise per frame) unblocks: actual moving-scene video, not just "same shot with slight wobble."
