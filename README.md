...
- **Studio diffusion ONNX I/O dtypes + tensor names are assumed, not verified against a live run.** [studio/src/engine/diffusion-engine.ts](studio/src/engine/diffusion-engine.ts) now targets real repos (`schmuell/sd-turbo-ort-web` single-file, `aislamov/lcm-dreamshaper-v7-onnx` with external-data sidecars) and tokenizes via `Xenova/clip-vit-large-patch14`. Several assumptions still need confirmation in a real WebGPU browser session: (a) text-encoder `input_ids` is `int32` (could be `int64`); (b) UNet `timestep` is `int64` scalar (could be `float32`); (c) UNet input names are `sample`/`timestep`/`encoder_hidden_states` and output is `out_sample`; (d) the CLIP-L tokenizer pads with eot=49407 which matches SD1.5 (LCM) but SD2.1/SD-Turbo historically padded with 0 — using CLIP-L for SD-Turbo may shift conditioning slightly; (e) the LCM UNet is fp32 + external data (~1.7GB) so the 6GB-VRAM floor is optimistic on first cold download. Fixing (run an 8-frame gen on Chrome 121+, inspect `session.inputNames`/`outputNames` for each ORT session, correct any dtype/name mismatches, and pick the SD-Turbo-correct tokenizer if quality is visibly off) unblocks: confirmed end-to-end image fidelity rather than just a pipeline that resolves without throwing.
...
- **The IDE video modality passes the tenant JWT as the `apiKey` prop to `<StudioPanel>`, not a `bfk_*` API key.** [frontend/src/components/IDENew.tsx](frontend/src/components/IDENew.tsx) passes `getStoredTenantToken()` to `<StudioPanel apiKey={...} hideHeader />` when `modality === 'video'`. Works because the LLM gateway's `requireTenantAccess` middleware accepts JWT in the Authorization header, but the prop is mis-named — a third-party npm consumer of `@seanhogg/builderforce-studio` would correctly pass a `bfk_*` key. Same JWT-vs-API-key duality the "JWT auth path is not cached" entry notes. Fixing (rename the StudioPanel prop to `authToken` or accept a union `{ apiKey } | { tenantToken }`, document in-app embedders use the JWT path while npm consumers mint a `bfk_*` key) unblocks: explicit auth-mode signalling.
- **Project modality is not gated by `platform_modules` entitlement — any tenant on any plan can switch a project to the Video modality.** [frontend/src/components/IDENew.tsx](frontend/src/components/IDENew.tsx) renders the modality switcher (from [frontend/src/lib/modality.ts](frontend/src/lib/modality.ts)) for every project; Video has a 6GB-VRAM floor and runs heavy diffusion. The agreed-on Pro+ gate (per AskUserQuestion answer) isn't enforced. Fixing (add a `studio`/`video` row to `platform_modules`, populate default entitlement for Pro+ plans, and have the modality switcher read entitlements — disabling/`·soon`-marking modalities the tenant isn't entitled to, same pattern as the `llm` coming-soon entry) unblocks: revenue protection on the heavy modality.
- **Project modality does not persist server-side — the `modality` field has no worker DB column or `projectRoutes` handling.** [frontend/src/components/IDENew.tsx](frontend/src/components/IDENew.tsx) `handleModalityChange` calls `updateProject(id, { modality })` and [frontend/src/lib/api.ts](frontend/src/lib/api.ts) now permits the field, but the worker's `PUT /api/projects/:id` (in `worker/`) neither accepts nor stores it, and `projects` has no `modality` column. The `.catch` swallows the rejection so the switch still works *for the session*, but reopening the project resets to `designer`. Fixing (add a `modality TEXT DEFAULT 'designer'` column via a worker migration, accept the field in the project update route + select it in fetch) unblocks: modality survives reload, and the IDE opens projects in their last-used mode.
- **The Brain's video prompt is modality-aware but not wired to drive generation — the user must copy the Brain's crafted prompt into the video panel by hand.** [frontend/src/components/AIChat.tsx](frontend/src/components/AIChat.tsx) injects the video persona (from [frontend/src/lib/modality.ts](frontend/src/lib/modality.ts)) so the Brain produces shot-level prompts, and [frontend/src/components/IDENew.tsx](frontend/src/components/IDENew.tsx) already threads `videoPrompt`/`setVideoPrompt` into `<StudioPanel promptValue>`. The missing link: nothing routes the Brain's assistant output into `setVideoPrompt`. Fixing (add an "Use in generator →" action on assistant messages when `modality === 'video'`, calling `setVideoPrompt(content)`; or auto-adopt the latest assistant message as the prompt) unblocks: Brain→generate flow without manual copy-paste, the cohesion the modality vision calls for.
- **The IDE right panel (Files / Train / Publish / State) and Run button are not modality-adapted.** [frontend/src/components/IDENew.tsx](frontend/src/components/IDENew.tsx) keeps the same right-panel tabs and the green Run button across all modalities. In Video mode, `Run` (npm install + Vite dev server) and the Train/Publish/State agent tabs are largely irrelevant; generated clips have nowhere to live in the Files tab. Fixing (drive the right-panel tab set + the primary action button from the modality config — Video shows a clips/frames browser + "Generate" as the primary action; LLM shows dataset/checkpoint tabs) unblocks: each modality's chrome surfaces only what's relevant while staying visually consistent.
...
- **Studio `expandPrompt` calls `BuilderforceClient` with `model: 'auto'` — that's not a real model id.** [studio/src/engine/llm-bridge.ts](studio/src/engine/llm-bridge.ts) passes `model: 'auto'` to the LLM gateway. The gateway's [LlmProxyService.complete()](api/src/application/llm/LlmProxyService.ts) treats unknown ids as hints and substitutes from the candidate chain anyway, so this *works*, but it's relying on undocumented gateway behaviour. A future strict-pin change (already in the gap register: "Gateway-side strict-pin mode for `model` is not implemented") would break it. Fixing (use an explicit model id like `gemini-2.5-flash-lite` or expose a `promptModel` option on `VideoEngineOptions` so the consumer can choose) unblocks: deterministic prompt-expansion behaviour, doesn't break when strict-pin lands.
- **CI `publish-studio` job will still hit `ENEEDAUTH` on the next version bump until OIDC trusted-publisher is configured on the now-existing npm package.** v0.1.0 was published manually, so the package now exists at https://www.npmjs.com/package/@seanhogg/builderforce-studio and the trusted-publisher UI is reachable. Without configuring it, bumping `studio/package.json` `version` and pushing to main will fail the `publish-studio` workflow the same way (OIDC discovery returns no trusted publisher → falls back to traditional auth → no token → `ENEEDAUTH`). Fixing (visit https://www.npmjs.com/package/@seanhogg/builderforce-studio/access → Publishing access → Add trusted publisher → GitHub Actions / SeanHogg / Builderforce.ai / release.yml / `publish-studio` → Save) unblocks: automatic OIDC publishing on every version bump, matching the sdk's existing zero-secret flow. The `NPM_TOKEN` secret fallback in [`.github/workflows/publish-npm-package.yml`](.github/workflows/publish-npm-package.yml) is an alternative if you'd rather use a long-lived token.
