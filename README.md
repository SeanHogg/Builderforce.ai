...
- **Marketing responsive changes verified by typecheck + lint only, not a live browser.** The landing/blog `MarketingHeader` swap and the homepage hero mascot-to-right-column restructure ([page.tsx](frontend/src/app/page.tsx)) pass `tsc` + `eslint` and the dev server compiles the route 200, but the hero streams behind a Suspense boundary so the rendered breakpoints (hamburger ≤820px, mascot hidden on mobile / shown as the right column ≥820px) were not visually confirmed (no Playwright in this env). Fixing (a manual `npm run dev` check at 375/768/1280px or a Playwright snapshot) unblocks: confidence the slide-out menu opens and the mascot column reflows correctly at each breakpoint.
...
- **Product + Agile CRUD features LANDED — 26 of 30 embed views now live.** Shipped + verified (api 137 tests, drift 88 tables, all tsc 0): the tracker factory was extracted to a shared [segmentTrackerRoutes](api/src/presentation/routes/segmentTrackerRoutes.ts) (DRY — used by governance/product/agile); migrations [0059](api/migrations/0059_product_management.sql) (8 Product tables: mvp/validation/roadmap/releases/changelog/feature-flags/business-value/feature-roi) + [0060](api/migrations/0060_agile_survival.sql) (5 Agile tables: sprints/velocity/capacity/cost/feature-scores); [productRoutes](api/src/presentation/routes/productRoutes.ts) + [agileRoutes](api/src/presentation/routes/agileRoutes.ts) (segment-threaded CRUD via the factory); one generic [TrackerSurface](frontend/src/components/governance/TrackerSurface.tsx) + [trackerConfigs](frontend/src/components/governance/trackerConfigs.ts) drives all 21 tracker surfaces; views flipped `available:true`. **Remaining 4 views need special infra, not CRUD:** `poker` + `retros` (realtime collaboration rooms — Yjs/DO, doc 03), `access-reviews` + `vuln-scans` (DevSecOps — need Repo connection + scan/agent integration, doc 07 SEC-8/9). Also deferred: the **`work_items` unified spine** (locked decision — `tasks`/`specs` → specializations) which would let backlog/kanban/sprints/scores cross-link items (the trackers are standalone CRUD today, not yet linked to a shared item graph); and AI enrichment of roadmap/validation (currently manual CRUD). Fixing the 4 surfaces (realtime room infra + repo-agent scanners) and the spine unblocks: full feature parity + cross-feature linkage.
