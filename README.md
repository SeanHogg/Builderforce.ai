...
- **SDK 0.8.0 publish still red after Trusted Publishing OIDC migration — root cause not yet pinpointed.** Latest version live on npm is `0.7.0`. Token-based publish was dismantled this pass (NPM_TOKEN env removed from [release.yml `publish-sdk` job](.github/workflows/release.yml), `registry-url` removed from `setup-node`, npm upgraded via `npm install -g npm@latest` to a Trusted-Publishing-capable version) AND the user configured the Trusted Publisher on npmjs.com matching `seanhogg/builderforce.ai` → `release.yml`. The next attempt (run 3 — 0.8.0, post-upgrade) still failed with `ENEEDAUTH need auth — This command requires you to be logged in`. The previous attempt with `npm install -g npm@^11.5.1` may have silently no-op'd if that version range didn't match a published release — but the workflow log was truncated above the publish step so we couldn't confirm. The workflow now also prints `node --version`, `npm --version`, `npm config get registry`, and OIDC-env presence (true/false, never the token) before the publish step. Next run's output should differentiate between: (a) npm still on 10.x → upgrade strategy needs more force (try `actions/setup-node@v4` `node-version: 22` plus an explicit `npm@latest` install, or fall back to `--auth-type=oidc` flag); (b) npm is 11.x but `ACTIONS_ID_TOKEN_REQUEST_TOKEN` is missing → permissions inheritance issue (job-level `permissions: id-token: write` may need to be set explicitly on `publish-sdk`); (c) both look right → Trusted Publisher config on npmjs.com has a subtle claim mismatch (case sensitivity on the org name, workflow file path including subdirectory, environment scoping). Fixing unblocks: 0.8.0 + this pass's `vendor` / `model` / `resolvedVendor` additions reaching customers, AND the consumer-side `inferVendorFromModel` prefix-heuristic cleanup that depends on those new fields landing on npm.
