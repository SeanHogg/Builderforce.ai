...
Execution + autonomous advancement are now wired and tested (migration 0068 `agent_dispatches`): `SwimlaneCoordinator.startTicket` compiles the lane's agents into dispatches and routes them (claw push via `ClawStageDispatcher`/`CLAW_RELAY`, or browser PULL); `reportDispatchResult` aggregates the stage and autonomously advances the ticket (or routes to needs_attention — never a silent advance). The browser runtime path is built: `POST /api/agent-runtime/claim` + `/result`, the portable runner in `src/application/browserRuntime/runner.ts`, and the server-side git-proxy `POST /api/git-proxy/:repoId/...`. Remaining roadmap items:

- **In-browser CODE execution (WebContainer + git) is not yet wired into the worker tab — the agent LLM loop is.** The user-facing worker tab ([frontend/src/app/agent-worker/page.tsx](frontend/src/app/agent-worker/page.tsx)) claims `browser` dispatches and runs each agent's loop on its own model via the gateway, reporting results that autonomously advance the kanban (built + render-tested; runner/transport unit-tested in [frontend/src/lib/browserRuntime/](frontend/src/lib/browserRuntime/)). What is NOT yet wired: booting a `@webcontainer/api` instance in that tab to actually `git clone` (through the built `/api/git-proxy`), run build/test, and push — so agents currently produce model output but do not yet edit code in-browser. Fixing (mount a WebContainer in the worker tab with COOP/COEP headers and have the runner perform isomorphic-git ops via the git-proxy) unblocks: full in-browser coding, not just in-browser reasoning.
...
- **Embed opt-in records no `consentVersion` and has no consent modal (spec §5.5/5.6 + §7 item 5).** [PUT /api/embed/config](api/src/presentation/routes/embedRoutes.ts) persists only `{enabled, capabilities}` to `tenants.settings.embed`; [EmbedIntegrationSettings.tsx](frontend/src/components/settings/EmbedIntegrationSettings.tsx) is a bare enable+capabilities toggle. The spec's "reuse the existing consent modal, record `consentVersion`" legal posture is unmet (default-OFF is met). Fixing (capture + persist `consentVersion`/`consentedAt`/`consentedBy` on enable, gated behind a consent modal) unblocks: the audited opt-in record the spec's legal posture depends on.
- **Cross-domain API seams (channel 3, spec §4) are not built.** No `/v1/ingest/feedback` route, no BI burn-rate pull (`GET /api/bi/burn-rate`, scope `read:bi.burn`), and no outbound product webhooks (`workitem.released` / `sprint.completed` / `roadmap.published`) exist in `api/src` — they appear only in the specs. Consequently §7's "webhooks HMAC-signed, replay-protected" is vacuously unmet for the BurnRateOS-facing direction (the separate board-sync/github webhook HMAC is unrelated). Fixing (build the ingest route + webhook emitter with per-tenant HMAC + replay nonce) unblocks: the feedback→backlog, cost-aware-agile, and investor/BI coupling the marketing-parity doc promises.
- **Service tokens are not per-scope (spec §2.3 + §7 item 4).** [tenant_api_keys](api/migrations/0043_tenant_api_keys.sql) is per-tenant, rotatable (`revoked_at`), and audited (`last_used_at`) but has **no scope column** — the spec's endpoint-scoped tokens (`ingest:feedback`, `read:bi.burn`) cannot be expressed or enforced. Fixing (add a `scopes` column + scope-check middleware) unblocks: least-privilege S2S tokens for the channel-3 seams above.
- **No `DELETE /v1/admin/segments/:id` DSR/erasure cascade (spec §3.4 + §7 DSR item).** [segmentRoutes.ts](api/src/presentation/routes/segmentRoutes.ts) implements GET/POST/PATCH only; there is no delete-and-cascade endpoint, so the "company deletion / GDPR erasure → cascade delete of all Segment-scoped rows" requirement is unmet. Fixing (a `DELETE` that cascades across every Segment-scoped table, honoring the BurnRateOS DSR queue) unblocks: per-Segment right-to-erasure.
- **`resolveSegment` in-isolate cache is unbounded and never invalidated.** [segmentResolver.ts](api/src/infrastructure/auth/segmentResolver.ts) keeps a module-level `Map` keyed by `(tenant, account, company) → segmentId` with no eviction and no invalidation on segment suspend/archive/delete. Low severity (the id mapping is stable) but: a suspended/archived/deleted segment keeps resolving inside a warm isolate, and the map grows unbounded across the lifetime of an isolate. Fixing (TTL or LRU bound + invalidate on PATCH/DELETE) unblocks: suspension/erasure taking effect without an isolate recycle, and bounded memory.
- **No copy-paste embed snippet is generated anywhere — the spec's "embed snippet rail" became an npm React component.** Spec §5.1 envisioned riding BurnRateOS's existing unified `<script>` embed-snippet rail (a `SystemFeature` row `embed_builderforce`, a snippet subsystem block, a toggle card on `EmbedBurnRateOSPage.tsx`). The implementation instead ships the `@seanhogg/builderforce-embedded` npm package exporting `<BuilderForceEmbed view=… token=… />`; the BuilderForce Settings page exposes only an enable + capabilities toggle, with no generated/copyable install code. A non-developer cannot "obtain a snippet" — integration requires a BurnRateOS **developer** to `npm install` the package and write the component into each thin-shell page, passing an SSO JWT. Fixing (decide: either generate a documented install block in Settings → Embedded Integration, or update doc 05 §5.1 to describe the React-component rail as the real contract) unblocks: a self-serve install path matching the spec's wording.
