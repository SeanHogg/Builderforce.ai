...
### Cloud Agent Boards (added 2026-06-01 — migrations 0064–0067; swimlanes / board-sync / PRD / repos)

- **Swimlane stage start does not yet compile the lane into a workflow or dispatch to a claw.** [boardRoutes.ts](api/src/presentation/routes/boardRoutes.ts) `POST /:boardId/tickets` and [SwimlaneCoordinator.startTicket](api/src/application/swimlane/SwimlaneCoordinator.ts) only create the `ticket_runs` row at the first lane. The pure compiler [compileStage.ts](api/src/application/swimlane/compileStage.ts) produces the `WorkflowTaskSpec[]`, but nothing yet persists those into `workflow_tasks` / `workflows` or dispatches the stage to `env.CLAW_RELAY`. Fixing (wire `compileStage()` output into the workflow engine + runtime dispatch, mirroring [runtimeRoutes.ts](api/src/presentation/routes/runtimeRoutes.ts) CLAW_RELAY usage) unblocks: a ticket actually executing its assigned agents instead of just tracking lifecycle state.
- **Autonomous transition has no automated workflow→coordinator callback.** [SwimlaneCoordinator.onStageComplete](api/src/application/swimlane/SwimlaneCoordinator.ts) is currently invoked only via `POST /tickets/:ticketRunId/advance` with an explicit `workflowStatus`. There is no hook that calls `onStageComplete` automatically when a stage workflow reaches a terminal status. Fixing (emit a coordinator callback from the execution/workflow terminal-state path, keyed by `ticket_runs.current_workflow_id`) unblocks: true hands-off autonomous-mode advancement.
...
- **QA target URL + test credentials are CI-secret-scoped, not per-project app config — and there is no credential *library*.** The harness reads a single `BF_BASE_URL` / `BF_QA_EMAIL` / `BF_QA_PASSWORD` triple from CI env ([qa-e2e/.env.example](qa-e2e/.env.example)), so there's one site-under-test and one login per workspace, configured outside the product. The intended UX is a per-project **QA automation suite**: a user adds a **project**, sets its **root domain URL(s)/environments**, and saves **multiple named test credentials (personas)** — and each AI-generated scenario runs as a chosen persona (e.g. admin creates → viewer verifies read-only), with some scenarios switching actors mid-flow. Missing pieces: (1) `qa_targets` (project_id → root_url + env) and `qa_credentials` (project_id → many; label/role + username + **encrypted** password) tables, plus a `credential_id` (or persona ref) on `qa_flows`/`qa_tests` and `project_id` on tests/runs (today tenant-scoped only); (2) **encrypted-at-rest** credential storage reusing the `INTEGRATION_ENCRYPTION_SECRET` AES-GCM pattern from [integrationRoutes.ts](api/src/presentation/routes/integrationRoutes.ts) — never logged/returned, decrypted only inside the worker to mint a session; (3) login-as-persona in the harness: mint + cache one `storageState` per credential, run each scenario under its assigned persona, and support multi-actor scenarios (multiple browser contexts / re-login within one test); (4) the generator tagging each scenario with the persona/role it needs; (5) a project-page config UI (manage targets + credential library). Fixing unblocks: the headline product — "point QA at my site, give it the logins, and let AI generate + run role-aware scenarios."
