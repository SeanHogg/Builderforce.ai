...
- **Segment tier foundation — LANDED in [migration 0054](api/migrations/0054_segment_tier.sql); follow-on API/UI still open.** The [`segments`](api/src/infrastructure/database/schema.ts) table now exists (uuid PK, `tenant_id` FK, `external_account_id`/`external_company_id`, `status`, `plan`, `is_default`), `tenants` gained `kind` (embedded|direct), `idp_issuer`, `isolation_mode` (single|segmented), `settings`, and every existing tenant was backfilled exactly one default segment. The design invariant (every tenant ALWAYS has ≥1 segment; single-tenant customers get one auto-created default they never see, so `segment_id` can be `NOT NULL` everywhere and both modes share ONE query path — no `if (segmented)` forks, no nullable escape hatch) is now DB-enforced by the partial unique index `uq_segments_one_default_per_tenant`. **Still open:** (a) a create-segment API + lazy `resolveSegment(jwt)` that maps `{tenant, accountId, companyId}` → segment (lazy-create on first sight); (b) UI gating on `isolation_mode` (a shared entitlement check, not a prop-drilled `canSegment`); (c) `segment_members` for per-segment roles. Unblocks: provisioning/onboarding an integrator's end-clients beyond the default segment.
- **No `segment_id` on entity tables + backfill.** The client-bearing tables (`projects`, `specs`, `workflows`, `tasks`-via-project, `ideProjectChats`, `agents`, `llm_usage_log`, `telemetrySpans`, `approvals`, `contributors`, reports…) carry `tenant_id` but nothing below it. Fixing (add a `segment_id` FK to each + a migration backfilling existing rows to a default segment per tenant) unblocks: row-level attribution of work to an end-client.
- **Isolation is hand-written `eq(tenantId)` per query with no RLS backstop — does NOT satisfy the hard-rule requirement for a second dimension.** Isolation is application-side ([authMiddleware:132](api/src/presentation/middleware/authMiddleware.ts) sets `tid`, repositories append `eq(table.tenantId, tenantId)` manually). The "segmented tenants get hard, non-bypassable isolation" rule rules out manual per-query filtering for the segment dimension — a guarantee that depends on every author remembering a filter is not a hard rule. Fixing: **Postgres RLS on Neon as the enforced floor** (session sets `tenant_id` + `segment_id` GUCs; policies refuse cross-segment rows even if app code forgets the filter), with the app still passing both IDs for defense in depth. A shared no-bypass base-repository is the weaker fallback (still discipline — the layer can be bypassed). Unblocks: an isolation guarantee that holds at the layer that can't be bypassed.
- **Token model carries no segment claim.** The tenant JWT is `{sub, tid, role, sid}` ([AuthService.tenantToken](api/src/application/auth/AuthService.ts#L166)) — no segment. Fixing (issue a segment-scoped token with a `seg` claim, or accept a per-request segment header validated against segment membership) unblocks: segment-scoped sessions and key authorization.
- **No segment membership / per-segment roles.** `tenant_members(tenantId, userId, role)` grants a tenant-wide role; a user who should only touch one end-client cannot be scoped. Fixing (add `segment_members`, or a segment list + role on membership) unblocks: per-end-client access control within an integrator's tenant.
