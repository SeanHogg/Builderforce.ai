...
- **`AUTH_CACHE_KV` namespace must be provisioned for the auth cache to take effect.** [keyResolutionCache.ts](api/src/infrastructure/auth/keyResolutionCache.ts) caches `bfk_*` / `clk_*` resolutions for ~60s when the optional binding is present, falling through to a per-request DB hit when unbound. Without it, every chat-completion call pays a ~30-80ms Neon round-trip for auth alone — fine in dev, expensive in prod. Provision once: `npx wrangler kv:namespace create AUTH_CACHE_KV` then add `[[kv_namespaces]] binding = "AUTH_CACHE_KV" id = "<id>"` to [wrangler.toml](api/wrangler.toml). Fixing unblocks: sub-millisecond gateway auth on warm cache.
