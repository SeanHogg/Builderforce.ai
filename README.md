...
- **First embedded widgets are LIVE end-to-end; the rest of the resurface-able set is unwired.** Shipped this pass (build + 118 api / 13 pkg tests green, frontend tsc clean): the SuperAdmin enablement flow ([embedRoutes](api/src/presentation/routes/embedRoutes.ts) `GET/PUT /api/embed/config` → `tenants.settings.embed`, [EmbedIntegrationSettings](frontend/src/components/settings/EmbedIntegrationSettings.tsx) self-gating toggle on `/settings`); the token bridge ([auth.ts `setEmbedAuth`](frontend/src/lib/auth.ts) — embed-mode token + embed-aware 401, one auth path); registry reconciled to reality (`available` flag + `capabilityForView` in [views.ts](builderforce-embedded/src/views.ts), `prd`/`security`/`approvals` added); and the frame self-gates on the host's enabled capabilities then RESURFACES [TaskMgmtContent](frontend/src/components/TaskMgmtContent.tsx) for `kanban` + `backlog`. **Still open:** the other resurface-able views are scaffolds — `ideas`→BrainPanel (mountable: BrainProvider is in the layout, but needs runtime validation), `prd`→PRDsContent (needs a project context — it requires `projectId`/`projectName`), `security`/`approvals` (inline `/security` + `/approvals` page bodies — must be EXTRACTED into mountable components first), `observability`/`workflows` (not in the 3 capability areas). Fixing (extract the page bodies into components, give `prd` a project picker, validate BrainPanel in-frame, then add each to `renderSurface` + flip `available:true`) unblocks: Product (ideas/PRD) + Security (sessions/approvals) widgets, not just Agile/backlog.
