...
- **`frontend/src/lib/webgpu-trainer.ts` `isWebGPUAvailable()` + `requestWebGPUDevice()` duplicate `studio/src/engine/device-router.ts`.** [frontend/src/lib/webgpu-trainer.ts](frontend/src/lib/webgpu-trainer.ts) and [frontend/src/lib/mamba-engine.ts](frontend/src/lib/mamba-engine.ts) each do their own WebGPU probe. With the studio package now exposing the canonical `probeDevice()` (covers WebNN + WebGPU + CPU + label + memory estimate), the frontend should import from `@seanhogg/builderforce-studio/engine` once the package is published, and the two in-tree helpers should be deleted. Currently three implementations of "can I run on GPU?" coexist (DRY-rule violation). Fixing (delete the in-tree probes, switch trainer + mamba-engine to `probeDevice` once studio is published to npm) unblocks: one canonical device-acquisition path, easier upgrade story when WebNN ships properly, no drift between studio and trainer fallback behaviour.
- **`frontend/src/lib/types.ts` `MambaStateSnapshot` duplicates `studio/src/types.ts` `MambaStateSnapshot`.** Same shape declared in two files. Should converge once studio is published — frontend re-exports from `@seanhogg/builderforce-studio` instead of carrying its own copy. Until then the two files MUST stay byte-identical for the shape; persisted v2.0 agent packages already in R2 will break otherwise. Fixing (delete the frontend copy, re-export from studio) unblocks: single source of truth, prevents the two definitions drifting and corrupting stored snapshots.
...
- **Studio panel sends the tenant JWT as the `apiKey` prop in `frontend/src/app/studio/page.tsx`, not a `bfk_*` API key.** [frontend/src/app/studio/page.tsx](frontend/src/app/studio/page.tsx) passes `getStoredTenantToken()` to `<StudioPanel apiKey={...} />`. Works today because the LLM gateway's `requireTenantAccess` middleware accepts JWT in the Authorization header, but the prop is mis-named — a third-party consumer embedding the StudioPanel would correctly pass a `bfk_*` key and the gateway path would diverge silently. Same JWT-vs-API-key duality that the gap-register entry about "JWT auth path is not cached" already notes. Fixing (rename the StudioPanel prop to `authToken` or accept a union `{ apiKey } | { tenantToken }`, and document that in-app embedders use the JWT path while npm consumers use a minted `bfk_*` key) unblocks: explicit auth-mode signalling, the ability to cache the JWT branch separately, and matches the gateway's two-path reality.
- **Studio is not gated by `platform_modules` entitlement — any tenant on any plan can open `/studio`.** [frontend/src/components/Sidebar.tsx](frontend/src/components/Sidebar.tsx) and [frontend/src/app/studio/page.tsx](frontend/src/app/studio/page.tsx) make `/studio` available to every authenticated tenant. The agreed-on Pro+ gate (per AskUserQuestion answer) isn't enforced yet; the README "What is missing" Custom-LLM-Support section already tracks platform-module assignment elsewhere. Fixing (add a `studio` row to `platform_modules`, populate the default entitlement table for Pro+ plans, gate the sidebar entry + the `/studio` page via the existing module-assignment helper) unblocks: revenue protection on the 6GB-VRAM-floor feature, parity with how other paid modules are gated.
...
- **`docker-compose.yml` does not bundle a local Postgres — `api` will fail to boot without an external `NEON_DATABASE_URL` reachable from the container.** [docker-compose.yml](docker-compose.yml) brings up `api` (wrangler dev) + `frontend` (Next.js dev) but assumes the user supplies `NEON_DATABASE_URL` in `api/.dev.vars` pointing at a real Neon or pooled Postgres. A truly offline `docker compose up` would need a `postgres:16` service + an init container that runs `npm run db:migrate`. Out of scope for v0 since the gateway is the bulk of the dep tree and most builders already have a free Neon DB. Fixing (add a `postgres` service with the Builderforce schema initialised via `migrations/`, switch the api service's default `NEON_DATABASE_URL` to point at the in-network host) unblocks: zero-prerequisite `docker compose up`.
- **`Dockerfile.frontend` rebuilds the studio + sdk each image build — there's no host-side `pnpm dev` hot-reload for changes to `studio/src/` or `sdk/src/`.** [Dockerfile.frontend](Dockerfile.frontend) `studio-build` + `sdk-build` stages are baked at image-build time and copied into the deps stage. Editing studio TS requires `docker compose build frontend` to pick up. Two ways to fix: (a) add a `studio-watch` service running `tsup --watch` that bind-mounts `studio/dist` into the frontend container — cleaner but adds a third long-running container, or (b) move studio + sdk into a pnpm workspace at the repo root so `pnpm dev` from the frontend transpiles them via Next's `transpilePackages` directly. Fixing unblocks: edit-studio → reload-page loop without container rebuild.
- **`Dockerfile.api` doesn't pre-install workerd — first `wrangler dev` boot inside the container downloads it on startup (slow first-up).** [Dockerfile.api](Dockerfile.api) `npm install` pulls wrangler but workerd's platform-specific binary downloads lazily. On a cold `docker compose up` this adds ~30s. Fixing (add `RUN npx wrangler --version` to the deps stage to force the workerd download at build time) unblocks: deterministic first-boot latency, no surprise downloads in CI.
