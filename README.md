...
- **Frame interpolation is latent slerp, not optical flow (RIFE/FILM).** This pass shipped keyframe interpolation in [frame-interpolator.ts](studio/src/engine/frame-interpolator.ts): keyframes run the full denoise loop and the frames between them are spherical-linear-interpolated in VAE latent space + one cheap decode each (`interpolationFactor` in [GenerateOptions](studio/src/types.ts)). Latent slerp produces a smooth *morph*, not physically-correct *motion* — fast pans or limb movement between distant keyframes can look like a cross-dissolve rather than real displacement. A true optical-flow interpolator (RIFE / FILM) would estimate per-pixel motion vectors and warp, but needs a separate ONNX model + session (weights, I/O contract, a new entry in `MODEL_REGISTRY`-style registry). Fixing (add a `RifeInterpolator` ONNX backend behind the same `buildInterpolatedSequence` seam, selectable per generation) unblocks: convincing motion on high-`interpolationFactor` clips instead of latent morph.
- **`cameraMoveToMotion` has no latent-zoom primitive — `dolly-in`/`dolly-out` are pan placeholders.** [scene-planner.ts](studio/src/engine/scene-planner.ts) `cameraMoveToMotion` maps pans/tilts to a latent-space directional shift via `shiftLatent`, but there is no latent *scale* operation, so `dolly-in`/`dolly-out` fall back to a small vertical drift + img2img recursion rather than an actual push/pull. Dolly shots therefore read as a slight tilt, not a zoom. Fixing (add a `scaleLatent` center-crop/pad helper in [mamba-coherence.ts](studio/src/engine/mamba-coherence.ts) — sibling to `shiftLatent` — and route dolly moves through it) unblocks: correct dolly/zoom camera language from the storyboard.
- **The VLM frame validator is advisory-only — failures aren't acted on.** [frame-validator.ts](studio/src/engine/frame-validator.ts) + `VideoEngine.generateStoryboard`'s `validate` path return a per-shot verdict (score + issues) and surface it via `onShot`/`validations`, but a failed validation does not trigger re-generation of the shot (e.g. reseed + retry, or nudge the prompt). It also validates only the FIRST keyframe per shot, not every frame, so mid-shot drift is invisible. Fixing (a retry loop in `generateStoryboard` that reseeds a shot whose first keyframe scores below threshold, up to N attempts, and optionally validates the last keyframe too) unblocks: the closed-loop "self-healing" generation the validator was built to enable, instead of a passive report.
- **`generateStoryboard` runs single-pass per shot — no two-pass refinement tier.** [video-engine.ts](studio/src/engine/video-engine.ts) `generate()` honours `refinementModel` (draft → refine), but `generateStoryboard()` does not invoke `refinementPass`, because swapping models per shot would reload weights N times. So cinematic mode can't use the "Refined" quality tier. Fixing (generate all shots with the draft model, then run ONE refinement pass over the whole concatenated latent set after a single model swap) unblocks: the Refined tier for cinematic/storyboard output without N model reloads.
- **Cinematic mode UI surfaces neither the storyboard nor validation results.** [StudioPanel.tsx](studio-embedded/src/components/StudioPanel.tsx) exposes a "Cinematic (auto-storyboard)" toggle that runs `planScene` → `generateStoryboard`, but the resulting `Storyboard` (treatment, character bible, per-shot prompts/camera moves) and any `validations` are discarded except for the treatment shown as the "expanded prompt". The user can't see or edit the shot list before rendering, or see which shots failed validation. Fixing (render the storyboard as an editable shot list pre-render, and a per-shot validation badge post-render) unblocks: director-style review/editing of the plan, the actual point of the planning layer.
