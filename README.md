...
- **Integration-key picker requires MANAGER but repo/board-connection CRUD does not — a plain member sees an empty/erroring key dropdown.** `integrationRoutes` is `requireRole(MANAGER)` while [repoRoutes](api/src/presentation/routes/repoRoutes.ts) + [boardConnectionRoutes](api/src/presentation/routes/boardConnectionRoutes.ts) use bare `authMiddleware`, so a non-manager opening the Source control / Integrations tab can attempt to add a repo/connection but the credential `list()` 403s (the picker silently shows no keys). Fixing (align the role gate — either relax integrations `list` to any member for read, or gate repo/board-connection writes to MANAGER) unblocks: a consistent permission story across the tab.
- **Pre-existing build blocker (NOT from this work): [dashboard/page.tsx](frontend/src/app/dashboard/page.tsx) has a syntax error (~L801, stray `)}`).** An in-progress edit adding `activeTab` Projects/Workforce tabs left an unbalanced JSX conditional; `tsc` fails only on this file (confirming the integration/task-panel work is clean). It blocks `next build`. Fixing (balance the `{activeTab === '…' && ( … )}` wrappers around the two `<section>`s) unblocks: a green production build.
