...
- **No CI-level e2e covers the studio's actual in-browser ONNX execution — eight invariants are unit-tested, the rest needs a real GPU browser.** Eight layers locked by unit tests in [studio/src/engine/](studio/src/engine/): (1) ORT WASM/JS version match (`_OrtGetInputOutputMetadata is not a function`); (2) per-model UNet input names (`input 'X' is missing in 'feeds'`); (3) per-input dtypes (LCM `timestep`=float32, SD-Turbo=int64; `Unexpected input data type`); (4) `reportProgress` fans out (silent-hang UX); (5) `buildOrtSessionOptions` pins `graphOptimizationLevel: 'basic'` (`SimplifiedLayerNormFusion` crash); (6) `checkMemoryForModel` reports when probed memory < minimum AND is currently inert for WebGPU (the API doesn't expose VRAM — `null` is honest); (7) `explainSessionCreateError` translates std::bad_alloc + graph-fusion ORT errors; (8) `lighterModelHint` never suggests the failing model itself as an alternative (regression for the "Try sd-turbo" when sd-turbo IS the failing model). Plus init-time `assertSessionMatchesSpec`. Still needing a real WebGPU browser: (a) UNet output name (we pick first Float32); (b) `timestep_cond` embedding values match LCM training; (c) CLIP-L pad-id assumption holds for SD-Turbo; (d) actual VRAM fit (no API exposes this — only the runtime OOM tells us); (e) progress timeline end-to-end. Need a Playwright job on a GPU runner.
