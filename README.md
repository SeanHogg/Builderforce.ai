...
- **Per-view feature surfaces are scaffolds — each view renders a "connected" placeholder, not the real UI.** [renderSurface](frontend/src/app/embed/[view]/page.tsx) returns a connected scaffold for every view because the feature UIs (kanban board, SOC 2 tracker, ideas, …) depend on the unbuilt `/v1` PM/Agile/Governance API. Fixing (build each surface against `/v1` and register it in `renderSurface`) unblocks: real embedded functionality, not just the wired transport. The transport, token handoff, resize, and deep-link sync are done, so each surface is additive.
