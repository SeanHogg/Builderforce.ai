...
- **No CI-level e2e covers the studio's actual in-browser ONNX execution — three invariants are unit-tested, the rest needs a real GPU browser.** Three layers locked by unit tests: (1) ORT WASM/JS version match ([onnx-runtime-config.test.ts](studio/src/engine/onnx-runtime-config.test.ts), guard for `_OrtGetInputOutputMetadata is not a function`); (2) per-model UNet input names ([diffusion-engine.test.ts](studio/src/engine/diffusion-engine.test.ts), guard for `input 'X' is missing in 'feeds'`); (3) per-input dtypes (same file — LCM `timestep` MUST be float32, SD-Turbo MUST be int64; guard for `Unexpected input data type. Actual: int64, expected: float`). Plus an init-time `assertSessionMatchesSpec` that fails fast with a clear error if the loaded model's `session.inputNames` disagrees with the registry. The layers still needing a real WebGPU browser: (a) exact UNet output name (we currently pick the first Float32 output rather than reading `outputNames`); (b) the LCM `timestep_cond` sinusoidal embedding values match the model's training; (c) the CLIP-L tokenizer's pad-id assumption holds for SD-Turbo; (d) the LCM UNet 1.7GB fp32 + external data actually fits 6GB VRAM. These need a Playwright job on a GPU runner. Fixing unblocks: confirmed end-to-end image fidelity.
...
- **Project modality persists via the worker path only — the `api/` worker's `projectRoutes` (the non-`NEXT_PUBLIC_WORKER_URL` fallback) doesn't carry the `modality` field.** The production path ([worker/src/routes/projects.ts](worker/src/routes/projects.ts) `PUT`, plus the `modality TEXT DEFAULT 'designer'` column in [worker/schema.sql](worker/schema.sql)) now persists modality. But when `NEXT_PUBLIC_WORKER_URL` is unset, `isWorkerForProjects()` routes project writes to the Drizzle-based `api/` worker instead, which has no `modality` column or update handling. Fixing (mirror the column + PUT field into the `api/` Drizzle schema + projectRoutes) unblocks: modality persistence in the API-only deployment topology.
...
- **The IDE center panel has no `llm` branch — enabling the LLM modality today renders the Designer preview/code view, not an LLM workspace.** [frontend/src/components/IDENew.tsx](frontend/src/components/IDENew.tsx) (~line 635) switches the center panel with `modality === 'video' ? <StudioPanel/> : <preview/code>` — only `video` is special-cased, so both `designer` *and* `llm` fall through to the WebContainer preview/code view. The switcher disable is correctly single-sourced (`disabled = !!m.comingSoon` reading [modality.ts](frontend/src/lib/modality.ts)), the Brain already receives `modality` and has an `llm` `brainSystemPrompt`, and the right-panel `train`/`publish`/`state` tabs already map to existing components — but there is no center component for the modality. Fixing (build an `LlmPanel` — dataset builder + train trigger via [frontend/src/lib/webgpu-trainer.ts](frontend/src/lib/webgpu-trainer.ts) + a chat/test surface, then add an `llm` arm to the center-panel switch and a capability map so the switch isn't a growing `mode === x ?` chain) unblocks: flipping `comingSoon` off actually surfaces a working LLM workspace instead of the Designer view. Depends on the existing entries for the Mamba kernel placeholder and the studio-weights upload.
- **README documents three distinct "LLM" surfaces as if they were one, with no cross-reference.** The platform has (A) the **LLM Gateway** — a shipped multi-vendor hosted proxy under [api/src/application/llm/](api/src/application/llm/) (the subject of most of this register and [PRD-builderforce-llm-gateway.md](PRD-builderforce-llm-gateway.md)), already used by the studio for Video prompt-expansion via [studio/src/engine/llm-bridge.ts](studio/src/engine/llm-bridge.ts); (B) **Custom LLM Support** — serving fine-tuned LoRA agents to the CoderClaw CLI via `/api/agents/:id/chat` (the "## Custom LLM Support" section above, P0 gaps still open); and (C) the **in-IDE `llm` modality** — the "soon" tab in [frontend/src/lib/modality.ts](frontend/src/lib/modality.ts), intended for *building + training* a custom model. The README never states these are separate, so a reader scopes the in-studio `llm` modality (C) against the CLI-serving (B) spec. Fixing (a short "Three LLM surfaces" subsection naming A/B/C with code links) unblocks: correct roadmap scoping for the in-studio LLM modality.
