...
- **SOC 2 Control Tracker is LIVE end-to-end (first Security-pillar widget); the rest of governance is schema-or-less.** Shipped + verified (api 125 tests, drift 69 tables, all tsc 0): schema [0057](api/migrations/0057_governance_compliance.sql) (`soc_controls`/`soc_evidence`/`security_vendors`/`security_incidents`); the **fully Segment-THREADED** [governanceRoutes](api/src/presentation/routes/governanceRoutes.ts) (`/api/governance/soc2/*` — list/seed-baseline/patch-status/add-evidence, every read+write scoped by `(tenantId, segmentId)` from context, 7 tests); [Soc2Content](frontend/src/components/governance/Soc2Content.tsx) (readiness scoreboard, seed, per-control status) wired into the embed at `soc2` (`available:true`). **Still open:** (a) `vendors` + `incidents` have tables but no routes/surfaces yet; (b) the remaining doc-07 tables — `pii_data_assets`, `security_dpas`, `security_trainings`, `compliance_events`, `data_subject_requests`, `data_suppression_list`, `access_reviews`, `vulnerability_scans`+`findings`, `security_audit_log`; (c) SEC-1 niceties not built — the evidence-attach UI (the `POST .../evidence` API exists, no UI), auditor-ready JSON export, and per-doc-07 the `SecurityAuditLog` write on every governance mutation; (d) the wired `soc2` surface is type-checked but NOT runtime-validated in-frame (same as the other 4 widgets). Fixing unblocks: the full Security pillar.
