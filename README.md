...
- **`@seanhogg/builderforce-studio` is not yet authorised for npm Trusted Publishing — `publish-studio` CI job fails with `ENEEDAUTH` on first publish.** The OIDC flow used by [`.github/workflows/release.yml`](.github/workflows/release.yml) `publish-studio` works for `@seanhogg/builderforce-sdk` because that package has a trusted publisher configured on npmjs.com. The new `@seanhogg/builderforce-studio` package has never been published, so npm has no trusted-publisher config and rejects the OIDC token. Chicken-and-egg: trusted publishing for a brand-new package requires either (a) a one-time manual publish to register the name, or (b) registering the trusted publisher on the *unpublished* package via npmjs.com → Add Package → trusted publishers. Fixing (operator action: on https://www.npmjs.com create the `@seanhogg/builderforce-studio` package entry and add trusted publisher: GitHub Actions / SeanHogg / Builderforce.ai / release.yml / `publish-studio`) unblocks: the studio actually gets published, third-party consumers can `npm install` it.
