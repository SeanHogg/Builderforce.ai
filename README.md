...
- **No CI-level e2e covers the studio's actual in-browser ONNX execution — only the version-match invariant is unit-tested.** The ORT WASM/JS version-match (the root of the `_OrtGetInputOutputMetadata is not a function` bug) is now locked by [studio/src/engine/onnx-runtime-config.test.ts](studio/src/engine/onnx-runtime-config.test.ts). But the layers that need a real WebGPU browser + CDN fetch remain unverified by CI: (a) text-encoder `input_ids` is `int32` (could be `int64`); (b) UNet `timestep` is `int64` scalar (could be `float32`); (c) UNet input names are `sample`/`timestep`/`encoder_hidden_states` and output is `out_sample`; (d) the CLIP-L tokenizer pads with eot=49407 which matches SD1.5 (LCM) but SD2.1/SD-Turbo historically padded with 0; (e) the LCM UNet is fp32 + external data (~1.7GB) so the 6GB-VRAM floor is optimistic. These can't run on standard CI runners (no GPU). Fixing (a Playwright job on a GPU-enabled runner that opens `/ide` in Video modality, generates an 8-frame clip, asserts each ORT `session.inputNames`/`outputNames`, and corrects any dtype/name mismatch) unblocks: confirmed end-to-end image fidelity in CI rather than just "the pipeline resolves without throwing" + a unit-tested version pin.
