...
- **`@builderforce/sdk` is not published to npm.** `npm view @builderforce/sdk` returns 404; the package sits at v0.1.0 in [sdk/](sdk/) with `prepublishOnly: npm run build` wired but never run against the registry. [sdk/package.json](sdk/package.json) is also missing `publishConfig.access: "public"` (required for first-time publish of a scoped package on a free npm org), `repository`, `homepage`, `bugs`, and `keywords`. Until published, tenant apps can only consume via `npm install file:../Builderforce.ai/sdk` or git URL — blocks hired.video / burnrateos migrations from PRD §5.
