...
- **Workflow runs are not linked back to their source definition.** A run record ([workflows](api/src/infrastructure/database/schema.ts)) carries `projectId` + `agentHostId` but no `workflowDefinitionId`, so the unified Workflows page ([WorkflowsContent.tsx](frontend/src/components/WorkflowsContent.tsx)) can only show one flat "Recent runs" list and cannot show per-workflow run history / counts on each definition card the way it should (e.g. "12 runs · 2 failed"). Runs of a project-bound definition now inherit its `projectId` (0093, threaded through [instantiateRun](api/src/application/workflow/instantiateRun.ts) + all trigger/swimlane callers), so project-level grouping works — but definition-level grouping needs a `workflow_definition_id` FK on `workflows`, set by `instantiateWorkflowRun`, plus a `GET /api/workflow-definitions/:id/runs` endpoint. Fixing unblocks: a per-workflow run-history panel + run counts/last-status on each workflow card, and a "View runs" action scoped to one workflow.
...
- **Existing IDE projects created before this pass have empty R2 template files — no backfill.** This pass made template seeding fire on *all three* creation paths ([seedProjectTemplate](api/src/presentation/routes/projectRoutes.ts) now called from `POST /`, `POST /upsert`, and `POST /scaffold`, gated to the `designer` modality with no connected repo). Previously only `POST /` seeded, so projects created via `/scaffold` (the "describe your idea" flow — e.g. `burnrateos`, `Hired.Video`) opened with the 5 template *paths* present but **empty content**, and the IDE Run pipeline fell back to run-only defaults in [IDENew.tsx](frontend/src/components/IDENew.tsx) that were never persisted. The forward fix does NOT repair already-created projects. Fixing (a one-shot backfill that, for each existing `designer`/no-repo project whose `ide/projects/{id}/` objects are missing or zero-length, writes the `VANILLA_TEMPLATE` contents — runnable as an admin endpoint or a `scheduled()` one-off; must be non-destructive: only fill missing/empty objects, never overwrite user content) unblocks: existing projects opening runnable instead of empty.
...
- **Cloud-agent telemetry is emitted from the Worker only (coarse-grained) — the V2 agent-runtime tool loop doesn't push per-tool events.** This pass made cloud agents observable end-to-end: [migration 0092](api/migrations/0092_cloud_agent_telemetry.sql) made `tool_audit_events`/`usage_snapshots.agent_host_id` nullable and added `cloud_agent_ref` + `execution_id`; [runCloudExecution](api/src/presentation/routes/runtimeRoutes.ts) now writes a `context.prepare` + `llm.complete` tool-audit event and a usage snapshot per run; [`GET /api/runtime/agents/:ref/tool-audit`](api/src/presentation/routes/runtimeRoutes.ts) + the extended `/executions/:id/trace` read them; the unified [ObservabilityContent](frontend/src/components/ObservabilityContent.tsx) renders them on the timeline. **What's still coarse:** the V1 Worker path is a single gateway LLM call, so it emits ~2 events; V2 cloud agents that run the full Claude-Agent-SDK tool loop in [agent-runtime](agent-runtime/src) execute many real tool calls that are NOT yet pushed with `cloud_agent_ref`/`execution_id`. There is also no *ingest* endpoint for an external cloud runtime to POST events (the existing [agent-host tool-audit ingest](api/src/presentation/routes/agentHostRoutes.ts) is authenticated by agent-host API key, which cloud agents don't have). Fixing (a cloud-run-authenticated `POST /api/runtime/executions/:id/tool-audit` ingest, plus emitting per-tool events from the agent-runtime cloud executor the way self-hosted hosts push over the relay) unblocks: full-fidelity cloud-agent timelines matching self-hosted granularity, not just the two run-boundary events.
- **Cloud log view is polled, not streamed; and the standalone `/timeline` route is now redundant with the consolidated Observability "Timeline view".** (a) Cloud agents have no live relay socket, so the unified Log view derives cloud log lines from polled tool-audit telemetry with a manual "Refresh cloud" button ([ObservabilityContent.tsx](frontend/src/components/ObservabilityContent.tsx)) rather than a live tail; a per-execution WS stream (the cloud equivalent of `logs.subscribe`) would make cloud logs live. (b) [app/timeline/page.tsx](frontend/src/app/timeline/page.tsx) still renders `ObservabilityContent initialView="timeline"` as its own route, but the Observability page now holds both views behind the in-component Log/Timeline toggle and the sidebar only links `/observability`; the `/timeline` route is an orphan kept to avoid breaking deep links. Fixing (add a cloud execution log stream; and either remove `/timeline` or redirect it to `/observability`) unblocks: live cloud logs and a single observability entry point.
