...
- **Swimlane→board columns are bound to the fixed `task_status` enum — swimlanes can rename/reorder/agent-assign columns but can't create genuinely new ones.** The task board now renders its columns directly from the project's swimlanes (`boardColumns` in [TaskMgmtContent.tsx](frontend/src/components/TaskMgmtContent.tsx)), seeded 1:1 from the statuses on board creation ([defaultSwimlanes.ts](api/src/application/swimlane/defaultSwimlanes.ts) via [boardRoutes.ts](api/src/presentation/routes/boardRoutes.ts) POST) and editable in [BoardConfigPanel.tsx](frontend/src/components/board/BoardConfigPanel.tsx) `LanesTab`. But because `tasks.status` is the hard `task_status` pgEnum ([schema.ts](api/src/infrastructure/database/schema.ts)), each lane must map to one of the 7 statuses via its `key`: (a) lanes whose key isn't a status (`isTaskStatus` false) don't appear as columns; (b) only the first lane per status renders (dedupe), so two lanes mapped to the same status hides the second; (c) there's no position-reorder UI in `LanesTab` yet, so column order is fixed at creation/`position`. Truly arbitrary user-defined columns would need tasks decoupled from the status enum (a `swimlane_id` FK on `tasks` + dispatch/migration changes). Fixing (add a lane-reorder control writing `position`; and/or migrate tasks to lane assignment) unblocks: free-form board structures beyond the 7 statuses, multiple lanes per status, and drag-to-reorder columns.
...
Shipped: create a cloud agent on [/workforce](frontend/src/app/workforce/page.tsx) ([CloudAgentsSection](frontend/src/components/workforce/CloudAgentsSection.tsx)), declare runtime support (cloud / claw / both + best-experience hint), publish to the marketplace with a price (flat-fee or consumption), and manage (edit / unpublish / delete). "Register claw" renamed to "Register remote agent". Backed by tenant-scoped CRUD in [workforceRoutes.ts](api/src/presentation/routes/workforceRoutes.ts) + columns on `ide_agents` (migration 0075: nullable `project_id`, `tenant_id`, `price_cents`/`pricing_model`/`price_unit`, `runtime_support`/`preferred_runtime`, `published`). Verified by `tsc` (api + frontend) + `check:schema`. Remaining roadmap items:
