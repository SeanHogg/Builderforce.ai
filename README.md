...
- **`studio/src/engine/diffusion-engine.ts` `embedPrompt()` returns a zero-vector stub instead of running a real CLIP BPE tokenizer + text encoder.** [studio/src/engine/diffusion-engine.ts](studio/src/engine/diffusion-engine.ts) — the function wires through end-to-end and the text-encoder ONNX session loads, but tokenisation requires a CLIP BPE implementation (vocab + merges from the tokenizer.json that ships with the model) plus a text-encoder forward pass. Left stubbed so the engine compiles and the rest of the pipeline (denoise → coherence → mux) can be smoke-tested with a deterministic null embedding. Fixing (port a minimal CLIP BPE tokenizer — ~200 LoC — or import `@xenova/transformers` AutoTokenizer as a peerDep) unblocks: actual prompt-driven generation, classifier-free guidance for SD-Turbo, the entire feature being useful.
...
- **`studio/src/engine/diffusion-engine.ts` LCM scheduler is a linear-sigma placeholder, not the LCMScheduler from diffusers.** [studio/src/engine/diffusion-engine.ts](studio/src/engine/diffusion-engine.ts) `sigmaForStep()` and `stepToTimestep()` use linear interpolation. LCM requires the trained noise schedule from the model's `scheduler_config.json` (`beta_schedule: 'scaled_linear'`, `prediction_type: 'epsilon'`, plus the LCM-specific consistency-model jump rule). With the placeholder the 4-step output will be noticeably worse than a paper-faithful LCM. Fixing (port the LCMScheduler step formula from diffusers — small, ~50 LoC — and load `scheduler_config.json` as a fourth weight file) unblocks: production-quality LCM output, accurate guidance for the SD-Turbo Euler step.
