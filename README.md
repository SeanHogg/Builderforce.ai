...
- **Resolution is fixed at 512×512; the StudioPanel UI has no control to drop to 256×256.** [studio/src/engine/video-engine.ts](studio/src/engine/video-engine.ts) `DEFAULT_WIDTH/HEIGHT = 512` and the StudioPanel doesn't surface width/height as user-tunable. When a device hits Windows D3D12 TDR (`DXGI_ERROR_DEVICE_HUNG`) during a denoise step — common on integrated GPUs or low-memory devices — the only recourse is to switch to a lighter model or accept failure. A resolution slider (256/384/512/768) would cut per-step compute 4× at the lowest setting and resolve most TDR cases. Fixing (add `width`/`height` controls to [studio-embedded/src/components/StudioPanel.tsx](studio-embedded/src/components/StudioPanel.tsx) + thread through to VideoEngine.create; default to 256 when the device label suggests an integrated/mobile GPU) unblocks: tiny-sd usable on more GPUs without per-frame TDR.
