...
- **No CI-level e2e covers the studio's actual in-browser ONNX execution — seven invariants are unit-tested, the rest needs a real GPU browser.** Seven layers locked by unit tests in [studio/src/engine/](studio/src/engine/): (1) ORT WASM/JS version match (`_OrtGetInputOutputMetadata is not a function`); (2) per-model UNet input names (`input 'X' is missing in 'feeds'`); (3) per-input dtypes (LCM `timestep`=float32, SD-Turbo=int64; `Unexpected input data type`); (4) `reportProgress` fans out to both `console.info` AND consumer (silent-hang UX); (5) `buildOrtSessionOptions` pins `graphOptimizationLevel: 'basic'` (`SimplifiedLayerNormFusion` crash); (6) `checkMemoryForModel` fails fast when probed memory < model minimum (opaque `std::bad_alloc` ERROR_CODE 6); (7) `explainSessionCreateError` translates std::bad_alloc + graph-fusion ORT errors into actionable diagnostics. Plus init-time `assertSessionMatchesSpec` for registry/model drift. Layers still needing a real WebGPU browser: (a) UNet output name (we pick the first Float32 output); (b) `timestep_cond` embedding values match LCM training; (c) CLIP-L pad-id assumption holds for SD-Turbo; (d) actual fit within `approxMemoryMb` (the pre-flight check is approximate); (e) progress timeline end-to-end. Need a Playwright job on a GPU runner.
