...
- **agent-runtime now depends on `@builderforce/memory@^2026.5.31` but that package is not yet published to npm — `pnpm install` will 404 until publish.** This pass rewired the SSM-memory consumer ([agent-runtime/src/infra/ssm-memory-service.ts](agent-runtime/src/infra/ssm-memory-service.ts) dynamic import + [agent-runtime/package.json](agent-runtime/package.json) dep) from the old `@seanhogg/ssmjs` to the consolidated runtime package `@builderforce/memory` (built + 232 tests green in the `builderforce-memory` repo). But `npm view @builderforce/memory` is a 404 — the new `@builderforce/*` scope has never been published (the `builderforce-memory` repo's release workflow needs an `npm login`/token + tag push, an outward-facing action). Until it's published, `agent-runtime`'s lockfile cannot be regenerated and a clean `pnpm install` there will fail to resolve the dependency. The dynamic import is wrapped in try/catch so the *runtime* degrades to memory-only rather than crashing, but install itself is hard-blocked. Fixing (publish `@builderforce/memory-engine` then `@builderforce/memory` at `2026.5.31` to npm, then run `pnpm install` in `agent-runtime` to refresh the lockfile and drop the stale `@seanhogg/ssmjs`/`mambakit`/`mambacode.js` lock entries) unblocks: agent-runtime installing and loading its SSM hippocampus from the canonical npm package.
...
- **Token-cost audit (2026-06-04): agent-runtime does not enable Anthropic prompt caching.** A token-usage review (paired with adding system-prefix caching + a read-through `CachingBridge`/`ResponseCache` to the `@builderforce/memory` library) found the agent-runtime cortex calls never emit `cache_control: {type:'ephemeral'}`. [agent-runtime/src/agents/pi-embedded-runner/extra-params.ts](agent-runtime/src/agents/pi-embedded-runner/extra-params.ts) resolves a `cacheRetention` → short/long TTL but it never becomes an actual Anthropic cache breakpoint; [agent-runtime/src/agents/builderforcellm-local-stream.ts](agent-runtime/src/agents/builderforcellm-local-stream.ts) sends system prompt + tools + RAG context unbounded every request. The system prompt is already byte-stable (only `userTimezone` injected — [system-prompt.ts](agent-runtime/src/agents/system-prompt.ts)) and tool order is deterministic, so the prefix is cache-ready; it just isn't marked. Fixing (mark the system + tool blocks `ephemeral` on the Anthropic upstream, gated by `cacheRetention`) unblocks: ~30–40% input-token cut on multi-turn coding sessions. Highest-$ item in the audit.
- **Token-cost audit: agent-runtime has the SSM hippocampus wired but no InferenceRouter offload.** [agent-runtime/src/infra/ssm-memory-service.ts](agent-runtime/src/infra/ssm-memory-service.ts) loads `@builderforce/memory` and exposes `recallSimilar()`/`learn()`, and the local amygdala (Qwen3-0.6B) does simple/complex triage ([builderforcellm-local-stream.ts](agent-runtime/src/agents/builderforcellm-local-stream.ts)), but the library's `InferenceRouter` (perplexity/length/complexity → SSM-vs-cortex) is not used to short-circuit cheap queries to the local SSM before paying for the cortex. Fixing (route through `InferenceRouter`, or check `recallSimilar` similarity > threshold before escalating) unblocks: cortex-call avoidance on repeated/simple queries on top of the existing amygdala triage.
- **Token-cost audit: agent-runtime compaction is reactive, not proactive.** [agent-runtime/src/agents/compaction.ts](agent-runtime/src/agents/compaction.ts) has the machinery (`estimateMessagesTokens`, `chunkMessagesByMaxTokens`, `stripToolResultDetails`) but it only fires on context overflow; the tool loop in [builderforcellm-local-stream.ts](agent-runtime/src/agents/builderforcellm-local-stream.ts) resends the full growing message list every round. Fixing (summarize completed sub-tasks after N tool rounds — using the hippocampus to distill — rather than waiting for window overflow) unblocks: ~20–30% per-turn savings on long tool-heavy runs.
- **Token-cost audit: the gateway loses Anthropic cache-token accounting, so cache hits are mis-metered.** The usage logger normalizes vendor usage to `prompt_tokens`/`completion_tokens` ([api/src/application/llm/vendors/types.ts](api/src/application/llm/vendors/types.ts) `pickUsage`, [api/src/presentation/routes/llmRoutes.ts](api/src/presentation/routes/llmRoutes.ts) `logUsage`, [schema.ts](api/src/infrastructure/database/schema.ts) `llmUsageLog`) and never reads `cache_creation_input_tokens` / `cache_read_input_tokens`. Anthropic bills cache reads at ~10% of input price; collapsing them into `prompt_tokens` means cached input is metered/charged at up to ~9× the real cost. Fixing (extend `VendorUsage` + `pickUsage` to capture both cache fields, add `cache_creation_tokens`/`cache_read_tokens` columns, and bill `cache_read × 0.1`) unblocks: correct cost accounting once caching is enabled upstream — and is a prerequisite for the agent-runtime caching item above to actually show up as savings.
- **Token-cost audit: the gateway strips `cache_control` from forwarded requests.** `stripStandardFields()` / `STANDARD_BODY_FIELDS` in [api/src/application/llm/LlmProxyService.ts](api/src/application/llm/LlmProxyService.ts) drop `cache_control` as a non-standard field before vendor dispatch, so a tenant (e.g. agent-runtime) that marks a cacheable prefix has it discarded at the gateway. Fixing (add `cache_control` to the preserved field set and pass it through to Anthropic/OpenRouter outbound bodies; ensure the double `JSON.stringify` in the dispatch path doesn't reorder keys and bust the prefix) unblocks: tenant-set prompt caching surviving the proxy. Pairs with the metering item above.
- **Token-cost audit: the gateway's PREMIUM Anthropic model is the retired `claude-3.7-sonnet`.** [api/src/application/llm/vendors/openrouter.ts:109](api/src/application/llm/vendors/openrouter.ts) registers `anthropic/claude-3.7-sonnet` as the PREMIUM-tier Anthropic model. Claude 3.7 Sonnet (`claude-3-7-sonnet-20250219`) was retired on the first-party API 2026-02-19; it may still route via OpenRouter's own lifecycle but is stale and weaker-per-dollar than `claude-sonnet-4-6`. Fixing (update the catalog entry to `anthropic/claude-sonnet-4-6`, and add `claude-haiku-4-5` for the cheap tier; verify OpenRouter's id strings) unblocks: current-generation quality at standard pricing and no surprise de-listing of the premium tier.
- **Token-cost audit: gateway has no `max_tokens` cap and no LLM response cache.** `max_tokens` is forwarded unvalidated ([api/src/presentation/routes/llmRoutes.ts](api/src/presentation/routes/llmRoutes.ts)) — a client can request 1M output tokens; add a per-plan ceiling (Free 4K / Pro 8K / Teams 16K). Separately, the idempotency-key guard detects replays but explicitly does not cache/replay the response body, and `getOrSetCached` ([api/src/infrastructure/cache/readThroughCache.ts](api/src/infrastructure/cache/readThroughCache.ts)) is used for auth/workflow lookups but not for LLM responses; deterministic re-runs re-bill the vendor. Both lower-$ than the caching/metering items but cheap to close. Fixing unblocks: bounded worst-case spend and dedup of identical completions.
