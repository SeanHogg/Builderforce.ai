...
- **No CI-level e2e covers the studio's actual in-browser ONNX execution — five invariants are unit-tested, the rest needs a real GPU browser.** Five layers locked by unit tests in [studio/src/engine/](studio/src/engine/): (1) ORT WASM/JS version match (guard for `_OrtGetInputOutputMetadata is not a function`); (2) per-model UNet input names (guard for `input 'X' is missing in 'feeds'`); (3) per-input dtypes (LCM `timestep` MUST be float32, SD-Turbo MUST be int64; guard for `Unexpected input data type`); (4) `reportProgress` fans out to both `console.info` AND the consumer callback (guard for the silent-hang UX where the engine appeared frozen at "Generating frames…"); (5) `buildOrtSessionOptions` pins `graphOptimizationLevel: 'basic'` on every device path (guard for the `InsertedPrecisionFreeCast / SimplifiedLayerNormFusion` session-create crash). Plus init-time `assertSessionMatchesSpec` that fails fast on registry/model drift. Layers still needing a real WebGPU browser: (a) UNet output name (we pick the first Float32 output rather than reading `outputNames`); (b) the LCM `timestep_cond` sinusoidal embedding values match the model's training; (c) the CLIP-L tokenizer's pad-id assumption holds for SD-Turbo; (d) the LCM UNet 1.7GB fp32 + external data actually fits 6GB VRAM; (e) progress timeline observable end-to-end through a real generation. Need a Playwright job on a GPU runner.
