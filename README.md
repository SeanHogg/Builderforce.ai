...
- **`@seanhogg/builderforce-sdk` v0.3.0 is publish-ready but not yet pushed to npm.** [sdk/package.json](sdk/package.json) carries the v0.3.0 surface (tool calling, structured output, vision, embeddings, per-call timeout/signal/idempotency, metadata) under the `@seanhogg/*` scope, with `publishConfig.access: "public"`, repo metadata, and bundled LICENSE. Remaining step needs interactive credentials this assistant doesn't have: `cd sdk && npm login && npm publish`. Until then tenant apps consume via `npm install file:../Builderforce.ai/sdk` or git URL.
