...
- **First-party server-side MCP server covers projects + tasks only — other domains not yet wired.** A built-in MCP server now exposes platform capabilities in-process through the gateway ([api/src/application/llm/builtinMcpService.ts](api/src/application/llm/builtinMcpService.ts), merged into `GET /v1/mcp/tools` + dispatched in `POST /v1/mcp/call` under the `builtin` extension id in [llmRoutes.ts](api/src/presentation/routes/llmRoutes.ts)), so external/headless MCP clients and the agent-runtime can call platform actions, not just the browser Brain (which skips the `builtin` set via [McpExtensionsBridge](frontend/src/components/brain/McpExtensionsBridge.tsx) since it has the richer client manifest). The advertise/dispatch/tenant-scoping wiring is generic, but the `CATALOG` currently maps only **projects** and **tasks** (full CRUD) to their application services; the remaining frontend-manifest domains (workflows, specs, approvals, agents, boards, cron, prompts, …) are not yet exposed server-side — many are route-level rather than clean tenant-scoped services, so each needs its own catalog entry + service/repo call. Fixing (extend `CATALOG` per domain, reusing each domain's service/repo with a tenant guard) unblocks: full server-side parity with the browser Brain's capability surface for headless/cron MCP consumers.
...
- **Some background internal `ideProxy().complete()` callers still don't record usage (tenant-context not threaded to the LLM call site).** Usage attribution is now broad: the gateway chat/messages paths tag `{ agentHostId }`, the cloud loop tags `cloud_agent_ref`/`execution_id`, image-gen is attributed, every row is priced (0097), and [SecurityReviewService](api/src/application/security/SecurityReviewService.ts) records via the shared [recordProxyUsage](api/src/application/llm/usageLedger.ts) helper. **Still unrecorded:** [QaGeneratorService](api/src/application/qa/QaGeneratorService.ts), [ArchitectAnalysisService](api/src/application/repoanalysis/ArchitectAnalysisService.ts), [BrainService](api/src/application/brain/BrainService.ts) (builds its proxy from a bare apiKey, no full `Env`/`tenantId` at the call), and the workflow [cloudExecutor](api/src/application/workflow/cloudExecutor.ts) `llm` node — each calls `complete()` without `tenantId`/`db`/full-`env` in scope, so wiring `recordProxyUsage` needs that context threaded in first. Separately, the IDE chat ([ideAiRoutes](api/src/presentation/routes/ideAiRoutes.ts)) is *streamed*, so its usage can't be captured without the SSE-tail interceptor (the streaming-usage gap). Fixing (thread `tenantId`+`db`+`env` into those services and call `recordProxyUsage` after `complete()`) unblocks: those low-volume background calls appearing in the ledger instead of being invisible. Lower priority — they're system/background, not the cloud-vs-on-prem agent traffic the dashboard splits.
