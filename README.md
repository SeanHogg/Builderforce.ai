...
- **SDK 0.8.0 publish blocked on npmjs.com-side Trusted Publisher config — must be added on the package, not the account/org.** Latest version live on npm is still `0.7.0`. Verbose-log diagnosis pinpointed the failure: npm CLI 11.15.0 successfully mints the OIDC token from GitHub (`GET .../idtoken/...?audience=npm:registry.npmjs.org 200`), POSTs it to `https://registry.npmjs.org/-/npm/v1/oidc/token/exchange/package/@seanhogg%2fbuilderforce-sdk`, and receives `404 — "OIDC token exchange error - package not found"`. The 404 doesn't mean the package is missing (0.7.0 is live); it means **npmjs.com has no Trusted Publisher entry keyed to `@seanhogg/builderforce-sdk` specifically**. A previously-added Trusted Publisher likely targets the user/org account or a sibling package. Workflow side ([release.yml `publish-sdk`](.github/workflows/release.yml)) is now stable on direct `npm publish --provenance --access public` (preceded by `npm install -g npm@latest` for OIDC support) with an inline `npm view ... && skip || publish` guard — `JS-DevTools/npm-publish@v3` was tried and rejected because it requires a `token` input and doesn't support OIDC. Fixing: log in to npmjs.com → navigate to **the package itself** at https://www.npmjs.com/package/@seanhogg/builderforce-sdk/access → "Publishing access" → "Add trusted publisher" → Publisher: GitHub Actions, Organization: `SeanHogg` (exact case — OIDC `repository` claim is case-sensitive), Repository: `Builderforce.ai`, Workflow filename: `release.yml`, Environment: blank. Fixing unblocks: 0.8.0 + this pass's `vendor` / `model` / `resolvedVendor` additions reaching customers, AND the consumer-side `inferVendorFromModel` prefix-heuristic cleanup that depends on those new fields landing on npm.
