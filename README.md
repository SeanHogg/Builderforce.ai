...
- **SDK publish workflow is broken — `NPM_TOKEN` no longer has publish permission for `@seanhogg/builderforce-sdk`.** Latest version live on npm is `0.7.0`. Two consecutive publish attempts have now failed identically: 0.9.0 (2026-05-22, original CI run) and 0.8.0 (2026-05-22, re-aligned to npm baseline by this pass) — both errored with `404 Not Found - PUT https://registry.npmjs.org/@seanhogg%2fbuilderforce-sdk` in [release.yml `publish-sdk` job](.github/workflows/release.yml) → [publish-if-new.mjs](sdk/scripts/publish-if-new.mjs). Diagnosis: the `Signed provenance statement` line above the error proves OIDC + `id-token: write` are working (provenance signing requires an OIDC token); the failure is on the PUT itself, whose auth comes from `NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}` ([release.yml:138](.github/workflows/release.yml#L138)) — `setup-node`'s `registry-url` writes that into `.npmrc` as `_authToken`. 404 (rather than 401) is npm's masked "permission denied" — the token authenticates but lacks write access to this scoped package. Since 0.7.0 was published successfully via this same workflow, root cause is one of: (a) `NPM_TOKEN` expired or was rotated and the repo secret is stale (npm automation tokens default to 365-day expiry; granular tokens 30 days), (b) the token was reissued as a granular access token without `@seanhogg/builderforce-sdk` write permission, or (c) the package's "Publishing access" on npmjs.com was switched to "Require 2FA" / "Trusted Publishers only" without completing the migration. SDK 0.8.0 (this pass — adds top-level `vendor` + `model` on `BuilderforceApiError`, `resolvedVendor` in `_builderforce`, `x-builderforce-vendor` header) will remain stuck until this is unblocked. Fixing (pick one — both close this gap): (1) regenerate `NPM_TOKEN` via npmjs.com → Access Tokens → "Generate new token" → Automation type → confirm `@seanhogg/builderforce-sdk` is in the package allowlist, then update the GitHub repo secret; (2) complete the Trusted Publishing migration on npmjs.com (package → Publishing access → Add trusted publisher → GitHub Actions / SeanHogg / Builderforce.ai / release.yml / `publish-sdk` env) AND drop `NODE_AUTH_TOKEN` from the publish step + remove `registry-url` from the `setup-node` step so `.npmrc` has no `_authToken` line and the CLI falls through to OIDC for publish-auth. Unblocks: customers picking up gateway features as soon as they land instead of being pinned to 0.7.0 forever, AND the consumer-side `inferVendorFromModel` prefix-heuristic cleanup that depends on the new error fields landing on npm.
