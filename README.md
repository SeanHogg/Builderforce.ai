...
- **SDK 0.8.0 not yet published to npm.** [sdk/package.json](sdk/package.json) bumped the version + raised `defaultTimeoutMs` from 60s to 180s so customer-side calls survive the gateway's premium routing path (per-vendor 60s × 3 PREMIUM attempts). Tags / npm publish still pending — customers consuming `@seanhogg/builderforce-sdk@^0.7` will continue to see 60s outer timeouts and won't benefit from the premium cascade until they upgrade. Fixing (`npm publish` from `sdk/`, with Trusted Publishing per the other Gap entry) unblocks: customers' long-running tailor/extract calls actually getting the full extended budget.
