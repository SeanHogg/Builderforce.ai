...
- **No per-tenant cap on the always-on paid last-resort fallback.** [LlmProxyService.PAID_LAST_RESORT_MODEL](api/src/application/llm/LlmProxyService.ts) is now appended to every non-strict candidate chain (Free plan included) — when the free pool + cross-vendor fallbacks are all exhausted, the proxy falls through to `google/gemini-2.5-flash-lite` so callers never see a cascade-exhausted 429. The residual risk: a Free-plan tenant in a tight retry loop can drive arbitrary spend against the Builderforce OpenRouter key because there's no per-tenant overflow cap or daily budget on the last-resort path. Fixing (track last-resort hits in `llm_usage_log` against a per-tenant `paid_overflow_daily_cap`, default ~$0.50/day, and surface the cap in the superadmin tenants page) unblocks: hard ceiling on overflow cost while preserving the "zero `LLM_UNAVAILABLE` escapes" guarantee.
