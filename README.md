...
- **Upstream `mambacode.js` (github:SeanHogg/Mamba) ships no built `dist/` — the installed package only contains `src/`, but its `package.json` declares `"main": "dist/index.js"`.** Effect: at production runtime, `import('mambacode.js')` in [frontend/src/lib/model-provider.ts](frontend/src/lib/model-provider.ts) throws; the surrounding try/catch swallows the error and `MambaModelProvider` is permanently `isReady() === false`. Vitest required a stub alias ([frontend/vitest.config.ts](frontend/vitest.config.ts) → [frontend/src/test/stubs/mambacode-stub.ts](frontend/src/test/stubs/mambacode-stub.ts)) to even transform the source — vite's static import-analysis was failing the entire test file before this. Webpack's production build is more lenient (defers resolution) so `next build` succeeds, but the Mamba runtime never actually starts. Fixing (add a `prepare: "npm run build"` script + `dist/` to `files`/`exports` in the upstream `SeanHogg/Mamba` repo, then bump the git ref in `frontend/package.json`) unblocks: the actual on-device Mamba WebGPU path that the entire studio's Mamba-coherence pitch depends on, and removes the test stub.
...
- **`@seanhogg/builderforce-studio` has never been published — the npmjs.com trusted-publisher UI does not exist for an unpublished package, so OIDC alone cannot bootstrap it.** The trusted-publisher settings page lives at `https://www.npmjs.com/package/<name>/access`, which only renders after the package exists. npmjs.com does not surface an "Add Package" affordance for users; the only way to create a package is to publish a tarball. [`.github/workflows/publish-npm-package.yml`](.github/workflows/publish-npm-package.yml) supports an `NPM_TOKEN` fallback for exactly this bootstrap. Operator steps: (1) On https://www.npmjs.com → Account → Access Tokens → Generate New Token → "Automation" type → copy the token. (2) On GitHub → Settings → Secrets and variables → Actions → New repository secret → name `NPM_TOKEN`, value the token. (3) Re-run `publish-studio` (or push any commit to main). (4) After the first successful publish, optionally visit https://www.npmjs.com/package/@seanhogg/builderforce-studio/access → Publishing access → Add trusted publisher (GitHub Actions / SeanHogg / Builderforce.ai / release.yml / `publish-studio`) and then delete the `NPM_TOKEN` secret — subsequent publishes will use OIDC. Fixing unblocks: the studio is actually published, third-party consumers can `npm install @seanhogg/builderforce-studio`, and the workflow eventually runs on zero-secret OIDC.
