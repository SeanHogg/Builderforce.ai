...
- **Superadmin / `/admin` mint-on-behalf flow for tenant API keys is missing.** Owners can self-serve at [/settings/api-keys](frontend/src/app/settings/api-keys/page.tsx), but platform admins can only do so via emulation (see [/admin](frontend/src/app/admin)) — there is no first-class admin-side "issue a `bfk_*` for tenant N" UI. Fixing unblocks: support / onboarding flows where Anthropic-side staff need to mint a key for a partner without impersonation overhead.
