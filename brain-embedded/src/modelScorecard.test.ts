...
describe('modelScorecard', () => {
  it('scores each model on the turns it served, first-seen order', () => {
    ...
    expect(scores).toEqual([
      { model: 'direct/qwen/qwen3.8-max', turns: 2, toolCalls: 3, textOnlyTurns: 0, unliftedMarkupTurns: 0, failures: 0 },
      { model: 'xai-oauth/grok-4.5', turns: 1, toolCalls: 0, textOnlyTurns: 1, unliftedMarkupTurns: 0, failures: 0 },
    ]);
  });
  ...
  it('counts a failed completion against a named model and ignores the auto-select placeholder', () => {
    ...
    expect(scores).toEqual([
      { model: 'xai-oauth/grok-4.5', turns: 0, toolCalls: 0, textOnlyTurns: 0, unliftedMarkupTurns: 0, failures: 1 },
    ]);
  });
});
