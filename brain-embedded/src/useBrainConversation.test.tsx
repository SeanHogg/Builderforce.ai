...
describe('a turn addressed to an invited agent', () => {
  const bob = { kind: 'agent' as const, ref: '42', name: 'Bob Developer' };
  const withAgentBackend = () => {
    const p = persistence as BrainPersistenceAdapter & {
      requestAgentReply: ReturnType<typeof vi.fn>;
      resolveAgentPersona: ReturnType<typeof vi.fn>;
    };
    p.requestAgentReply = vi.fn(async () => ({ id: 900, role: 'assistant', content: 'server said', metadata: JSON.stringify({ authoredBy: bob }), seq: 900, createdAt: '' }));
    p.resolveAgentPersona = vi.fn(async () => ({ directives: 'You are Bob, a developer.', model: null }));
    return p;
  };

  it("runs HERE, under the agent persona with this host's tools, when the host has them", async () => {
    const p = withAgentBackend();
    mockStream.mockResolvedValueOnce(result({ text: 'Committed.', resolvedModel: 'm1' }));
    const { rerender, result: hook } = renderHook(
      (props: { chatId: number }) =>
        useBrainConversation({ chatId: props.chatId, toolSpecs: [TOOL], runTool: vi.fn(), runAddressedAgentLocally: true }),
      { initialProps: { chatId: 1 }, wrapper },
    );
    rerender({ chatId: 1 });

    await act(async () => { await hook.current.send('commit and push to main', { addressedTo: bob }); });

    // The persona was fetched with the request text (knowledge recall), the local loop
    // ran with the host's tools, and the server was NOT asked to reply for the agent.
    expect(p.resolveAgentPersona).toHaveBeenCalledWith(1, { agentRef: '42', query: 'commit and push to main' });
    expect(p.requestAgentReply).not.toHaveBeenCalled();
    expect(mockStream).toHaveBeenCalledTimes(1);
    const opts = mockStream.mock.calls[0][0];
    const system = String((opts.messages[0] as { content: string }).content);
    expect(system.startsWith('You are Bob, a developer.')).toBe(true);
    expect(system).toContain('Reply AS Bob Developer');
    expect(opts.tools?.map((t) => t.function.name)).toContain('do_thing');
    // The reply is attributed to Bob in the transcript, exactly like a server reply.
    const assistant = vi.mocked(persistence.sendMessages).mock.calls.flatMap(([, m]) => m).find((m) => m.role === 'assistant');
    expect(JSON.parse(assistant!.metadata!)).toMatchObject({ authoredBy: bob, provenance: { model: 'm1' } });
    await waitFor(() => expect(hook.current.messages.map((m) => m.content)).toEqual(['commit and push to main', 'Committed.']));
  });

  it('falls back to the server reply when the persona cannot be fetched (an editor ahead of its API)', async () => {
    const p = withAgentBackend();
    p.resolveAgentPersona.mockRejectedValueOnce(new Error('404'));
    const { rerender, result: hook } = renderHook(
      (props: { chatId: number }) =>
        useBrainConversation({ chatId: props.chatId, toolSpecs: [TOOL], runTool: vi.fn(), runAddressedAgentLocally: true }),
      { initialProps: { chatId: 1 }, wrapper },
    ...
    let ok = false;
    await act(async () => { ok = await hook.current.send('commit it', { addressedTo: bob }); });

    expect(ok).toBe(true);
    expect(p.requestAgentReply).toHaveBeenCalledWith(1, { agentRef: '42', agentName: 'Bob Developer' });
    expect(mockStream).not.toHaveBeenCalled();
    await waitFor(() => expect(hook.current.messages.map((m) => m.content)).toEqual(['commit it', 'server said']));
  });

  it('still asks the server to reply for the agent where the host has no local tools', async () => {
    const p = withAgentBackend();
    const { rerender, result: hook } = renderHook(
    ...
    await act(async () => { await hook.current.send('what is the plan?', { addressedTo: bob }); });

    expect(p.requestAgentReply).toHaveBeenCalledWith(1, { agentRef: '42', agentName: 'Bob Developer' });
    expect(p.resolveAgentPersona).not.toHaveBeenCalled();
    expect(mockStream).not.toHaveBeenCalled();
    await waitFor(() => expect(hook.current.messages.map((m) => m.content)).toEqual(['what is the plan?', 'server said']));
  });
});
