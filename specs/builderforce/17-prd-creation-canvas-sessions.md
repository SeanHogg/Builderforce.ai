...
- **Focused Session chrome and diagnostics:** the header exposes only Session-level essentials; secondary modes live in one More menu and object operations appear contextually after selection. Canvas Brain uses the shared guest/authenticated model transport and MCP catalog, carries the bounded Session transcript through follow-up requests, and owns the `/create/*` conversation so the redundant floating Brain launcher is suppressed. Both the Session conversation and Canvas diagnostics panel produce a shared one-paste report with build, revision, realtime/access, graph, Brain, and recent-transcript state.
...
- **Release automation:** the deployed Playwright matrix now covers guest/onboarding/template/Brain review, durable Dashboard restore, Canvas-native Workflow and Voice, legacy Brainstorm adaptation, keyboard outline, reduced motion, forced colors, zoom, and 360px review. A 25-editor capacity probe and a fail-closed release-evidence auditor are documented in `docs/design/creation-canvas/evidence/README.md`.
...
| R-03 | **Automation complete; deployed execution pending** | Beta | QA + Product Engineering | The deployable Playwright matrix covers guest/onboarding/template/Brain review, Dashboard restore, Canvas Workflow/Voice surfaces, legacy adaptation, and accessibility modes. Execute it in the release environment and supplement it with configured real-resource Project comparison/delivery, actual audio capture/synthesis, multiplayer reconnect, and packaged VSIX scenarios. Retain traces and failure artifacts. |
| R-04 | **Harness complete; production evidence pending** | GA readiness | Security + Platform | The explicit-session capacity probe defaults to 25 concurrent editors and reports success/latency/revision ranges. Run it against the prepared 100-visible/1,000-referenced Session, complete invitation replay/cross-scope/malicious-resource review and large Project restore tests, and resolve every qualifying P0/P1 finding. |
| R-05 | **Automated modes complete; formal audit pending** | GA readiness | Frontend + Accessibility | Playwright now exercises keyboard focus, structured graph, 200% zoom, 360px review, forced colors, and reduced motion. Execute the deployed suite with a screen-reader user audit, retain results, and remediate release-blocking findings. |
| R-06 | **Repository conformance complete; production packaging pending** | GA and VSIX release | Developer Experience + Platform | Shared contract tests and web route smokes exist. Validate the same deployed Session in the packaged VSIX, including persistence, permissions, reconnect, theming, and supported rendering; complete Cloudflare/Vercel packaging smokes in the release environment. |
| R-07 | **Evidence contract complete; drills/sign-off pending** | GA | SRE + Support + Product | The runbook, evidence template, capacity command, and fail-closed `canvas:audit` check exist. Execute incident, backup/restore, feature/navigation rollback, delivery kill-switch, and capacity drills; attach dated records and final owner sign-offs. |
