...
class BaseAdapter(ABC):
    ...
    def validate_payload(self, payload: Dict[str, Any]) -> bool:
        ...
        """
        raise NotImplementedError("Subclasses must implement validate_payload")
...
).__getattribute__("sanitize_pii")
            if normalizer:
                try:
                    if any(keyword in str(record.service_id).lower() 
        ...
        for field in timestamp_fields:
            if field in payload:
                ...
                if isinstance(value, str):
                    ...
                    return datetime.fromtimestamp(value) if value > 10000000000000 else datetime.fromtimestamp(value / 1000.0)
