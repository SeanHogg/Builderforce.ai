...
export default function FreelancerGigsPage() {
  ...
  return (
    <PageContainer width="full" style={{ padding: '32px 40px' }}>
      ...
      {/* Open jobs to bid on */}
      {!loading && tab === 'work' && (
        jobs.length === 0 ? <div style={{ ...card, textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('jobs.emptyOpen')}</div> : (
      ...
      {!loading && tab === 'proposals' && (
        proposals.length === 0 ? <div style={{ ...card, textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('proposals.empty')}</div> : (
      ...
      {!loading && tab === 'engagements' && (
        engagements.length === 0 ? <div style={{ ...card, textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('gigs.empty')}</div> : (
