...
interface AgentHostChatContentProps {
  agentHostId: number;
...
export function AgentHostChatContent({ agentHostId, agentHostName }: AgentHostChatContentProps) {
  ...
  useEffect(() => {
    ...
    const gateway = new AgentHostGateway({
      url,
      onEvent: (ev) => {
        ...
        } else if (ev.type === 'agent_host_offline') {
          setConnStatus('offline');
        } else if (ev.type === 'agent_host_online') {
          setConnStatus('connected');
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!sent) {
        await dispatchApi.send(agentHostId, { type: 'chat.message', role: 'user', content: text });
      }
    } catch {
