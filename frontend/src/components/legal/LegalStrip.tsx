 ...
 * to render.
 *
 * Split out of `LegalCorner` so the SAME row can sit in two different layout
 * contexts without two copies of the fetch, the modal state and the markup: the
 * app shell's flow-based footer (`LegalCorner`, full width, in normal flow) and
 * the canvas's floating bottom-right corner (`CanvasUsageCorner`, a fixed
 * overlay that also carries the usage meters). Each caller owns ONLY where it
 * sits and whether it renders at all.
 */
