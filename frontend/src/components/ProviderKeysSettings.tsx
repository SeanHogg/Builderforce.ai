...
import { ProbeResultLine, ProviderHealthHeader, useProbeRunner, type ProbeVerdict } from '@/components/llm/ProviderProbeVerdict';
import {
  isDeadConnectCode,
...
function ProviderConnectionCard({
  ...
  const runProbe = useProbeRunner(t);
  ...
  const testConnection = async () => {
    ...
    try {
      const { verdict, result } = await runProbe(config.label, () => providerKeysApi.test(config.id));
...
function OllamaLocalConnectionCard({
  ...
  const testConnection = async () => {
    setTesting(true); setTestResult(null); setError(null);
    try {
      const { verdict, result } = await runProbe(config.label, () => providerKeysApi.test('ollama-local'));
      setTestResult(verdict);
      if (!result) return;
      onHealthChange(result.authAlert ?? null);
      await loadDiagnostic();
...
function OpenRouterConnectionsPanel({
  ...
  const testConnection = async (connection: OpenRouterConnection) => {
    ...
    setError(null);
    try {
      const { verdict, result } = await runProbe(connection.label, () => openRouterConnectionsApi.test(connection.id));
      ...
      if (result) onHealthChange(connection.id, result.authAlert ?? null);
    } finally {
      setTestingId(null);
    }
  };
