...
function AllocationPanel() {
  ...
  useEffect(() => { empMetricsApi.allocationHealth().then(setData).catch((e: unknown) => setErr(errorMessage(e))); }, [errorMessage]);
        ...
        <tbody>
          {data == null ? (
            ...
            <tr key={`${m.memberKind}:${m.memberRef}`} style={{ borderBottom: '1px solid var(--border-subtle)', background: m.overAllocated ? 'rgba(229,72,77,0.08)' : undefined }}>
              <td style={tdL}>
                {m.name}
                {m.overAllocated && <span style={{ marginLeft: 8, fontSize: 10, fontWeight: 700, color: 'var(--text-on-accent)', background: DANGER, padding: '1px 6px', borderRadius: 'var(--radius-full)' }}>{t('emp.overAllocated')}</span>}
              </td>
...
function CollaborationPanel({ days }: { days: number }) {
  ...
  const [err, setErr] = useState<string | null>(null);
  useEffect(() => { empMetricsApi.collaboration(days).then(setData).catch((e: unknown) => setErr(errorMessage(e))); }, [days, errorMessage]);
...
function DocActivityPanel({ days }: { days: number }) {
  ...
  const [err, setErr] = useState<string | null>(null);
  useEffect(() => { empMetricsApi.docActivity(days).then(setData).catch((e: unknown) => setErr(errorMessage(e))); }, [days, errorMessage]);
...
function LaborCostPanel({ days }: { days: number }) {
  ...
  const [err, setErr] = useState<string | null>(null);
  useEffect(() => { empMetricsApi.laborCost(days).then(setData).catch((e: unknown) => setErr(errorMessage(e))); }, [days, errorMessage]);
...
function CoachingNotes({ member }: { member: PerformerRow }) {
  ...
  return (
    <div style={{ padding: '10px 12px', background: 'var(--bg-elevated)', borderRadius: 'var(--radius-md)', marginTop: 6 }}>
      {notes == null ? (
...
function PerformersPanel({ days }: { days: number }) {
  ...
  useEffect(() => { empMetricsApi.performerTiers(days).then(setData).catch((e: unknown) => setErr(errorMessage(e))); }, [days, errorMessage]);
      ...
      {data && (
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {(['high', 'solid', 'watch'] as PerformerTier[]).map((tier) => (
            <div key={tier} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 'var(--radius-full)', fontSize: 12, border: '1px solid var(--border-subtle)' }}>
              <span style={{ width: 8, height: 8, borderRadius: 'var(--radius-full)', background: TIER_COLOR[tier] }} />
            ...
            return (
              <Fragment key={key}>
                <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  <td style={tdL}>{m.name}</td>
                  <td style={td}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-on-accent)', background: TIER_COLOR[m.tier], padding: '1px 8px', borderRadius: 'var(--radius-full)' }}>{tierLabel(m.tier)}</span>
                  </td>
                  <td style={td}>{pct(m.percentile)}</td>
                  <td style={{ ...td, fontWeight: 700 }}>{int(m.composite)}</td>
                  <td style={td}>
                    <button onClick={() => setOpen(open === key ? null : key)} style={{ background: 'none', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', padding: '2px 8px', fontSize: 11, cursor: 'pointer', color: 'var(--text-secondary)' }}>
                      {t('emp.addNote')}
                    </button>
                  </td>
                </tr>
...
function InitiativeAllocationPanel({ days }: { days: number }) {
  ...
  const [err, setErr] = useState<string | null>(null);
  useEffect(() => { empMetricsApi.initiativeAllocation(days).then(setData).catch((e: unknown) => setErr(errorMessage(e))); }, [days, errorMessage]);
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
          {data.initiatives.map((i) => (
            <span key={i.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
              <span style={{ width: 10, height: 10, borderRadius: 'var(--radius-sm)', background: colorByInit.get(i.id) }} />
              {i.name}
            </span>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {data.members.map((m) => (
            <div key={`${m.memberKind}:${m.memberRef}`}>
              ...
              <div style={{ display: 'flex', height: 20, borderRadius: 'var(--radius-sm)', overflow: 'hidden', background: 'var(--border-subtle)' }} title={m.slices.map((s) => `${s.initiativeName} ${s.pct}%`).join(' · ')}>
                {m.slices.map((s) => (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button onClick={doExport} disabled={exporting} style={{
            padding: '4px 12px', borderRadius: 'var(--radius-md)', fontSize: 12, cursor: 'pointer', border: '1px solid var(--border-subtle)',
