 ...
 * two API calls on every keystroke elsewhere on the page. Fetching on click also makes
 ...
 * session. Both reads fail soft and the report states which one was unavailable —
...
export function ManagerCopyDiagnostics({ projectId, overview }: ManagerCopyDiagnosticsProps) {
  ...
  const buildReport = useCallback(async (): Promise<string> => {
    if (!overview) return '';
    // Both in parallel, both fail-soft: a missing block is reported as missing.
    const [stalls, census] = await Promise.all([
