...
export default function LlmRatingsPanel() {
  ...
  return (
    <div>
      ...
      {empty ? (
        <div style={{ ...card }}>
          <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>{t('emptyTitle')}</p>
          <p className="text-muted" style={{ margin: '6px 0 0', fontSize: 12 }}>{t('emptyBody')}</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ ...card, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 16 }}>
            <div>
              <div className="text-muted" style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: .4 }}>{t('statSatisfaction')}</div>
              <div style={{ fontSize: 26, fontWeight: 700, color: scoreColor(totals.score) }}>{pct(totals.score)}</div>
            </div>
            <div>
              <div className="text-muted" style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: .4 }}>{t('statUp')}</div>
              <div style={{ fontSize: 26, fontWeight: 700, color: 'var(--text-strong)' }}>{fmtNum(totals.up)}</div>
            </div>
            <div>
              <div className="text-muted" style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: .4 }}>{t('statDown')}</div>
              <div style={{ fontSize: 26, fontWeight: 700, color: 'var(--text-strong)' }}>{fmtNum(totals.down)}</div>
            </div>
            <div>
              <div className="text-muted" style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: .4 }}>{t('statModels')}</div>
              <div style={{ fontSize: 26, fontWeight: 700, color: 'var(--text-strong)' }}>{fmtNum(data?.models ?? 0)}</div>
            </div>
          </div>
          ...
          <div style={card}>
            <h3 style={{ margin: '0 0 4px', fontSize: 14, fontWeight: 600, color: 'var(--text-strong)' }}>{t('verdictsTitle')}</h3>
            <p className="text-muted" style={{ margin: '0 0 12px', fontSize: 12 }}>{t('verdictsHint')}</p>
            {(data?.leaders.length ?? 0) === 0 ? (
              <p className="text-muted" style={{ margin: 0, fontSize: 12 }}>{t('verdictsEmpty')}</p>
            ) : (
              <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {data!.leaders.map((leader) => (
                  <li
                    key={`${leader.actionType}-${leader.toolName ?? ''}`}
                    style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', gap: 8, fontSize: 13, color: 'var(--text-primary)' }}
                  >
          ...
          <div style={card}>
            <h3 style={{ margin: '0 0 12px', fontSize: 14, fontWeight: 600, color: 'var(--text-strong)' }}>{t('breakdownTitle')}</h3>
            ...
            <p className="text-muted" style={{ margin: '10px 0 0', fontSize: 11 }}>{t('scoreNote')}</p>
          </div>
        </div>
      )}
    </div>
