    ...
    <div>
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        <h2 className="page-title" style={{ fontSize: 18, margin: 0 }}>
          {t('impsessions.title')}
          <span style={{ marginLeft: 8, fontSize: 13, fontWeight: 400, color: 'var(--text-muted)' }}>{t('impsessions.total', { n: impSessionsTotal })}</span>
        </h2>
      ...
      <table className="data-table">
        <thead>
          <tr>
            <th>{t('impsessions.target')}</th>
            <th>{t('impsessions.workspace')}</th>
            <th>{t('impsessions.role')}</th>
            <th>{t('impsessions.reason')}</th>
            <th>{t('impsessions.started')}</th>
            <th>{t('impsessions.ended')}</th>
            <th>{t('impsessions.durationPages')}</th>
          </tr>
        </thead>
        <tbody>
          {impSessions.map((s) => {
          ...
          {impSessions.length === 0 && (
            <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 20 }}>{t('impsessions.noneFound')}</td></tr>
          )}
        </tbody>
      </table>
      {impSessionsTotal > 50 && (
        <div style={{ marginTop: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 13 }}>{t('impsessions.pagination', { from: impSessionsOffset + 1, to: Math.min(impSessionsOffset + 50, impSessionsTotal), total: impSessionsTotal })}</span>
          <button
            type="button"
            className="admin-tab"
