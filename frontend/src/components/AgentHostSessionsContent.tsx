...
export function AgentHostSessionsContent({ agentHostId }: AgentHostSessionsContentProps) {
  ...
  if (selectedSession) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {showHandoff && (
          <div style={cardStyle}>
            <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 10 }}>Hand off to another agentHost</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {sessions.length === 0 ? (
        <div style={{ ...cardStyle, fontSize: 13, color: 'var(--text-muted)', textAlign: 'center' }}>
          No sessions yet. Sessions are created when a agentHost starts a conversation.
        </div>
      ) : (
