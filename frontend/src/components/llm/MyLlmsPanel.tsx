...
export function MyLlmsPanel() {
  ...
  const remove = async (m: TenantModel) => {
    if (!confirm(t('deleteConfirm', { name: m.name }))) return;
    try {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, gap: 12, flexWrap: 'wrap' }}>
        <div>
          <h2 style={{ fontSize: '1.1rem', fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>{t('heading')}</h2>
      ...
      {draft && (
        <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 12, padding: 20, marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input style={inputStyle} value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder={t('namePlaceholder')} autoFocus />
          </div>
          <div>
          ...
          <div>
            ...
            <textarea style={{ ...inputStyle, minHeight: 100, resize: 'vertical', fontFamily: 'inherit' }} value={draft.systemPrompt} onChange={(e) => setDraft({ ...draft, systemPrompt: e.target.value })} placeholder={t('systemPromptPlaceholder')} />
          </div>
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 160px' }}>
              <label style={labelStyle}>{t('temperature')}</label>
              <input style={inputStyle} type="number" step="0.1" min="0" max="2" value={draft.temperature} onChange={(e) => setDraft({ ...draft, temperature: e.target.value })} placeholder={t('temperaturePlaceholder')} />
            </div>
            <div style={{ flex: '1 1 160px' }}>
              ...
              <Select style={inputStyle} value={draft.visibility} onChange={(e) => setDraft({ ...draft, visibility: e.target.value as 'private' | 'tenant' })}>
                <option value="tenant">{t('visibilityTenant')}</option>
                <option value="private">{t('visibilityPrivate')}</option>
              </Select>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <button type="button" onClick={() => setDraft(null)} style={{ padding: '8px 14px', fontSize: 13, background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }}>{t('cancel')}</button>
            <button type="button" onClick={save} disabled={saving || !draft.name.trim()} style={{ padding: '8px 18px', fontSize: 13, fontWeight: 600, background: 'var(--coral-bright)', color: '#fff', border: 'none', borderRadius: 8, cursor: saving || !draft.name.trim() ? 'not-allowed' : 'pointer', opacity: saving || !draft.name.trim() ? 0.7 : 1 }}>
              {saving ? t('saving') : draft.id ? t('saveChanges') : t('createLlm')}
            </button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 40, background: 'var(--bg-elevated)', borderRadius: 12, border: '1px solid var(--border-subtle)' }}>
          ...
          <p style={{ color: 'var(--text-secondary)' }}>{t('emptyState')}</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {models.map((m) => (
            <div key={m.id} style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{m.baseModel ?? t('defaultBaseModel')}</div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                <button type="button" onClick={() => setDraft(draftFromModel(m))} style={{ fontSize: 12, fontWeight: 600, background: 'none', border: 'none', color: 'var(--coral-bright)', cursor: 'pointer', padding: 0 }}>{t('edit')}</button>
                <button type="button" onClick={() => remove(m)} style={{ fontSize: 12, fontWeight: 600, background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', padding: 0 }}>{t('delete')}</button>
              </div>
            </div>
          ))}
        </div>
