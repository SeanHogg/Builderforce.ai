...
export interface AgentHostDebugContentProps {
  /** AgentHost to debug (required). */
  agentHostId: number;
  /** Display name for the agentHost. */
...
export function AgentHostDebugContent({
  agentHostId,
  agentHostName,
  className,
  style,
  ...
  useEffect(() => {
    ...
    const gw = new AgentHostGateway({
      ...
      onEvent: (ev) => {
        ...
        if (ev.type === 'agent_host_online') {
          setConnState('connected');
          return;
        }
        if (ev.type === 'agent_host_offline') {
          setConnState('offline');
          rejectPending('agentHost offline');
          return;
        }
