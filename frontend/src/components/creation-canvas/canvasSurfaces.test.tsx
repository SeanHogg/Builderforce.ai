...
describe('the chat surface on the canvas', () => {
  ...
  it('offers no full-size surface for a kind whose card is the whole object', () => {
    ...
    const panel = screen.getByRole('dialog', { name: /Everything/ });
    expect(within(panel).queryByTestId('open-page-surface')).not.toBeInTheDocument();
    expect(within(panel).queryByTestId('open-play-surface')).not.toBeInTheDocument();
    expect(within(panel).queryByTestId('open-site-surface')).not.toBeInTheDocument();
    expect(within(panel).queryByTestId('open-timeline-surface')).not.toBeInTheDocument();
  ...
  it('opens a site at a width the reader picks, without re-authoring the object', () => {
    ...
    fireEvent.click(within(surface).getByRole('button', { name: 'Back to the board' }));
    expect(board()).toHaveAttribute('data-view', 'graph');
    fireEvent.click(within(screen.getByRole('dialog', { name: /Everything/ })).getByTestId('open-site-surface'));
