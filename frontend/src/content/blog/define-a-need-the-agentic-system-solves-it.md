...
- **`deploy`** is a registry of *surfaces* — IDE, desktop, cloud-durable, cloud-container, and workflow-step. It takes a finished `AgentSpec`, resolves the right engine through one shared DI registry and the right transport for the surface, and — through `deployAndDispatch` — actually starts the run on the machinery that already exists.
...
A persona's temperature changes how the agent behaves whether it runs as a workflow step or a cloud agent. The proprietary documents you trained on are recalled at inference no matter where the agent executes — down to a stock OpenAI-SDK call that addresses the agent by its model id.

And a governance gate is *enforced*, not merely suggested. The same compiled gate is evaluated at each engine's tool seam — the durable cloud loop, the on-premise runner, and the in-editor IDE loop — through one shared `evaluatePolicyGate` decision. `block` refuses the tool and tells the agent to take another path; `require-approval` pauses the run, asks a human, and resumes once they answer. Author `block` on the `shell` tool once, and it stops the shell in your editor exactly as it stops it on a cloud tick — because the gate rides on the agent, not on the place it runs. That is the whole point of putting policy on the spec instead of inside one front door: it cannot be the case that a rule holds in one surface and not another, because there is only one rule and one place it is checked.
...
- **Deploy *and dispatch* that one spec.** `deploy()` resolves the engine, transport, and run input for any surface; `deployAndDispatch()` then *starts* it — a step-bearing spec becomes a live workflow, a cloud spec a running cloud agent with its governance gates already in the payload.
...
- **Ground a stock OpenAI call** — address a trained agent by its `builderforce/workforce-<id>` id on the standard `/v1/chat/completions` endpoint and it recalls your ingested docs for that query, the same grounding the dedicated chat path gets.
- **Govern from the spec** — a `block` or `require-approval` gate authored once is enforced at the tool seam on the cloud, on-premise, and IDE runners alike: refused or human-gated, not just instructed.
