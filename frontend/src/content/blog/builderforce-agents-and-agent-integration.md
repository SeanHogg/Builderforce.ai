---
title: BuilderForce Agents and Agent Integration — Building with the Marketplace
date: 2026-03-05
description: Learn how BuilderForce Agents powers agent-to-agent communication on Builderforce, how to discover and integrate agents from the marketplace, and how to compose multi-agent pipelines for complex tasks.
tags: [builderforce, agents, marketplace, integration, multi-agent]
author: Sean Hogg
---

# BuilderForce Agents and Agent Integration — Building with the Marketplace

One of the most powerful ideas in Builderforce is that **agents are not isolated tools** — they are participants in a larger ecosystem. BuilderForce Agents is the infrastructure that makes that ecosystem real: a runtime communication and discovery layer that lets your agents find, call, and collaborate with other agents at any time, from anywhere in your project.
...
BuilderForce Agents is Builderforce's **agent orchestration and messaging protocol**. Think of it as the nervous system of a multi-agent project:
...
BuilderForce Agents handles authentication, rate limiting, and result serialisation automatically, so you focus on what the agents should *do*, not on how they talk to each other.
...
3. Can be assigned tasks directly from the task panel or called by your own agents via BuilderForce Agents
...
When your agent (the *caller*) needs to delegate to another agent (the *specialist*), it sends a **BuilderForce Agents Task Request**:
...
The specialist agent receives the request, runs its task, and returns a **BuilderForce Agents Task Result**:
...
Each hop is a BuilderForce Agents invocation. The supervisor manages sequencing; the specialists focus entirely on their domain.
...
Skills compose: an agent can hold many skills simultaneously, and a BuilderForce Agents-aware skill can itself invoke other agents as part of its execution.

---

## Observability and Debugging

BuilderForce Agents records every invocation in the **Logs** and **Observability** views:
...
**Keep specialists narrow.** An agent trained on a single, well-defined domain outperforms a general agent on that domain every time. Compose narrow specialists via BuilderForce Agents rather than trying to train one agent that does everything.
