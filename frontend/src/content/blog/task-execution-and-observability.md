...
| **Execution** | A specific attempt to run that task on a specific BuilderForce Agents instance |
| **Workflow** | A structured multi-step orchestration that a BuilderForce Agents agent executes to complete a task |
| **Workflow task** | An individual step within a workflow (e.g. the "coder" step or the "reviewer" step) |

A single **task** can have multiple **executions** over time (retries, re-runs). Each execution is associated with exactly one **workflow** when the BuilderForce Agents orchestrator runs a DAG to complete it.
...
- **Assigned agentHost** — which BuilderForce Agents instance should execute it
- **Persona** — which agent role should lead execution
- **GitHub PR URL** — linked automatically once a agentHost creates a pull request

---

## Execution Lifecycle

When a task is submitted for execution (via `POST /api/runtime/executions` or via portal dispatch), an **execution record** is created and a `task.assign` event is dispatched to the agentHost via the relay.
...
The agentHost reports each transition back to Builderforce automatically:
...
You can watch these transitions in real time on the [Timeline](/timeline) page — the execution card updates live as the agentHost reports status.

---

## Workflow Telemetry

When BuilderForce Agents runs an orchestrated workflow to complete a task, it emits **structured telemetry spans** — one per workflow and one per task step. These spans appear in two places:

### 1. Local JSONL (on the agentHost)
...
This means the Workflows page is a **live view** of what every connected agentHost is doing right now. No manual querying required.
...
- **AgentHost** — filter to a specific machine
...
The dashboard breaks down by project, by agentHost, and by agent role so you can see which parts of your system consume the most resources or fail most often.
...
For executions that matter right now, you can subscribe to live updates via the WebSocket stream at `GET /api/runtime/executions/:id/stream`. This is what the portal's live execution card uses under the hood — each status transition and telemetry event is pushed the moment it arrives from the agentHost.
...
**Assign agentHosts to tasks explicitly** when you have a fleet. An unassigned task broadcasts to all connected agentHosts — fine for exploration, but noisy in production. Pin tasks to the agentHost with the right workspace and model.
