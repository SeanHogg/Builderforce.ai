...
export const CANVAS_SURFACES: readonly CanvasSurfaceDef[] = [
  ...
  // so there is no card to enter it from. It sits straight after the board because it is
  // the Idea stage of the arc — the thing a founder does BEFORE there is anything to meet
  // about, run or measure. It does not draw the board or its objects: it is a reading of
  // the `idea` cards (newest first, grouped by stage, the untested ones called out), and a
  // floating "Align · Frame" toolbar over a list would be chrome for something not there.
  //
