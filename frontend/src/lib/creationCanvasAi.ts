...
function canvasModeDirective(mode: ChatMode, projectId: number | null | undefined): string {
  ...
  return (
    ...
    + '• FINISH BY DISPATCHING. builtin_tasks_create and builtin_tasks_update return an `autoRun` verdict — read it. If `autoRun.dispatched` is true, name the agent that picked the work up. If it is false, pick a capable agent with builtin_cloud_agents_list_mine and start the run with builtin_tasks_run_now. If dispatch is refused, report the EXACT reason the tool returned and what would clear it.\n'
