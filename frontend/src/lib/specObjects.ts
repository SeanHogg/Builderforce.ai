...
import type { CanvasObjectGroup as CreationObjectGroup } from '@/domains/canvas/domain/canvasObject';
// A worded derivation's descriptor, and the ONE formatter the card and the model share.
// Its own module — see `specVerdict.ts` for the verdict rules.
...
export interface SpecField {
   ...
   *
   * ── A DERIVATION THAT SAYS SOMETHING IN WORDS RETURNS A {@link SpecVerdict} ────
   * Never an English sentence built in code: the card would draw it to every locale.
   * Return `specVerdict(key, values)` (or a list of them) and put the sentence in the
   * catalog under `<namespace>.verdict.<key>`. The node body translates it; the AI
   * snapshot formats the same descriptor in English. Numbers, dates and ISO strings
   * stay plain values — only WORDS move.
   */
 ...
 *
 * A worded derivation's {@link SpecVerdict} is formatted in ENGLISH here — the model
 * reads the sentence, never the descriptor — through the same formatter the card uses.
 */
export function specDerivedValues(
  ...
  if (!entry) return {};
  return Object.fromEntries(entry.spec.fields.flatMap((field) => {
    if (!field.derive || field.restricted) return [];
    const value = specValueInEnglish(field.derive(data, board), entry.set.namespace);
