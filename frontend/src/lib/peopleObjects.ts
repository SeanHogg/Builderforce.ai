...
export const PEOPLE_OBJECT_SPECS: readonly SpecObjectSpec[] = [
  ...
  // ── THE PAPER AND THE PROCESS ─────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      { name: 'reviewAt', render: 'stat', label: 'reviewAt', hint: 'ISO date this must be reviewed by. Bind a `trigger` with comparator "due-within" to it — the failure mode of a policy library is not a bad policy, it is a stale one nobody re-read.', deadline: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'dueAt', render: 'stat', label: 'dueAt', hint: 'ISO date of the next deadline. Bind a `trigger` with comparator "due-within" to it so the board warns before rather than reporting after.', deadline: true },
