...
export function managerChatFindings(input: ManagerChatDiagnosticsInput): ManagerFinding[] {
  ...
  if (narrating.length > 0 && roll.toolCalls === 0) {
    push('critical', 'tools_narrated_never_called',
      `${narrating.length} repl${narrating.length === 1 ? 'y' : 'ies'} describe the tools the manager needs while the trace records ZERO tool calls${roll.modelTurns > 0 ? ` across ${roll.modelTurns} model turn${roll.modelTurns === 1 ? '' : 's'}` : ''}. The manager is naming tools it never invoked. The usual cause is a NAME MISMATCH: the system prompt referencing a tool by its catalog id (manager.digest) while the model is advertised a transformed name (builtin_manager_digest), so the instruction points at nothing and the model narrates instead. Check that every tool named in the prompt appears in the advertised tool list${roll.advertisedTools != null ? ` (this turn advertised ${roll.advertisedTools} tools)` : ''}.`);
