import type { ChatCompletionMessage, ChatMode } from '@seanhogg/builderforce-brain-embedded';
import { isWebsiteRedesignRequest } from '@/lib/canvasTurnOutcome';
import { founderCanvasSystemPrompt } from '@/lib/founderCanvasPrompt';
import { canvasModeDirective } from '@/lib/canvasModeDirective';

/**
 * THE CANVAS SYSTEM PROMPT — every instruction a Canvas turn sends ahead of the
 * user's message, assembled once.
 *
 * Split out of `creationCanvasAi.ts` because a prompt and a loop change for
 * different reasons and on different evidence. Nearly every block below carries
 * a dated measurement of the failure that put it there (a model refusing to
 * generate an image it had a tool for, a free coder echoing the previous reply,
 * a redesign delivered with no "before"), and those edits have nothing to say
 * about step budgets, provider fallback or the confirm gate — which is the rest
 * of what the runner does. Keeping them in one module meant every prompt fix
 * touched the file that runs the agent loop.
 *
 * The order is load-bearing and the comments say so where it is: the tenant-gated
 * blocks are conditional because `check-canvas-tool-contract` fails the build if
 * the prompt names a tool the turn will not be given, and MODE is last so it sits
 * nearest the user's turn.
 *
 * Pure: it reads the turn's facts and returns messages. No transport, no state.
 */
export interface CanvasPromptInput {
  prompt: string;
  canvasSnapshot: string;
  persistence: 'local' | 'server';
  /** Evermind recall, already formatted; empty when nothing was seeded. */
  memoryBlock: string;
  participant?: { name: string; instructions?: string };
  mode?: ChatMode;
  projectId?: number | null;
  conversation?: Array<{ role: 'user' | 'assistant' | 'system'; content: string }>;
}

export function canvasSystemMessages(options: CanvasPromptInput): ChatCompletionMessage[] {
 ...
 *
 * Lives here rather than with the runner because it reads THIS file's output: the
 * set it returns is only correct for as long as it is derived from the same blocks
 * above, and a tool named in a new paragraph must not need a second edit elsewhere.
 */
export function promptNamedTools(messages: readonly ChatCompletionMessage[]): Set<string> {
