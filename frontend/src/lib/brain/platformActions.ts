...
/** Normalize a title for idempotent-create dedup — mirror of the server builtin
 *  catalog (`builtinMcpService.normTitle`) so both runtimes dedup identically. */
const normTitle = (v: unknown): string => String(v ?? '').replace(/\s+/g, ' ').trim().toLowerCase();

/**
 * Idempotent create for the name-keyed planning tools (tasks / objectives / key
 * results). If an item whose title matches `title` already exists in `existing()`,
 * return it flagged `deduped` instead of minting a duplicate — so the Brain
 * re-running a roadmap reconciliation gets the existing id back for traceability.
 * This is the client-manifest twin of the guard in the gateway builtin catalog
 * (`builtinMcpService.ts`), so the web Brain never duplicates whether it runs the
 * server `builtin_*` tool or falls back to this client capability.
 */
async function dedupedCreate<T extends { title?: unknown }>(
  title: string,
  existing: () => Promise<T[]>,
  create: () => Promise<T>,
): Promise<T | (T & { deduped: true })> {
  const n = normTitle(title);
  if (n) {
    const dupe = (await existing()).find((r) => normTitle(r.title) === n);
...
export function buildPlatformCapabilities(ctx: PlatformActionContext): PlatformCapability[] {
  ...
  const caps: PlatformCapability[] = [
    ...
    { domain: 'cloud_agents', method: 'create', mutates: true, description: 'Create a cloud agent. The engine is always the current agent version (no selection).', parameters: obj({ name: S, title: S, bio: S, skills: arr(S), baseModel: S, published: B }, ['name']), run: (a) => createCloudAgent(a as unknown as Parameters<typeof createCloudAgent>[0]) },
