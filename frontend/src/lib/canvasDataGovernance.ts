...
// ---------------------------------------------------------------------------
// Vocabulary and the use gate — RE-EXPORTED, not declared here
// ---------------------------------------------------------------------------
//
// Both moved to `@builderforce/creation-canvas-contract/dataGovernance`, and the reason
// is the one the roadmap recorded: `evaluateDatasetUse` refuses a TRAINING use, and a
// training run is dispatched by `POST /api/ide/training` in the API — which cannot import
// a frontend module. So the gate that most needed to be asked at a server boundary was
// structurally unreachable from it.
//
// Re-exported rather than moved-and-rewired so every existing import path in the canvas
// keeps working: this module is still the one place canvas code asks about classification,
// and there is still exactly one implementation. Detection, masking and the data-contract
// evaluator stay declared BELOW — they read `TabularSource` and only ever run in a browser.
export {
  DATA_CLASSIFICATIONS, PII_CATEGORIES, DATASET_USES, LAWFUL_BASES,
  highestClassification, isDataClassification, isPiiCategory, isLawfulBasis, isDatasetUse,
  normalizeClassifications, normalizeUsePolicy, evaluateDatasetUse,
} from '@builderforce/creation-canvas-contract';
export type {
  DataClassification, PiiCategory, ClassificationConfidence, ColumnClassification,
  DatasetUse, LawfulBasis, DatasetUsePolicy, DatasetUseDecision, UseRefusalCode,
} from '@builderforce/creation-canvas-contract';

// The re-export above makes these available to this module's CONSUMERS; a re-export does
// not put a binding in local scope, so the handful this file's own detector and contract
// evaluator call are imported as well.
import {
  ALWAYS_MASKED, PII_CLASSIFICATION, highestClassification, isDataClassification, isPiiCategory,
  type ColumnClassification, type ClassificationConfidence,
  type DataClassification, type PiiCategory,
} from '@builderforce/creation-canvas-contract';
