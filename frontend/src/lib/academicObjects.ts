...
// The arithmetic the computed fields resolve through. A DERIVATION, not a declaration —
// see the header of that module for why the two are not in one file.
import {
  deriveAssignmentMarkedCount, deriveAssignmentSubmissionCount, deriveBibliographyEntryCount,
...
export const ACADEMIC_OBJECT_SPECS: readonly SpecObjectSpec[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'dueAt', render: 'stat', label: 'dueAt', hint: `The deadline. ${ISO_DATE} Bind a \`trigger\` with comparator "due-within" so the cohort is reminded before it, not marked late after it.`, deadline: true },
      ...
      { name: 'deliverables', render: 'list', label: 'deliverables', hint: 'What must be handed in: [{title, detail}] where detail names the format and any length limit.' },
      {
        ...
        derive: deriveAssignmentSubmissionCount,
      },
      {
        name: 'markedCount',
        render: 'stat',
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        derive: deriveGradebookDistribution,
      },
      {
        name: 'mean',
        render: 'stat',
      ...
      },
      {
        ...
        derive: deriveGradebookPassRate,
      },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'slots', render: 'rows', label: 'slots', columns: ['startsAt', 'durationMinutes', 'mode', 'bookedBy', 'topic'], hint: 'Bookable slots: {startsAt, durationMinutes, mode, bookedBy, topic}. `bookedBy` is a learnerRef or empty.' },
      {
        ...
        derive: deriveOfficeHoursUtilisation,
      },
    ],
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        derive: deriveCurriculumMapCoverage,
      },
      {
        name: 'gaps',
        render: 'chips',
        ...
        derive: deriveCurriculumMapGaps,
      },
      SOURCES_FIELD,
    ],
  },
  ...
  // ══ RESEARCH ══════════════════════════════════════════════════════════════════
  {
    ...
    fields: [
      ...
      { name: 'deadlineAt', render: 'stat', label: 'deadlineAt', hint: `Submission deadline. ${ISO_DATE} A missed funder deadline is a whole year, so bind a \`trigger\` with comparator "due-within".`, deadline: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'demographics', render: 'rows', label: 'demographics', columns: ['attribute', 'category', 'count', 'percent'], hint: 'Composition of the sample as recruited: {attribute, category, count, percent}. Aggregate only — never individual records.' },
      {
        ...
        derive: deriveParticipantPoolConsentRate,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'dueAt', render: 'stat', label: 'dueAt', hint: `When the review is due. ${ISO_DATE}`, deadline: true },
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        derive: deriveBibliographyEntryCount,
      },
