 ...
 *
 * They were chained `tsgo --noEmit && tsc --noEmit`, which is 27s + 35s serial for
...
for (const [name, file] of CHECKERS) {
  if (!existsSync(resolve(here, file))) {
    throw new Error(`typecheck.manifest: ${name} not found at ${file} — run \`pnpm install\` in frontend/.`);
  }
}
