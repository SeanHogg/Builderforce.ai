...
if (failures.length > 0) {
  ...
  if (measured.literalHexFiles > BASELINE.literalHexFiles) {
    ...
    for (const f of hexFiles.slice(0, 20000)) console.error(`    • ${f}`);
  ...
  if (measured.offScaleRadii > BASELINE.offScaleRadii) {
    ...
    for (const r of offScale.slice(0, 20000)) console.error(`    • ${r}`);
  ...
  if (measured.offScaleFontSizes > BASELINE.offScaleFontSizes) {
    ...
    for (const f of offScaleType.slice(0, 20000)) console.error(`    • ${f}`);
  ...
  if (measured.publicColumnLiterals > BASELINE.publicColumnLiterals) {
    ...
    for (const c of columnLiterals.slice(0, 20000)) console.error(`    • ${c}`);
