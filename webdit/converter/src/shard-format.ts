...
/**
 * WebDiT weight-shard binary format. Single owner of the spec — both the
 * writer (bundle-writer.ts) and the reader / verifier consume this module so
...
export function parseBundleShard(buf: Uint8Array): ShardSummary {
  ...
  for (const [name, entry] of Object.entries(header)) {
    ...
    const data = copyToOwnedBytes(buf.subarray(dataLo, dataHi));
    const scaleBytes = copyToOwnedBytes(buf.subarray(scaleLo, scaleHi));
...
/**
 * Copy `view` to a freshly-allocated buffer so any TypedArray view we hand
 * back to callers owns its memory and is properly aligned (Uint16Array needs
 * 2-byte alignment, which subarray does not guarantee).
 */
function copyToOwnedBytes(view: Uint8Array): Uint8Array {
