...
| 16 | `8a97a09c` | Marketplace Upwork-competitive | Payments move end-to-end: charge → escrow → payout live in production | **Partial** | **Charge — implemented**: `api/src/infrastructure/payment/StripeProvider.ts` (213 lines) — subscription billing via Stripe Checkout, webhook handling for checkout/subscription/invoice events; `api/src/infrastructure/payment/HelcimProvider.ts` (180 lines) — HelcimPay.js fallback. **Payout — seam exists on the incoming base branch** (see note below): `api/src/application/integrations/payments.ts` defines a `createPayout` / `isPayoutsConfigured` provider seam gated on `PAYOUT_WEBHOOK_URL` (`api/src/env.ts`), consumed by `POST /invoices/:invId/pay` in `api/src/presentation/routes/activityRoutes.ts`; unconfigured deployments fall back to a manual "Mark paid" with no money movement. **Escrow — absent**: no escrow or milestone-payment code found in `api/src`. Status remains `Partial`: charge is live, payout is a configurable seam rather than proven-live-in-production, and the escrow leg is missing entirely. |
...
3. **Marketplace monetization (13–16)**: Subscription billing is `Implemented` (Stripe + Helcim providers). The marketplace has listings (`marketplaceRoutes.ts`, 698 lines) and stats (`marketplaceStatsRoutes.ts`, 181 lines). Escrow, milestone-based payments and fixed-price contracts remain absent. Payouts are the exception to the earlier "zero code" claim: a `PAYOUT_WEBHOOK_URL`-gated provider seam (`integrations/payments.ts` + `POST /invoices/:invId/pay`) exists on the **base branch** being synced in, though not in this branch's checkout — see the base-sync correction above. The ROADMAP.md Consolidated Feature Register lists "Agent marketplace monetization (listing + tx fee)" as 📋 Planned (P1, Q3 2026). Net: charge leg live, payout seam inbound and env-gated, escrow leg missing.
